...
def test_public_constants():
    assert GOVERNANCE_KERNEL_CLI_DRY_RUN_VERSION == "5.0.0"
    assert GOVERNANCE_KERNEL_CLI_SCHEMA_VERSION == "5.0.0"
...
def test_canonicalize_event_mode_returns_v460_canonicalization_version():
    ...
    assert result["dry_run_status"] == "pass"
    assert result["result"]["canonicalization_version"] == "5.0.0"
...
def test_invalid_schema_version_is_blocked():
    ...
    assert result["dry_run_status"] == "blocked"
    assert "schema_version must equal 5.0.0" in result["blocking_reasons"]
