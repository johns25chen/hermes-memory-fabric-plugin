...
from hermes_memory_fabric.governance_anti_overreach_governance_firewall import (
    ...
    V6_11_HANDOFF_STATUS,
    _anti_overreach_governance_firewall_hash,
    _firewall_hash,
    _firewall_record_hash,
    build_governance_anti_overreach_governance_firewall,
    get_governance_anti_overreach_governance_firewall_check,
    get_governance_anti_overreach_governance_firewall_contract,
    get_governance_anti_overreach_governance_firewall_record,
    get_governance_anti_overreach_governance_firewall_section,
    governance_anti_overreach_governance_firewall_to_json,
    list_governance_anti_overreach_governance_firewall_check_names,
    list_governance_anti_overreach_governance_firewall_contract_names,
    list_governance_anti_overreach_governance_firewall_record_ids,
    list_governance_anti_overreach_governance_firewall_section_names,
)
...
MODULE_PATH = (
    ...
    / "governance_anti_overreach_governance_firewall.py"
)
...
def result() -> dict[str, object]:
    return build_governance_anti_overreach_governance_firewall()


def test_constants_and_public_shape(result):
    assert GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_VERSION == "6.10.0"
    assert (
        GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_SCHEMA_VERSION
        == "6.10.0"
    )
    ...
    assert ANTI_OVERREACH_GOVERNANCE_FIREWALL_STATUS == "firewall_candidate_only"
    assert ANTI_OVERREACH_GOVERNANCE_FIREWALL_ACTIVE_STATUS == "not_active"
    assert POLICY_ENFORCEMENT_STATUS == "not_performed"
    assert OVERREACH_BLOCKING_STATUS == "metadata_only"
    ...
    assert NEXT_STAGE == "v6.11_source_audit_replay_engine"
    assert NEXT_STAGE_TITLE == "Source Audit Replay Engine"
    assert V6_11_HANDOFF_STATUS == "ready_for_source_audit_replay_engine_design"
    assert result["anti_overreach_governance_firewall_status"] == "pass"
    assert result["blocking_reasons"] == []


def test_deterministic_repeated_build_and_hash(result):
    repeated = build_governance_anti_overreach_governance_firewall()
...
def test_upstream_v6_9_handoff_verification(result):
    assert result["upstream_human_sovereignty_lock_status"] == "pass"
    assert len(result["upstream_human_sovereignty_lock_hash"]) == 64
    assert result["upstream_handoff_status"] == (
        "ready_for_anti_overreach_governance_firewall_design"
    )
    assert (
        result["upstream_next_stage"]
        == "v6.10_anti_overreach_governance_firewall"
    )
    assert result["upstream_next_stage_title"] == (
        "Anti-Overreach Governance Firewall"
    )
    assert result["upstream_human_sovereignty_lock_statuses_safe"] is True
    assert (
        result["upstream_human_sovereignty_lock_required_flags_false"] is True
    )
...
def test_all_twelve_records_complete_stable_and_metadata_only(result):
    records = result["anti_overreach_firewall_records"]
    assert (
        REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_RECORD_IDS
        == EXPECTED_RECORD_IDS
    )
    assert [record["firewall_record_id"] for record in records] == list(
        EXPECTED_RECORD_IDS
    )
    assert len(records) == 12
    for record in records:
        assert record["firewall_record_status"] == "registered_metadata_only"
        assert record["introduced_in_version"] == "6.10.0"
        assert record["introduced_in_stage"] == (
            "v6.10_anti_overreach_governance_firewall"
        )
        assert record["introduced_in_layer"] == "layer_15_star_source_memory"
        assert record["inherited_from_stage"] == "v6.9_human_sovereignty_lock"
        assert record["required"] is True
        assert record["metadata_only"] is True
        assert record["overreach_blocking_required"] is True
        assert record["metadata_only_disposition"] == "metadata_only"
        assert record["blocking_reasons"] == []
        assert record["firewall_hash"] == _firewall_hash(record)
        assert record["firewall_record_hash"] == _firewall_record_hash(record)


def test_records_and_getters_are_detached_copy_safe(result):
    record_id = EXPECTED_RECORD_IDS[0]
    first = get_governance_anti_overreach_governance_firewall_record(record_id)
    second = get_governance_anti_overreach_governance_firewall_record(record_id)
    first["firewall_scope"] = "changed"
    first["network_call_performed"] = True
    assert second["firewall_scope"] != "changed"
    assert second["network_call_performed"] is False
    assert result["anti_overreach_firewall_records"][0] != first
    ...
    for getter, name in (
        ...
        (
            get_governance_anti_overreach_governance_firewall_check,
            REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_CHECK_NAMES[0],
        ),
    ):
...
def test_lists_are_stable_and_detached():
    ...
    expected = (
        ...
        list(REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_SECTION_NAMES),
        list(REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_CONTRACT_NAMES),
        list(REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_CHECK_NAMES),
    )
...
def test_all_sections_contracts_and_checks_pass(result):
    for container, name_key, status_key, expected_names in (
        (
            "anti_overreach_firewall_sections",
            "section_name",
            "section_status",
            REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_SECTION_NAMES,
        ),
        (
            "anti_overreach_firewall_contracts",
            "contract_name",
            "contract_status",
            REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_CONTRACT_NAMES,
        ),
        (
            "anti_overreach_firewall_checks",
            "check_name",
            "check_status",
            REQUIRED_GOVERNANCE_ANTI_OVERREACH_GOVERNANCE_FIREWALL_CHECK_NAMES,
        ),
...
def test_all_safety_fields_false(result):
    ...
    for record in result["anti_overreach_firewall_records"]:
...
def test_json_is_deterministic_and_rejects_non_finite(result):
    first = governance_anti_overreach_governance_firewall_to_json(result)
    second = governance_anti_overreach_governance_firewall_to_json(result)
    assert first == second
    ...
    with pytest.raises(TypeError):
        governance_anti_overreach_governance_firewall_to_json({1: "invalid"})


def test_no_sensitive_terms_leak(result):
    payload = governance_anti_overreach_governance_firewall_to_json(
        result
    ).lower()
    sensitive_terms = (
        "api_" + "key",
