...
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal import (
    PROPOSAL_COMPONENTS,
...
from tests.test_governed_star_law_candidate_rule_violation_response_audit_closure_boundary_review_gate import (
    _build as _valid_v2_44_review_gate,
)
...
def _valid_review_gate_report() -> dict[str, object]:
    return deepcopy(_valid_v2_44_review_gate())


def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal(
        report or _valid_review_gate_report()
    )


def test_valid_candidate_rule_violation_response_audit_closure_boundary_review_gate_creates_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_ready():
    result = _build()
    ...
    assert result["source_review_gate_version"] == "2.44.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["proposal_only"] is True
    assert (
        result[
            "candidate_rule_violation_response_audit_finalization_boundary_proposal_only"
        ]
        is True
    )
    assert (
        result[
            "star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_finalization_boundary_proposed_for_human_review_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_finalization_boundary_review_gate_ready_for_human_review_only"
        ]
        is True
    )
    ...
    assert proposal["source_review_gate_version"] == "2.44.0"
    assert (
        proposal["candidate_rule_violation_response_audit_finalization_boundary_proposed"]
        is True
    )
    assert (
        proposal[
    ...
    assert (
        proposal[
            "source_star_law_candidate_rule_violation_response_audit_closure_boundary_review_gate_valid"
        ]
        is True
    )
    assert (
...
def test_proposed_chain_versions_exactly_match_v2_9_0_through_v2_44_0():
    result = _build()

    assert result["proposed_chain_versions"] == PROPOSED_CHAIN_VERSIONS
    assert PROPOSED_CHAIN_VERSIONS == [
        f"v2.{minor}.0" for minor in range(9, 45)
    ]
    assert (
        result[
            "star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal"
...
def test_proposed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_44_0():
    result = _build()
    ...
    assert PROPOSED_STAR_HUB_CHAIN_VERSIONS == [
        f"v2.{minor}.0" for minor in range(19, 45)
    ]


def test_blocked_candidate_rule_violation_response_audit_closure_boundary_review_gate_blocks_proposal():
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_to_json(
        result
    )
...
def test_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()[
...
def test_proposal_does_not_perform_finalization_execution_or_create_finalization_artifacts():
    result = _build()
    proposal = result[
        "star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal"
    ]
...
def test_proposal_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_proposal_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_46_0_star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate():
