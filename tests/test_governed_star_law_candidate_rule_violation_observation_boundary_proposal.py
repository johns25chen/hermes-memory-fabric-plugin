...
from tests.test_governed_star_law_candidate_rule_enforcement_boundary_review_gate import (
    _build as _valid_v2_36_review_gate,
)
...
def _valid_review_gate_report() -> dict[str, object]:
    return deepcopy(_valid_v2_36_review_gate())


def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_law_candidate_rule_violation_observation_boundary_proposal(
        report or _valid_review_gate_report()
    )


def test_valid_candidate_rule_enforcement_boundary_review_gate_creates_star_law_candidate_rule_violation_observation_boundary_proposal_ready():
    result = _build()

    assert result["version"] == "2.37.0"
    assert (
        result["status"]
        == "star_law_candidate_rule_violation_observation_boundary_proposal_ready"
    )
    assert result["source_review_gate_version"] == "2.36.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["proposal_only"] is True
    assert result["candidate_rule_violation_observation_boundary_proposal_only"] is True
    assert (
        result["star_law_candidate_rule_violation_observation_boundary_proposal_only"]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_observation_boundary_proposed_for_human_review_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_observation_boundary_review_gate_ready_for_human_review_only"
        ]
        is True
    )
    assert all(check["passed"] is True for check in result["proposal_checks"])
    assert result["blocking_reasons"] == []
    proposal = result["star_law_candidate_rule_violation_observation_boundary_proposal"]
    assert (
        proposal["proposal_status"]
        == "star_law_candidate_rule_violation_observation_boundary_proposed_for_human_review_only"
    )
    assert proposal["boundary_type"] == (
        "star_law_candidate_rule_violation_observation_boundary_proposal"
    )
    assert proposal["source_review_gate_version"] == "2.36.0"
    assert proposal["candidate_rule_violation_observation_boundary_proposed"] is True
    assert (
        proposal[
            "candidate_rule_violation_observation_boundary_review_gate_ready_for_human_review_only"
        ]
    ...
    assert (
        proposal[
            "source_star_law_candidate_rule_enforcement_boundary_review_gate_valid"
        ]
        is True
    )


def test_proposed_chain_versions_exactly_match_v2_9_0_through_v2_36_0():
    result = _build()
    ...
    assert (
        result["star_law_candidate_rule_violation_observation_boundary_proposal"][
            "proposed_chain_versions"
        ]
        == PROPOSED_CHAIN_VERSIONS
    )
...
def test_proposed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_36_0():
    result = _build()
...
def test_blocked_candidate_rule_enforcement_boundary_review_gate_blocks_proposal():
    ...
    proposal = result["star_law_candidate_rule_violation_observation_boundary_proposal"]
    assert proposal["candidate_rule_violation_observation_boundary_proposed"] is False
    assert (
        proposal[
            "candidate_rule_violation_observation_boundary_review_gate_ready_for_human_review_only"
        ]
...
def test_unsafe_authorization_or_execution_claim_blocks_proposal(field: str):
    report = _valid_review_gate_report()
    report[field] = True
...
def test_violation_observation_claim_blocks_proposal(field: str):
    report = _valid_review_gate_report()
    report[field] = True
...
def test_violation_detection_claim_blocks_proposal():
    report = _valid_review_gate_report()
...
def test_violation_recording_claim_blocks_proposal():
    report = _valid_review_gate_report()
...
def test_violation_enforcement_claim_blocks_proposal():
    report = _valid_review_gate_report()
...
def test_violation_ledger_entry_creation_claim_blocks_proposal():
    report = _valid_review_gate_report()
...
def test_candidate_rule_enforcement_claim_blocks_proposal():
    report = _valid_review_gate_report()
...
def test_candidate_rule_enforcement_authorization_claim_blocks_proposal():
    report = _valid_review_gate_report()
...
def test_star_law_candidate_rule_or_rule_claim_blocks_proposal(field: str):
    report = _valid_review_gate_report()
    report[field] = True
...
def test_malformed_reviewed_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_chain_versions"] = ["v2.9.0", "v2.36.0"]
...
def test_malformed_reviewed_star_hub_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_star_hub_chain_versions"] = ["v2.19.0", "v2.36.0"]
...
def test_missing_required_review_gate_field_blocks_proposal():
    report = _valid_review_gate_report()
    del report["candidate_rule_violation_observation_boundary_proposal_ready_for_human_review_only"]

    assert _build(report)["status"] == "blocked"


def test_proposed_candidate_rule_violation_observation_boundaries_are_proposal_only_and_non_authorizing():
    result = _build()
    boundaries = result["proposed_candidate_rule_violation_observation_boundaries"]
...
def test_star_law_candidate_rule_violation_observation_boundary_proposal_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()["star_law_candidate_rule_violation_observation_boundary_proposal"][
            field
        ]
        is False
    )
...
def test_proposal_does_not_observe_violations():
    result = _build()
    ...
    proposal = result["star_law_candidate_rule_violation_observation_boundary_proposal"]
    assert proposal["candidate_rule_violation_observation_performed"] is False
    assert proposal["candidate_rule_violation_observation_authorized"] is False


def test_proposal_does_not_detect_violations():
    result = _build()
...
def test_proposal_does_not_record_violations():
    result = _build()
...
def test_proposal_does_not_enforce_violations():
    result = _build()
...
def test_proposal_does_not_create_violation_ledger_entries():
    result = _build()
...
def test_proposal_does_not_enforce_or_authorize_candidate_rules():
    result = _build()
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_observation_boundary_proposal_ready_is_candidate_rule_enforcement_authorization"
        ]
        is False
    )


def test_proposal_does_not_activate_or_create_candidate_rules():
    result = _build()
...
def test_proposal_does_not_create_activate_or_enforce_star_law_rules():
    ...
    assert (
        boundary[
            "star_law_candidate_rule_violation_observation_boundary_proposal_ready_is_rule_creation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_violation_observation_boundary_proposal_ready_is_rule_activation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_violation_observation_boundary_proposal_ready_is_rule_enforcement"
        ]
        is False
    )
...
def test_proposal_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_observation_boundary_proposal_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_proposal_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_observation_boundary_proposal_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_38_0_star_law_candidate_rule_violation_observation_boundary_review_gate():
