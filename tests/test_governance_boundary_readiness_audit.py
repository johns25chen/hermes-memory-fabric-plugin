...
def test_public_constants():
    assert GOVERNANCE_BOUNDARY_READINESS_AUDIT_VERSION == "4.10.0"
    assert GOVERNANCE_BOUNDARY_READINESS_AUDIT_SCHEMA_VERSION == "4.10.0"
    assert (
        GOVERNANCE_BOUNDARY_READINESS_AUDIT_TYPE
...
def test_readiness_audit_shape_is_deterministic():
    ...
    assert first["validation_matrix_version"] == "4.10.0"
...
def test_get_readiness_check_by_name_returns_detached_copies():
    ...
    assert second["observed"]["versions"]["kernel_version"] == "4.10.0"  # type: ignore[index]
...
def test_validation_matrix_metadata_is_present():
    ...
    assert audit["validation_matrix_version"] == "4.10.0"
