...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_dry_run_proposal import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_request_review_gate import (
    _build as _valid_v3_5_approval_request_review_gate,
)


_BASE_SOURCE_REPORT = _valid_v3_5_approval_request_review_gate()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_proposal(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_5_review_gate_creates_v3_6_dry_run_proposal_ready():
    result = _build()
    ...
    assert (
        result["source_star_soul_approval_request_review_gate_version"]
        == "3.5.0"
    )
    for field in [
        "read_only",
    ...
    assert all(
        ...
        for check in result["approval_request_dry_run_proposal_checks"]
    )
    ...
    assert (
        proposal[
            "source_star_soul_memory_continuity_boundary_approval_request_review_gate_valid"
        ]
        is True
    )
    assert (
        proposal[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_proposed"
        ]
        is True
    )
    assert (
    ...
    assert (
        proposal[
            "thirteenth_memory_layer_star_soul_boundary_approval_request_dry_run_proposed_for_human_review_only"
        ]
        is True
    )
    assert set(proposal["approval_request_dry_run_proposal_components"]) >= (
        APPROVAL_REQUEST_DRY_RUN_PROPOSAL_COMPONENT_KEYS
    )


def test_proposed_dry_run_chain_versions_exactly_match_required_ranges():
    source_expected = [
    ...
    expected = [*source_expected, "v3.6.0"]
    expected_star_hub = [*source_star_hub_expected, "v3.6.0"]
    result = _build()
    proposal = result[
        "star_soul_memory_continuity_boundary_approval_request_dry_run_proposal"
    ]
    ...
    assert PROPOSED_DRY_RUN_CHAIN_VERSIONS == expected
    assert result["proposed_dry_run_chain_versions"] == expected
    assert proposal["proposed_dry_run_chain_versions"] == expected
    assert (
        SOURCE_REVIEWED_STAR_HUB_CHAIN_VERSIONS
        == source_star_hub_expected
    )
    ...
    assert (
        result["proposed_dry_run_star_hub_chain_versions"]
        == expected_star_hub
    )
    assert (
        proposal["proposed_dry_run_star_hub_chain_versions"]
        == expected_star_hub
    )


def test_blocked_source_review_gate_blocks_dry_run_proposal():
    ...
    assert (
        result["proposed_dry_run_chain_versions"]
        == PROPOSED_DRY_RUN_CHAIN_VERSIONS
    )
    assert (
        result["proposed_dry_run_star_hub_chain_versions"]
...
def test_unsafe_claim_blocks_dry_run_proposal_and_output_stays_false(
    field: str,
):
    report = _valid_source_report()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("approval_request_review_gate_only", False),
        (
        ...
        ("source_star_soul_approval_request_proposal_version", "3.3.0"),
        ("next_allowed_step", "wrong"),
        ("blocking_reasons", ["blocked"]),
    ],
)
def test_missing_or_invalid_required_source_field_blocks_dry_run_proposal(
...
def test_malformed_reviewed_chain_versions_blocks_dry_run_proposal():
    report = _valid_source_report()
    report["reviewed_chain_versions"] = ["v3.4.0"]

    assert _build(report)["status"] == "blocked"


def test_malformed_reviewed_star_hub_chain_versions_blocks_dry_run_proposal():
    report = _valid_source_report()
    report["reviewed_star_hub_chain_versions"] = ["v3.4.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_review_gate_blocks_dry_run_proposal():
    report = _valid_source_report()
    review_gate = report[
...
def test_invalid_reviewed_approval_request_boundaries_block_dry_run_proposal():
    report = _valid_source_report()
    report[
...
def test_invalid_thirteenth_memory_layer_boundary_blocks_dry_run_proposal():
    report = _valid_source_report()
...
def test_proposed_dry_run_boundaries_are_proposal_only_and_non_executing():
    result = _build()
    boundaries = result[
    ...
    assert set(boundaries) >= APPROVAL_REQUEST_DRY_RUN_PROPOSAL_COMPONENT_KEYS
    for boundary in boundaries.values():
        ...
        for field in APPROVAL_REQUEST_DRY_RUN_PROPOSAL_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_request_dry_run_proposal_to_json(
        result
    )
...
@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_DRY_RUN_PROPOSAL_FALSE_FLAGS)
)
def test_dry_run_proposal_preserves_all_top_level_false_flags(field: str):
    assert _build()[field] is False


@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_DRY_RUN_PROPOSAL_OBJECT_FALSE_FLAGS)
)
def test_dry_run_proposal_object_preserves_all_false_flags(field: str):
    proposal = _build()[
        "star_soul_memory_continuity_boundary_approval_request_dry_run_proposal"
    ]
...
def test_dry_run_proposal_crosses_no_unsafe_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert (
        boundary["boundary_approval_request_dry_run_proposal_only"] is True
    )
    assert (
        boundary["approval_request_dry_run_proposed_for_human_review_only"]
        is True
    )
    ...
    assert boundary["dry_run_plan_executed"] is False
...
def test_positive_unsafe_text_claim_blocks_dry_run_proposal(claim: str):
    report = _valid_source_report()
