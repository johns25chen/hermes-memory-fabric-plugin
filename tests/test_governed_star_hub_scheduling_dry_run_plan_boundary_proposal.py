...
from hermes_memory_fabric.governed_star_hub_scheduling_dry_run_plan_boundary_proposal import (
...
def _valid_review_gate_report() -> dict[str, object]:
    return {
        ...
        "next_allowed_step": "v2.22.0 Star-Hub scheduling dry-run plan boundary proposal",
        "blocking_reasons": [],
        "reviewed_star_hub_design_boundary": {
            "review_status": "star_hub_scheduling_design_boundary_reviewed_for_human_review_only",
...
def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_hub_scheduling_dry_run_plan_boundary_proposal(
        report or _valid_review_gate_report()
    )


def test_valid_review_gate_report_creates_star_hub_scheduling_dry_run_plan_boundary_proposal_ready():
    result = _build()

    assert result["version"] == "2.22.0"
    assert result["status"] == "star_hub_scheduling_dry_run_plan_boundary_proposal_ready"
    assert result["source_review_gate_version"] == "2.21.0"
    assert all(check["passed"] is True for check in result["proposal_checks"])
    assert result["blocking_reasons"] == []
    proposed = result["proposed_star_hub_dry_run_plan_boundary"]
    assert proposed["proposal_status"] == (
        "star_hub_scheduling_dry_run_plan_boundary_proposed_for_human_review_only"
    )
    assert proposed["boundary_type"] == "star_hub_scheduling_dry_run_plan_boundary_proposal"
    assert proposed["star_hub_dry_run_plan_boundary_proposed"] is True


def test_proposed_chain_versions_exactly_match_v2_9_0_through_v2_21_0():
    result = _build()

    assert result["proposed_chain_versions"] == EXPECTED_CHAIN
    assert (
        result["proposed_star_hub_dry_run_plan_boundary"]["proposed_chain_versions"]
        == EXPECTED_CHAIN
    )
...
def test_blocked_review_gate_report_blocks_proposal():
    ...
    assert result["proposed_chain_versions"] == EXPECTED_CHAIN
    assert (
        result["proposed_star_hub_dry_run_plan_boundary"][
            "star_hub_dry_run_plan_boundary_proposed"
        ]
        is False
    )
    assert result["blocking_reasons"]
...
def test_malformed_reviewed_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_chain_versions"] = ["v2.9.0", "v2.20.0"]
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    payload = governed_star_hub_scheduling_dry_run_plan_boundary_proposal_to_json(result)
...
def test_proposal_does_not_authorize_dry_run_plan():
    result = _build()

    assert result["dry_run_plan_authorized"] is False
    assert result["proposed_star_hub_dry_run_plan_boundary"]["dry_run_plan_authorized"] is False
...
def test_proposal_does_not_perform_dry_run():
    ...
    assert result["proposed_star_hub_dry_run_plan_boundary"]["dry_run_performed"] is False
...
def test_proposal_does_not_execute_dry_run():
    ...
    assert result["proposed_star_hub_dry_run_plan_boundary"]["dry_run_executed"] is False
...
def test_proposal_does_not_authorize_star_hub_scheduling():
    ...
    assert (
        result["proposed_star_hub_dry_run_plan_boundary"]["star_hub_scheduling_authorized"]
        is False
    )
...
def test_proposal_does_not_perform_scheduling():
    ...
    assert result["proposed_star_hub_dry_run_plan_boundary"]["scheduling_performed"] is False
...
def test_proposal_does_not_authorize_approval():
    ...
    assert result["proposed_star_hub_dry_run_plan_boundary"]["approval_authorized"] is False
...
def test_layer_mapping_is_correct():
    ...
    assert mapping["direction"] == (
        "Star-Hub scheduling design boundary review gate -> Star-Hub scheduling dry-run plan boundary proposal"
    )


def test_next_allowed_step_is_v2_23_star_hub_scheduling_dry_run_plan_boundary_review_gate():
    result = _build()

    assert result["next_allowed_step"] == (
        "v2.23.0 Star-Hub scheduling dry-run plan boundary review gate"
    )
    assert result["proposed_star_hub_dry_run_plan_boundary"]["next_allowed_step"] == (
        "v2.23.0 Star-Hub scheduling dry-run plan boundary review gate"
    )
