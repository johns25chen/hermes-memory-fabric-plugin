...
def test_empty_store_shape_is_deterministic():
    ...
    assert first == second
    assert LOCAL_EVENT_STORE_DRY_RUN_VERSION == "4.6.0"
    assert LOCAL_EVENT_STORE_SCHEMA_VERSION == "4.6.0"
    assert LOCAL_EVENT_STORE_MODE == "in_memory_dry_run_only"
    assert LOCAL_EVENT_STORE_HASH_ALGORITHM == "sha256"
    assert first["version"] == "4.6.0"
...
def test_append_valid_event_succeeds():
    ...
    assert result["appended_event"]["canonicalization_version"] == "4.6.0"
...
def test_invalid_schema_version_is_rejected():
    ...
    assert result["append_status"] == "blocked"
    assert "schema_version must equal 4.6.0" in result["blocking_reasons"]
