...
def test_empty_store_shape_is_deterministic():
    ...
    assert first == second
    assert LOCAL_EVENT_STORE_DRY_RUN_VERSION == "6.1.0"
    assert LOCAL_EVENT_STORE_SCHEMA_VERSION == "6.1.0"
...
def test_append_valid_event_succeeds():
    ...
    assert result["appended_event"]["canonicalization_version"] == "6.1.0"
...
def test_invalid_schema_version_is_rejected():
    ...
    assert result["append_status"] == "blocked"
    assert "schema_version must equal 6.1.0" in result["blocking_reasons"]
