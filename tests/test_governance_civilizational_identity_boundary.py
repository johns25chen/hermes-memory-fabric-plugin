...
def test_constants_match_v6_3_contract():
    assert GOVERNANCE_CIVILIZATIONAL_IDENTITY_BOUNDARY_VERSION == "6.6.0"
    assert GOVERNANCE_CIVILIZATIONAL_IDENTITY_BOUNDARY_SCHEMA_VERSION == "6.6.0"
    assert (
        GOVERNANCE_CIVILIZATIONAL_IDENTITY_BOUNDARY_TYPE
...
def test_boundary_shape_is_deterministic_and_passes(boundary):
    ...
    assert boundary["version"] == "6.6.0"
    assert boundary["schema_version"] == "6.6.0"
...
def test_upstream_v6_2_handoff_verification(boundary):
    assert boundary["upstream_origin_provenance_ledger_version"] == "6.6.0"
