...
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_observation_boundary_proposal import (
    PROPOSED_CANDIDATE_RULE_VIOLATION_OBSERVATION_BOUNDARY_KEYS,
    PROPOSED_CHAIN_VERSIONS,
    PROPOSED_STAR_HUB_CHAIN_VERSIONS,
)
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_observation_boundary_review_gate import (
...
from tests.test_governed_star_law_candidate_rule_violation_observation_boundary_proposal import (
    _build as _valid_v2_37_proposal,
)
...
def _valid_proposal_report() -> dict[str, object]:
    return deepcopy(_valid_v2_37_proposal())


def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_law_candidate_rule_violation_observation_boundary_review_gate(
        report or _valid_proposal_report()
    )


def test_valid_candidate_rule_violation_observation_boundary_proposal_creates_star_law_candidate_rule_violation_observation_boundary_review_gate_ready():
    result = _build()
    ...
    assert result["source_proposal_version"] == "2.37.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["review_gate_only"] is True
    assert result["candidate_rule_violation_observation_boundary_review_gate_only"] is True
    assert (
        result["star_law_candidate_rule_violation_observation_boundary_review_gate_only"]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_observation_boundary_reviewed_for_human_review_only"
        ]
        is True
    )
    assert (
    ...
    assert review_gate["boundary_type"] == (
        "star_law_candidate_rule_violation_observation_boundary_review_gate"
    )
    assert review_gate["source_proposal_version"] == "2.37.0"
    assert review_gate["candidate_rule_violation_observation_boundary_reviewed"] is True
    assert (
        review_gate[
    ...
    assert (
        review_gate[
            "source_star_law_candidate_rule_violation_observation_boundary_proposal_valid"
        ]
        is True
    )


def test_reviewed_chain_versions_exactly_match_v2_9_0_through_v2_37_0():
    result = _build()
    ...
    assert (
        result["star_law_candidate_rule_violation_observation_boundary_review_gate"][
            "reviewed_chain_versions"
        ]
        == REVIEWED_CHAIN_VERSIONS
    )
...
def test_reviewed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_37_0():
    result = _build()
...
def test_blocked_candidate_rule_violation_observation_boundary_proposal_blocks_review_gate():
    ...
    assert review_gate["candidate_rule_violation_observation_boundary_reviewed"] is False
    assert (
        review_gate[
            "candidate_rule_violation_response_boundary_proposal_ready_for_human_review_only"
        ]
...
def test_violation_observation_claim_blocks_review_gate(field: str):
    report = _valid_proposal_report()
    report[field] = True
...
def test_violation_observation_execution_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_observation_executed"] = True

    assert _build(report)["status"] == "blocked"


def test_violation_detection_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_detected"] = True

    assert _build(report)["status"] == "blocked"


def test_violation_recording_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_recorded"] = True

    assert _build(report)["status"] == "blocked"


def test_violation_enforcement_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_enforced"] = True

    assert _build(report)["status"] == "blocked"


def test_violation_ledger_entry_creation_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_ledger_entry_created"] = True

    assert _build(report)["status"] == "blocked"


def test_violation_response_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_response_performed"] = True

    assert _build(report)["status"] == "blocked"


def test_violation_response_authorization_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_violation_response_authorized"] = True

    assert _build(report)["status"] == "blocked"


def test_automated_correction_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["automated_correction_performed"] = True

    assert _build(report)["status"] == "blocked"


def test_automated_correction_authorization_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["automated_correction_authorized"] = True

    assert _build(report)["status"] == "blocked"


def test_candidate_rule_enforcement_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_enforcement_performed"] = True

    assert _build(report)["status"] == "blocked"


def test_candidate_rule_enforcement_authorization_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["candidate_rule_enforcement_authorized"] = True
...
def test_star_law_candidate_rule_or_rule_claim_blocks_review_gate(field: str):
    report = _valid_proposal_report()
    report[field] = True
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_chain_versions"] = ["v2.9.0", "v2.37.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_star_hub_chain_versions"] = ["v2.19.0", "v2.37.0"]
...
def test_missing_required_proposal_field_blocks_review_gate():
    report = _valid_proposal_report()
    del report["candidate_rule_violation_observation_boundary_review_gate_ready_for_human_review_only"]

    assert _build(report)["status"] == "blocked"


def test_reviewed_candidate_rule_violation_observation_boundaries_are_review_only_and_non_authorizing():
    result = _build()
    boundaries = result["reviewed_candidate_rule_violation_observation_boundaries"]

    assert isinstance(boundaries, dict)
    assert set(boundaries) == PROPOSED_CANDIDATE_RULE_VIOLATION_OBSERVATION_BOUNDARY_KEYS
...
def test_star_law_candidate_rule_violation_observation_boundary_review_gate_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()[
...
def test_review_gate_does_not_observe_or_execute_violation_observation():
    result = _build()
    review_gate = result[
        "star_law_candidate_rule_violation_observation_boundary_review_gate"
    ]

    assert result["candidate_rule_violation_observation_performed"] is False
    assert result["candidate_rule_violation_observation_authorized"] is False
    assert result["candidate_rule_violation_observation_executed"] is False
    assert (
        result["candidate_rule_violation_observation_execution_authorized"] is False
    )
    assert review_gate["candidate_rule_violation_observation_performed"] is False
    assert review_gate["candidate_rule_violation_observation_authorized"] is False
    assert review_gate["candidate_rule_violation_observation_executed"] is False
    assert (
        review_gate["candidate_rule_violation_observation_execution_authorized"]
        is False
    )


def test_review_gate_does_not_detect_record_or_enforce_violations():
    result = _build()
    review_gate = result[
        "star_law_candidate_rule_violation_observation_boundary_review_gate"
    ]

    assert result["candidate_rule_violation_detected"] is False
    assert result["candidate_rule_violation_recorded"] is False
    assert result["candidate_rule_violation_enforced"] is False
    assert result["candidate_rule_violation_ledger_entry_created"] is False
    assert review_gate["candidate_rule_violation_detected"] is False
    assert review_gate["candidate_rule_violation_recorded"] is False
    assert review_gate["candidate_rule_violation_enforced"] is False
    assert review_gate["candidate_rule_violation_ledger_entry_created"] is False


def test_review_gate_does_not_perform_or_authorize_violation_response_or_automated_correction():
    result = _build()
    review_gate = result[
        "star_law_candidate_rule_violation_observation_boundary_review_gate"
    ]

    assert result["candidate_rule_violation_response_performed"] is False
    assert result["candidate_rule_violation_response_authorized"] is False
    assert result["automated_correction_performed"] is False
    assert result["automated_correction_authorized"] is False
    assert review_gate["candidate_rule_violation_response_performed"] is False
    assert review_gate["candidate_rule_violation_response_authorized"] is False
    assert review_gate["automated_correction_performed"] is False
    assert review_gate["automated_correction_authorized"] is False
...
def test_review_gate_does_not_create_activate_or_enforce_star_law_rules():
    ...
    assert (
        boundary[
            "star_law_candidate_rule_violation_observation_boundary_review_gate_ready_is_rule_creation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_violation_observation_boundary_review_gate_ready_is_rule_activation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_violation_observation_boundary_review_gate_ready_is_rule_enforcement"
        ]
        is False
    )
...
def test_review_gate_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_observation_boundary_review_gate_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_review_gate_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_observation_boundary_review_gate_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_39_0_star_law_candidate_rule_violation_response_boundary_proposal():
...
def test_source_chain_constants_remain_v2_37_inputs():
    assert PROPOSED_CHAIN_VERSIONS[-1] == "v2.36.0"
    assert PROPOSED_STAR_HUB_CHAIN_VERSIONS[-1] == "v2.36.0"
