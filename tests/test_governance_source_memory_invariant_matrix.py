...
def test_constants_match_v6_4_contract():
    assert GOVERNANCE_SOURCE_MEMORY_INVARIANT_MATRIX_VERSION == "6.9.0"
    assert GOVERNANCE_SOURCE_MEMORY_INVARIANT_MATRIX_SCHEMA_VERSION == "6.9.0"
    assert (
        GOVERNANCE_SOURCE_MEMORY_INVARIANT_MATRIX_TYPE
...
def test_matrix_shape_is_deterministic_and_passes(matrix):
    ...
    assert matrix["version"] == "6.9.0"
    assert matrix["schema_version"] == "6.9.0"
...
def test_upstream_v6_3_handoff_verification(matrix):
    assert matrix["upstream_civilizational_identity_boundary_version"] == "6.9.0"
