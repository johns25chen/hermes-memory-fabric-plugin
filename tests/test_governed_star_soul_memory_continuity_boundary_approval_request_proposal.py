...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_proposal import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_review_gate import (
    _build as _valid_v3_3_approval_review_gate,
)


_BASE_SOURCE_REPORT = _valid_v3_3_approval_review_gate()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_request_proposal(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_3_approval_review_gate_creates_v3_4_approval_request_proposal_ready():
    result = _build()
    ...
    assert result["source_star_soul_approval_review_gate_version"] == "3.3.0"
    for field in [
        "read_only",
        "read_only_memory",
        "proposal_only",
    ...
    assert all(
        ...
        for check in result["approval_request_proposal_checks"]
    )
    ...
    assert (
        proposal["source_star_soul_approval_review_gate_version"]
        == "3.3.0"
    )
    assert (
        proposal[
            "source_star_soul_memory_continuity_boundary_approval_review_gate_valid"
        ]
        is True
    )
    assert (
    ...
    assert (
        proposal[
            "thirteenth_memory_layer_star_soul_boundary_approval_request_proposed_for_human_review_only"
        ]
        is True
    )
    assert set(proposal["approval_request_proposal_components"]) >= (
        APPROVAL_REQUEST_PROPOSAL_COMPONENT_KEYS
    )
...
def test_blocked_source_approval_review_gate_blocks_approval_request_proposal():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("approval_review_gate_only", False),
        (
        ...
        ("source_star_soul_approval_proposal_version", "3.1.0"),
...
def test_malformed_reviewed_chain_versions_blocks_proposal():
    report = _valid_source_report()
    report["reviewed_chain_versions"] = ["v3.2.0"]
...
def test_malformed_reviewed_star_hub_chain_versions_blocks_proposal():
    report = _valid_source_report()
    report["reviewed_star_hub_chain_versions"] = ["v3.2.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_approval_review_gate_blocks_proposal():
    report = _valid_source_report()
    review_gate = report[
    ...
    review_gate[
        "star_soul_memory_continuity_boundary_approval_structurally_review_ready"
    ] = False

    assert _build(report)["status"] == "blocked"


def test_invalid_reviewed_approval_boundaries_block_proposal():
    report = _valid_source_report()
    report["reviewed_star_soul_memory_continuity_approval_boundaries"] = {
...
def test_proposed_approval_request_boundaries_are_proposal_only_non_authorizing():
    result = _build()
    boundaries = result[
    ...
    assert set(boundaries) >= APPROVAL_REQUEST_PROPOSAL_COMPONENT_KEYS
    for boundary in boundaries.values():
        ...
        for field in APPROVAL_REQUEST_PROPOSAL_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_request_proposal_to_json(
        result
    )
...
@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_PROPOSAL_FALSE_FLAGS)
)
def test_approval_request_proposal_preserves_all_top_level_false_flags(
    field: str,
):
...
@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_PROPOSAL_OBJECT_FALSE_FLAGS)
)
def test_approval_request_proposal_object_preserves_all_false_flags(
    field: str,
):
    proposal = _build()[
        "star_soul_memory_continuity_boundary_approval_request_proposal"
...
def test_approval_request_proposal_crosses_no_execution_or_authorization_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert boundary["boundary_approval_request_proposal_only"] is True
    assert boundary["approval_request_proposed_for_human_review_only"] is True
