...
def test_constants_match_v6_7_contract():
    assert GOVERNANCE_SOURCE_MUTATION_PROPOSAL_BOUNDARY_VERSION == "6.7.0"
    assert GOVERNANCE_SOURCE_MUTATION_PROPOSAL_BOUNDARY_SCHEMA_VERSION == "6.7.0"
...
def test_public_shape_is_deterministic_and_passes(boundary):
    ...
    assert boundary["version"] == "6.7.0"
    assert boundary["schema_version"] == "6.7.0"
...
def test_upstream_v6_6_handoff_verification(boundary):
    assert boundary["upstream_multi_cycle_continuity_protocol_version"] == "6.7.0"
...
def test_all_records_registered_metadata_only(boundary):
    for record in boundary["source_mutation_proposal_boundary_records"]:
        ...
        assert record["introduced_in_version"] == "6.7.0"
