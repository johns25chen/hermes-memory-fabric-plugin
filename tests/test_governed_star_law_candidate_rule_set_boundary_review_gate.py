...
from hermes_memory_fabric.governed_star_law_candidate_rule_set_boundary_review_gate import (
...
def _valid_proposal_report() -> dict[str, object]:
    design_proposal = build_governed_star_law_design_boundary_proposal(
        _valid_preflight_report()
    )
    design_review_gate = build_governed_star_law_design_boundary_review_gate(
        design_proposal
    )
...
def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_law_candidate_rule_set_boundary_review_gate(
        report or _valid_proposal_report()
    )


def test_valid_candidate_rule_set_proposal_creates_star_law_candidate_rule_set_boundary_review_gate_ready():
    result = _build()

    assert result["version"] == "2.32.0"
    assert (
        result["status"] == "star_law_candidate_rule_set_boundary_review_gate_ready"
    )
    assert result["source_proposal_version"] == "2.31.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["review_gate_only"] is True
    assert result["candidate_rule_set_boundary_review_gate_only"] is True
    assert result["star_law_candidate_rule_set_boundary_review_gate_only"] is True
    assert result["candidate_rule_set_boundary_reviewed_for_human_review_only"] is True
    assert (
        result[
            "candidate_rule_activation_boundary_proposal_ready_for_human_review_only"
        ]
        is True
    )
    assert all(check["passed"] is True for check in result["review_gate_checks"])
    assert result["blocking_reasons"] == []
    review_gate = result["star_law_candidate_rule_set_boundary_review_gate"]
    assert (
        review_gate["review_status"]
        == "star_law_candidate_rule_set_boundary_reviewed_for_human_review_only"
    )
    assert review_gate["boundary_type"] == "star_law_candidate_rule_set_boundary_review_gate"
    assert review_gate["source_proposal_version"] == "2.31.0"
    assert review_gate["candidate_rule_set_boundary_reviewed"] is True
    assert review_gate["candidate_rule_set_boundary_structurally_review_ready"] is True
    assert (
        review_gate[
            "candidate_rule_activation_boundary_proposal_ready_for_human_review_only"
        ]
    ...
    assert (
        review_gate["source_star_law_candidate_rule_set_boundary_proposal_valid"]
        is True
    )


def test_reviewed_chain_versions_exactly_match_v2_9_0_through_v2_31_0():
    result = _build()
    ...
    assert (
        result["star_law_candidate_rule_set_boundary_review_gate"][
            "reviewed_chain_versions"
        ]
        == REVIEWED_CHAIN_VERSIONS
    )
...
def test_reviewed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_31_0():
    result = _build()
...
def test_blocked_candidate_rule_set_proposal_blocks_review_gate():
    ...
    assert result["candidate_rule_set_boundary_reviewed_for_human_review_only"] is False
    assert (
        result[
            "candidate_rule_activation_boundary_proposal_ready_for_human_review_only"
        ]
    ...
    review_gate = result["star_law_candidate_rule_set_boundary_review_gate"]
    assert review_gate["candidate_rule_set_boundary_reviewed"] is False
    assert (
        review_gate["candidate_rule_set_boundary_structurally_review_ready"] is False
    )
    assert result["reviewed_chain_versions"] == REVIEWED_CHAIN_VERSIONS
    assert (
        result["reviewed_star_hub_chain_versions"]
...
def test_unsafe_source_proposal_nested_claim_blocks_review_gate(field: str):
    report = _valid_proposal_report()
    proposal = report["star_law_candidate_rule_set_boundary_proposal"]
    ...
    assert (
        result["star_law_candidate_rule_set_boundary_review_gate"][
            "source_star_law_candidate_rule_set_boundary_proposal_valid"
        ]
        is False
    )
...
def test_self_executing_policy_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["self_executing_policy_created"] = True
...
def test_memory_write_authorization_claim_blocks_review_gate():
    report = _valid_proposal_report()
    report["durable_memory_write_authorized"] = True
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_chain_versions"] = ["v2.31.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_star_hub_chain_versions"] = ["v2.31.0"]

    assert _build(report)["status"] == "blocked"


def test_reviewed_candidate_rule_set_boundaries_are_review_only_and_non_authorizing():
    result = _build()
    boundaries = result["reviewed_candidate_rule_set_boundaries"]

    assert isinstance(boundaries, dict)
    assert set(boundaries) == SOURCE_PROPOSED_CANDIDATE_RULE_SET_BOUNDARY_KEYS
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    serialized = governed_star_law_candidate_rule_set_boundary_review_gate_to_json(
        _build(report)
    )
...
def test_star_law_candidate_rule_set_boundary_review_gate_preserves_non_authorization_flags(
    field: str,
):
    assert _build()["star_law_candidate_rule_set_boundary_review_gate"][field] is False
...
def test_review_gate_does_not_create_activate_or_enforce_star_law_rules():
    ...
    assert (
        boundary[
            "star_law_candidate_rule_set_boundary_review_gate_ready_is_rule_creation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_set_boundary_review_gate_ready_is_rule_activation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_set_boundary_review_gate_ready_is_rule_enforcement"
        ]
        is False
    )
...
def test_review_gate_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        boundary[
            "star_law_candidate_rule_set_boundary_review_gate_ready_is_autonomous_governance"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_set_boundary_review_gate_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_33_star_law_candidate_rule_activation_boundary_proposal():
    result = _build()

    assert result["next_allowed_step"] == READY_NEXT_ALLOWED_STEP
    assert (
        result["star_law_candidate_rule_set_boundary_review_gate"]["next_allowed_step"]
        == READY_NEXT_ALLOWED_STEP
    )


def test_blocked_result_does_not_copy_source_proposal_wholesale():
    report = _valid_proposal_report()
    ...
    assert result["status"] == "blocked"
    assert "proposed_candidate_rule_set_boundaries" not in result
    assert "reviewed_candidate_rule_set_boundaries" in result
