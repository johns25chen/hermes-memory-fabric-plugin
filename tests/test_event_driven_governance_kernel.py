...
def test_valid_full_event_sequence_reaches_finalized():
    ...
    assert result["transition_policy_version"] == "5.3.0"
    assert result["transition_policy_registry_version"] == "5.3.0"
    assert all(
...
def test_blocked_event_moves_to_blocked_state():
    ...
    assert result["accepted_events"][0]["canonicalization_version"] == "5.3.0"
    assert result["rejected_events"] == []
    assert result["audit_status"] == "blocked"
