...
def test_public_constants():
    assert GOVERNANCE_DRY_RUN_FIXTURE_PACK_VERSION == "4.8.0"
    assert GOVERNANCE_DRY_RUN_FIXTURE_SCHEMA_VERSION == "4.8.0"
    assert (
        GOVERNANCE_DRY_RUN_FIXTURE_PACK_TYPE
...
def test_fixture_pack_shape_is_deterministic():
    ...
    assert first["schema_version"] == "4.8.0"
...
def test_sensitive_redaction_sequence_keeps_raw_inputs_only_in_events():
    ...
    assert "fixture-secret-4-8" in raw_events
