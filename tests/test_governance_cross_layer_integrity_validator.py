...
from hermes_memory_fabric.governance_cross_layer_integrity_validator import (
    AUDIT_LOG_WRITE_STATUS,
    AUDIT_REPLAY_EXECUTION_STATUS,
    COMMON_DISABLED_FLAGS,
    CROSS_LAYER_REPAIR_STATUS,
    CROSS_LAYER_VALIDATION_EXECUTION_STATUS,
    CROSS_LAYER_VALIDATION_STATUS,
    GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_HASH_ALGORITHM,
    GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_SCHEMA_VERSION,
    GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_TYPE,
    GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_VERSION,
    ...
    CROSS_LAYER_INTEGRITY_VALIDATOR_MODE,
    CROSS_LAYER_INTEGRITY_VALIDATOR_STAGE,
    ...
    V6_13_HANDOFF_STATUS,
    _integrity_hash,
    _integrity_record_hash,
    _cross_layer_integrity_validator_hash,
    build_governance_cross_layer_integrity_validator,
    get_governance_cross_layer_integrity_validator_check,
    get_governance_cross_layer_integrity_validator_contract,
    get_governance_cross_layer_integrity_validator_record,
    get_governance_cross_layer_integrity_validator_section,
    governance_cross_layer_integrity_validator_to_json,
    list_governance_cross_layer_integrity_validator_check_names,
    list_governance_cross_layer_integrity_validator_contract_names,
    list_governance_cross_layer_integrity_validator_record_ids,
    list_governance_cross_layer_integrity_validator_section_names,
)
...
MODULE_PATH = (
    ...
    / "governance_cross_layer_integrity_validator.py"
)
SMOKE_PATH = PROJECT_ROOT / "scripts" / "smoke_governance_cross_layer_integrity_validator.py"
...
def result() -> dict[str, object]:
    return build_governance_cross_layer_integrity_validator()


def test_constants_and_public_shape(result):
    assert GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_VERSION == "6.12.0"
    assert GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_SCHEMA_VERSION == "6.12.0"
    assert (
        GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_TYPE
        == "governance_cross_layer_integrity_validator"
    )
    assert GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_HASH_ALGORITHM == "sha256"
    assert CROSS_LAYER_INTEGRITY_VALIDATOR_STAGE == "v6.12_cross_layer_integrity_validator"
    assert CROSS_LAYER_INTEGRITY_VALIDATOR_MODE == "cross_layer_integrity_validator_only"
    assert CROSS_LAYER_INTEGRITY_VALIDATOR_STATUS == "validator_candidate_only"
    assert CROSS_LAYER_INTEGRITY_VALIDATOR_ACTIVE_STATUS == "not_active"
    assert CROSS_LAYER_VALIDATION_STATUS == "metadata_only"
    ...
    assert NEXT_STAGE == "v6.13_civilization_core_stability_index"
    assert NEXT_STAGE_TITLE == "Civilization Core Stability Index"
    assert V6_13_HANDOFF_STATUS == "ready_for_civilization_core_stability_index_design"
    assert result["cross_layer_integrity_validator_status"] == "pass"
    assert result["blocking_reasons"] == []


def test_deterministic_repeated_build_and_hash(result):
    repeated = build_governance_cross_layer_integrity_validator()
...
def test_upstream_v6_11_handoff_verification(result):
    assert result["upstream_source_audit_replay_engine_status"] == "pass"
    assert len(result["upstream_source_audit_replay_engine_hash"]) == 64
    assert result["upstream_handoff_status"] == (
        "ready_for_cross_layer_integrity_validator_design"
    )
    assert result["upstream_next_stage"] == "v6.12_cross_layer_integrity_validator"
    assert result["upstream_next_stage_title"] == "Cross-Layer Integrity Validator"
    assert result["upstream_source_audit_replay_engine_statuses_safe"] is True
    assert (
        result[
...
def test_all_twelve_records_complete_stable_and_metadata_only(result):
    records = result["cross_layer_integrity_records"]
    assert REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_RECORD_IDS == (
        EXPECTED_RECORD_IDS
    )
    assert [record["integrity_record_id"] for record in records] == list(
        EXPECTED_RECORD_IDS
    )
    assert len(records) == 12
    for record in records:
        assert record["integrity_record_status"] == "registered_metadata_only"
        assert record["introduced_in_version"] == "6.12.0"
        assert record["introduced_in_stage"] == "v6.12_cross_layer_integrity_validator"
        assert record["introduced_in_layer"] == "layer_15_star_source_memory"
        assert record["inherited_from_stage"] == (
            "v6.11_source_audit_replay_engine"
        )
        assert record["required"] is True
        assert record["metadata_only"] is True
        assert record["cross_layer_integrity_metadata_required"] is True
        assert record["metadata_only_disposition"] == "metadata_only"
        assert record["blocking_reasons"] == []
        assert record["integrity_hash"] == _integrity_hash(record)
        assert record["integrity_record_hash"] == _integrity_record_hash(record)


def test_records_and_getters_are_detached_copy_safe(result):
    record_id = EXPECTED_RECORD_IDS[0]
    first = get_governance_cross_layer_integrity_validator_record(record_id)
    second = get_governance_cross_layer_integrity_validator_record(record_id)
    first["integrity_scope"] = "changed"
    first["network_call_performed"] = True
    assert second["integrity_scope"] != "changed"
    assert second["network_call_performed"] is False
    assert result["cross_layer_integrity_records"][0] != first

    for getter, name in (
        (
            get_governance_cross_layer_integrity_validator_section,
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_SECTION_NAMES[0],
        ),
        (
            get_governance_cross_layer_integrity_validator_contract,
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_CONTRACT_NAMES[0],
        ),
        (
            get_governance_cross_layer_integrity_validator_check,
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_CHECK_NAMES[0],
        ),
    ):
...
def test_lists_are_stable_and_detached():
    ...
    expected = (
        ...
        list(REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_SECTION_NAMES),
        list(REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_CONTRACT_NAMES),
        list(REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_CHECK_NAMES),
    )
    assert observed == expected
    observed[0].append("changed")
    assert list_governance_cross_layer_integrity_validator_record_ids() == expected[0]


def test_all_sections_contracts_and_checks_pass(result):
    for container, name_key, status_key, expected_names in (
        (
            "cross_layer_integrity_sections",
            "section_name",
            "section_status",
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_SECTION_NAMES,
        ),
        (
            "cross_layer_integrity_contracts",
            "contract_name",
            "contract_status",
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_CONTRACT_NAMES,
        ),
        (
            "cross_layer_integrity_checks",
            "check_name",
            "check_status",
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_CHECK_NAMES,
        ),
...
def test_all_safety_fields_false(result):
    ...
    for record in result["cross_layer_integrity_records"]:
...
def test_no_integrity_write_approval_authorization_dispatch_or_mutation(result):
    expected_statuses = {
        ...
        "handoff_status": "ready_for_civilization_core_stability_index_design",
...
def test_json_is_deterministic_and_rejects_non_finite(result):
    first = governance_cross_layer_integrity_validator_to_json(result)
    second = governance_cross_layer_integrity_validator_to_json(result)
    assert first == second
    ...
    with pytest.raises(ValueError):
        governance_cross_layer_integrity_validator_to_json({"value": math.nan})
    with pytest.raises(TypeError):
        governance_cross_layer_integrity_validator_to_json({1: "invalid"})


def test_no_sensitive_terms_leak(result):
    payload = governance_cross_layer_integrity_validator_to_json(result).lower()
    sensitive_terms = (
        "api_" + "key",
        "pass" + "word",
        "creden" + "tial",
