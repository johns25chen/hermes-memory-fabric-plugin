...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_review_gate import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposal import (
    _build as _valid_v3_8_execution_proposal,
)


_BASE_SOURCE_REPORT = _valid_v3_8_execution_proposal()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_review_gate(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_8_execution_proposal_creates_v3_9_review_gate_ready():
    result = _build()
    ...
    assert all(
        ...
        for check in result[
            "approval_request_dry_run_execution_review_gate_checks"
        ]
    )
    ...
    assert (
        review_gate[
            "source_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposal_valid"
        ]
        is True
    )
    assert (
    ...
    assert (
        review_gate[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_execution_structurally_review_ready"
        ]
        is True
    )
    assert set(
        review_gate["reviewed_dry_run_execution_boundary_components"]
    ) >= APPROVAL_REQUEST_DRY_RUN_EXECUTION_REVIEW_GATE_COMPONENT_KEYS


def test_all_source_and_reviewed_lineages_are_exact():
    reviewed_expected = [
    ...
    assert (
        result["reviewed_dry_run_execution_chain_versions"]
        == execution_reviewed_expected
    )
    assert (
        review_gate["reviewed_dry_run_execution_chain_versions"]
        == execution_reviewed_expected
    )
    assert (
        REVIEWED_DRY_RUN_EXECUTION_STAR_HUB_CHAIN_VERSIONS
    ...
    assert (
        result["reviewed_dry_run_execution_star_hub_chain_versions"]
        == execution_reviewed_star_hub_expected
    )
    assert (
        review_gate["reviewed_dry_run_execution_star_hub_chain_versions"]
        == execution_reviewed_star_hub_expected
    )


def test_blocked_source_execution_proposal_blocks_review_gate():
...
def test_unsafe_claim_blocks_review_gate_and_output_stays_false(
    field: str,
):
    report = _valid_source_report()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("proposal_only", False),
        ("dry_run_execution_proposal_only", False),
        ("approval_request_dry_run_execution_proposal_only", False),
        (
...
def test_malformed_proposed_dry_run_execution_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_dry_run_execution_chain_versions"] = ["v3.8.0"]

    assert _build(report)["status"] == "blocked"


def test_malformed_proposed_dry_run_execution_star_hub_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_dry_run_execution_star_hub_chain_versions"] = ["v3.8.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_execution_proposal_blocks_review_gate():
    report = _valid_source_report()
    proposal = report[
...
def test_invalid_proposed_execution_boundaries_block_review_gate():
    report = _valid_source_report()
    report[
...
def test_reviewed_execution_boundaries_are_review_only_and_non_executing():
    result = _build()
    boundaries = result[
    ...
    for boundary in boundaries.values():
        ...
        assert boundary["dry_run_execution_review_gate_only"] is True
        assert (
            boundary["approval_request_dry_run_execution_review_gate_only"]
            is True
        )
        ...
        for field in APPROVAL_REQUEST_DRY_RUN_EXECUTION_REVIEW_GATE_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_review_gate_to_json(
        result
    )
...
@pytest.mark.parametrize(
    "field",
    sorted(APPROVAL_REQUEST_DRY_RUN_EXECUTION_REVIEW_GATE_FALSE_FLAGS),
)
...
@pytest.mark.parametrize(
    "field",
    sorted(APPROVAL_REQUEST_DRY_RUN_EXECUTION_REVIEW_GATE_OBJECT_FALSE_FLAGS),
)
...
def test_review_gate_crosses_no_unsafe_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert (
        ...
        == "v3.9.0 reviews approval request dry-run execution proposal boundaries for human review only and does not execute dry-run, execute dry-run plan, create, submit, execute, grant, record approval, or record real human decisions"
    )
    ...
    assert (
        boundary[
            "boundary_approval_request_dry_run_execution_review_gate_only"
        ]
        is True
    )
    assert (
        boundary[
            "approval_request_dry_run_execution_reviewed_for_human_review_only"
        ]
        is True
    )
    ...
    assert (
        READY_NEXT_ALLOWED_STEP
        == "v3.10.0 Star-Soul Memory continuity boundary approval request dry-run completion attestation proposal"
    )
