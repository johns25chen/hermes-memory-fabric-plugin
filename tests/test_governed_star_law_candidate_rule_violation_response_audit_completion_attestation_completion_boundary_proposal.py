...
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_response_audit_completion_attestation_completion_boundary_proposal import (
    PROPOSAL_COMPONENTS,
...
from tests.test_governed_star_law_candidate_rule_violation_response_audit_completion_attestation_finalization_boundary_review_gate import (
    _build as _valid_v2_54_review_gate,
)


def _valid_review_gate_report() -> dict[str, object]:
    return deepcopy(_valid_v2_54_review_gate())


def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_law_candidate_rule_violation_response_audit_completion_attestation_completion_boundary_proposal(
        report or _valid_review_gate_report()
    )


def test_valid_attestation_finalization_boundary_review_gate_creates_completion_boundary_proposal_ready():
    result = _build()
    ...
    assert result["source_review_gate_version"] == "2.54.0"
    for field in [
        "read_only",
        "read_only_memory",
        "proposal_only",
        ...
        "candidate_rule_violation_response_audit_completion_attestation_completion_boundary_proposed_for_human_review_only",
        "candidate_rule_violation_response_audit_completion_attestation_completion_boundary_review_gate_ready_for_human_review_only",
    ]:
    ...
    assert (
        proposal[
            "source_star_law_candidate_rule_violation_response_audit_completion_attestation_finalization_boundary_review_gate_valid"
        ]
        is True
    )
    assert (
        set(
            proposal[
                "star_law_candidate_rule_violation_response_audit_completion_attestation_completion_boundary_components"
            ]
        )
        == PROPOSAL_COMPONENTS
    )
...
def test_proposed_chain_versions_exactly_match_v2_9_0_through_v2_54_0():
    expected = [f"v2.{minor}.0" for minor in range(9, 55)]

    assert PROPOSED_CHAIN_VERSIONS == expected
    assert _build()["proposed_chain_versions"] == expected


def test_proposed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_54_0():
    expected = [f"v2.{minor}.0" for minor in range(19, 55)]

    assert PROPOSED_STAR_HUB_CHAIN_VERSIONS == expected
    assert _build()["proposed_star_hub_chain_versions"] == expected


def test_blocked_attestation_finalization_boundary_review_gate_blocks_proposal():
    ...
    assert (
        proposal[
            "candidate_rule_violation_response_audit_completion_attestation_completion_boundary_proposed"
        ]
        is False
    )
...
def test_malformed_reviewed_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_chain_versions"] = ["v2.9.0", "v2.51.0"]
...
def test_malformed_reviewed_star_hub_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_star_hub_chain_versions"] = ["v2.19.0", "v2.51.0"]
...
def test_malformed_reviewed_boundaries_block_proposal():
    ...
    report[
        ...
        "unsafe": {
            "review_only": True,
            "candidate_rule_violation_response_audit_completion_attestation_finalization_authorized": True,
        }
    }
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_violation_response_audit_completion_attestation_completion_boundary_proposal_to_json(
        result
    )
