...
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_response_audit_boundary_review_gate import (
...
from tests.test_governed_star_law_candidate_rule_violation_response_audit_boundary_proposal import (
    _build as _valid_v2_41_proposal,
)


TOP_LEVEL_FALSE_FLAGS = list(REVIEW_GATE_FALSE_FLAGS)
REVIEW_BOUNDARY_FALSE_FLAGS = list(REVIEW_OBJECT_FALSE_FLAGS)
...
def _valid_proposal_report() -> dict[str, object]:
    return deepcopy(_valid_v2_41_proposal())
...
def test_valid_candidate_rule_violation_response_audit_boundary_proposal_creates_star_law_candidate_rule_violation_response_audit_boundary_review_gate_ready():
    result = _build()
    ...
    assert result["source_proposal_version"] == "2.41.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["review_gate_only"] is True
    assert (
        result["candidate_rule_violation_response_audit_boundary_review_gate_only"]
        is True
    )
    assert (
        result[
            "star_law_candidate_rule_violation_response_audit_boundary_review_gate_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_boundary_reviewed_for_human_review_only"
        ]
        is True
    )
    assert (
    ...
    assert review_gate["source_proposal_version"] == "2.41.0"
    assert review_gate["candidate_rule_violation_response_audit_boundary_reviewed"] is True
    assert (
        review_gate[
    ...
    assert (
        review_gate[
            "source_star_law_candidate_rule_violation_response_audit_boundary_proposal_valid"
        ]
        is True
    )
    assert set(
...
def test_reviewed_chain_versions_exactly_match_v2_9_0_through_v2_41_0():
    result = _build()
...
def test_reviewed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_41_0():
    result = _build()
...
def test_blocked_candidate_rule_violation_response_audit_boundary_proposal_blocks_review_gate():
    ...
    assert review_gate["candidate_rule_violation_response_audit_boundary_reviewed"] is False
    assert (
        review_gate[
            "candidate_rule_violation_response_audit_closure_boundary_proposal_ready_for_human_review_only"
        ]
...
def test_unsafe_authorization_execution_write_audit_or_boundary_claim_blocks_review_gate(
    field: str,
):
    report = _valid_proposal_report()
    report[field] = True
...
def test_nested_unsafe_claim_blocks_review_gate(field: str):
    report = _valid_proposal_report()
    proposal = report[
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_chain_versions"] = ["v2.9.0", "v2.41.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_star_hub_chain_versions"] = ["v2.19.0", "v2.41.0"]
...
def test_reviewed_audit_boundaries_are_review_only_and_non_authorizing():
    result = _build()
    boundaries = result["reviewed_candidate_rule_violation_response_audit_boundaries"]

    assert isinstance(boundaries, dict)
    assert set(boundaries) == SOURCE_PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_BOUNDARY_KEYS
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_violation_response_audit_boundary_review_gate_to_json(
        result
    )
...
@pytest.mark.parametrize("field", REVIEW_BOUNDARY_FALSE_FLAGS)
def test_star_law_candidate_rule_violation_response_audit_boundary_review_gate_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()[
...
def test_review_gate_does_not_perform_audit_execution_or_create_audit_artifacts():
    result = _build()
    review_gate = result[
        "star_law_candidate_rule_violation_response_audit_boundary_review_gate"
    ]
...
def test_review_gate_does_not_create_activate_or_enforce_star_law_rules():
    ...
    assert (
        boundary[
            "star_law_candidate_rule_violation_response_audit_boundary_review_gate_ready_is_rule_creation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_violation_response_audit_boundary_review_gate_ready_is_rule_activation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_violation_response_audit_boundary_review_gate_ready_is_rule_enforcement"
        ]
        is False
    )
...
def test_review_gate_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_response_audit_boundary_review_gate_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_review_gate_does_not_authorize_handoff_durable_memory_or_openclaw_execution():
    ...
    assert result["durable_memory_write_authorized"] is False
...
def test_review_gate_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_response_audit_boundary_review_gate_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_layer_mapping_is_correct():
    ...
    assert mapping["direction"] == (
        "Star-Law candidate rule violation response audit boundary proposal -> Star-Law candidate rule violation response audit boundary review gate"
    )


def test_next_allowed_step_is_v2_43_0_star_law_candidate_rule_violation_response_audit_closure_boundary_proposal():
