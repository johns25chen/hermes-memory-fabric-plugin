...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_dry_run_review_gate import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_proposal import (
    _build as _valid_v3_6_dry_run_proposal,
)


_BASE_SOURCE_REPORT = _valid_v3_6_dry_run_proposal()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_review_gate(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_6_dry_run_proposal_creates_v3_7_review_gate_ready():
    result = _build()
    ...
    assert all(
        ...
        for check in result["approval_request_dry_run_review_gate_checks"]
    )
    ...
    assert (
        gate[
            "source_star_soul_memory_continuity_boundary_approval_request_dry_run_proposal_valid"
        ]
        is True
    )
    assert (
    ...
    assert (
        gate[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_structurally_review_ready"
        ]
        is True
    )
    assert (
    ...
    assert (
        gate[
            "thirteenth_memory_layer_star_soul_boundary_approval_request_dry_run_reviewed_for_human_review_only"
        ]
        is True
    )
    assert set(gate["approval_request_dry_run_review_gate_components"]) >= (
        APPROVAL_REQUEST_DRY_RUN_REVIEW_GATE_COMPONENT_KEYS
    )


def test_reviewed_dry_run_chain_versions_exactly_match_required_ranges():
    source_expected = [
    ...
    assert SOURCE_PROPOSED_DRY_RUN_CHAIN_VERSIONS == expected
    assert result["source_proposed_dry_run_chain_versions"] == expected
    assert REVIEWED_DRY_RUN_CHAIN_VERSIONS == expected
    assert result["reviewed_dry_run_chain_versions"] == expected
    assert gate["reviewed_dry_run_chain_versions"] == expected
    assert SOURCE_REVIEWED_STAR_HUB_CHAIN_VERSIONS == source_star_hub_expected
    assert (
        result["source_reviewed_star_hub_chain_versions"]
        == source_star_hub_expected
    )
    assert SOURCE_PROPOSED_DRY_RUN_STAR_HUB_CHAIN_VERSIONS == expected_star_hub
    assert (
        result["source_proposed_dry_run_star_hub_chain_versions"]
        == expected_star_hub
    )
    assert REVIEWED_DRY_RUN_STAR_HUB_CHAIN_VERSIONS == expected_star_hub
    assert (
        result["reviewed_dry_run_star_hub_chain_versions"]
        == expected_star_hub
    )
    assert (
        gate["reviewed_dry_run_star_hub_chain_versions"]
        == expected_star_hub
    )


def test_blocked_source_dry_run_proposal_blocks_review_gate():
    ...
    assert (
        result["reviewed_dry_run_chain_versions"]
        == REVIEWED_DRY_RUN_CHAIN_VERSIONS
    )
    assert (
        result["reviewed_dry_run_star_hub_chain_versions"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("proposal_only", False),
        ("dry_run_proposal_only", False),
        ("approval_request_dry_run_proposal_only", False),
        (
        ...
        ("source_star_soul_approval_request_review_gate_version", "3.4.0"),
...
def test_malformed_proposed_dry_run_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_dry_run_chain_versions"] = ["v3.6.0"]

    assert _build(report)["status"] == "blocked"


def test_malformed_proposed_dry_run_star_hub_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_dry_run_star_hub_chain_versions"] = ["v3.6.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_dry_run_proposal_blocks_review_gate():
    report = _valid_source_report()
    proposal = report[
...
def test_invalid_proposed_dry_run_boundaries_block_review_gate():
    report = _valid_source_report()
    report[
...
def test_reviewed_dry_run_boundaries_are_review_only_and_non_executing():
    result = _build()
    boundaries = result[
    ...
    assert set(boundaries) >= APPROVAL_REQUEST_DRY_RUN_REVIEW_GATE_COMPONENT_KEYS
    for boundary in boundaries.values():
        assert boundary["review_only"] is True
        assert boundary["dry_run_review_gate_only"] is True
        assert boundary["approval_request_dry_run_review_gate_only"] is True
        ...
        for field in APPROVAL_REQUEST_DRY_RUN_REVIEW_GATE_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_request_dry_run_review_gate_to_json(
        result
    )
...
@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_DRY_RUN_REVIEW_GATE_FALSE_FLAGS)
)
...
@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_DRY_RUN_REVIEW_GATE_OBJECT_FALSE_FLAGS)
)
...
def test_dry_run_review_gate_crosses_no_unsafe_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert (
        ...
        == "v3.7.0 reviews approval request dry-run proposal boundaries for human review only and does not execute dry-run, execute dry-run plan, create, submit, execute, grant, record approval, or record real human decisions"
    )
    boundary = result["thirteenth_memory_layer_boundary"]
    assert boundary["memory_layer"] == "星魂记忆"
    assert boundary["memory_layer_number"] == 13
    assert boundary["boundary_approval_request_dry_run_review_gate_only"] is True
    assert (
        boundary["approval_request_dry_run_reviewed_for_human_review_only"]
        is True
    )
