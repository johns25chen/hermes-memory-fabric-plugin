...
from hermes_memory_fabric.governed_star_law_candidate_rule_enforcement_boundary_review_gate import (
...
from tests.test_governed_star_law_candidate_rule_enforcement_boundary_proposal import (
    _build as _valid_v2_35_proposal,
)
...
def _valid_proposal_report() -> dict[str, object]:
    return deepcopy(_valid_v2_35_proposal())
...
def test_valid_candidate_rule_enforcement_boundary_proposal_creates_star_law_candidate_rule_enforcement_boundary_review_gate_ready():
    result = _build()

    assert result["version"] == "2.36.0"
    assert (
        result["status"]
        == "star_law_candidate_rule_enforcement_boundary_review_gate_ready"
    )
    assert result["source_proposal_version"] == "2.35.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["review_gate_only"] is True
    assert result["candidate_rule_enforcement_boundary_review_gate_only"] is True
    assert (
        result[
            "star_law_candidate_rule_enforcement_boundary_review_gate_only"
        ]
        is True
    )
    assert (
        result["candidate_rule_enforcement_boundary_reviewed_for_human_review_only"]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_observation_boundary_proposal_ready_for_human_review_only"
        ]
        is True
    )
    assert all(check["passed"] is True for check in result["review_gate_checks"])
    assert result["blocking_reasons"] == []
    review_gate = result["star_law_candidate_rule_enforcement_boundary_review_gate"]
    assert (
    ...
    assert review_gate["source_proposal_version"] == "2.35.0"
    assert review_gate["candidate_rule_enforcement_boundary_reviewed"] is True
    assert (
        review_gate[
            "candidate_rule_enforcement_boundary_structurally_review_ready"
        ]
        is True
    )
    assert (
    ...
    assert (
        review_gate[
            "source_star_law_candidate_rule_enforcement_boundary_proposal_valid"
        ]
        is True
    )


def test_reviewed_chain_versions_exactly_match_v2_9_0_through_v2_35_0():
    result = _build()
    ...
    assert (
        result["star_law_candidate_rule_enforcement_boundary_review_gate"][
            "reviewed_chain_versions"
        ]
        == REVIEWED_CHAIN_VERSIONS
    )
...
def test_reviewed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_35_0():
    result = _build()
...
def test_blocked_candidate_rule_enforcement_boundary_proposal_blocks_review_gate():
    ...
    review_gate = result["star_law_candidate_rule_enforcement_boundary_review_gate"]
    assert review_gate["candidate_rule_enforcement_boundary_reviewed"] is False
    assert (
        review_gate[
            "candidate_rule_violation_observation_boundary_proposal_ready_for_human_review_only"
        ]
...
def test_star_law_candidate_rule_enforcement_claim_blocks_review_gate():
    report = _valid_proposal_report()
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_chain_versions"] = ["v2.9.0", "v2.35.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_star_hub_chain_versions"] = ["v2.19.0", "v2.35.0"]
...
def test_missing_required_proposal_field_blocks_review_gate():
    report = _valid_proposal_report()
    del report["candidate_rule_enforcement_boundary_review_gate_ready_for_human_review_only"]

    assert _build(report)["status"] == "blocked"


def test_reviewed_candidate_rule_enforcement_boundaries_are_review_only_and_non_authorizing():
    result = _build()
    boundaries = result["reviewed_candidate_rule_enforcement_boundaries"]
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_enforcement_boundary_review_gate_to_json(
        result
    )
...
def test_star_law_candidate_rule_enforcement_boundary_review_gate_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()["star_law_candidate_rule_enforcement_boundary_review_gate"][field]
        is False
    )


def test_review_gate_does_not_enforce_candidate_rules():
    result = _build()

    assert result["candidate_rule_enforcement_performed"] is False
    assert result["candidate_rule_enforcement_authorized"] is False
    review_gate = result["star_law_candidate_rule_enforcement_boundary_review_gate"]
    assert review_gate["candidate_rule_enforcement_performed"] is False
    assert review_gate["candidate_rule_enforcement_authorized"] is False


def test_review_gate_does_not_authorize_candidate_rule_enforcement():
    result = _build()

    assert result["candidate_rule_enforcement_authorized"] is False
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_enforcement_boundary_review_gate_ready_is_candidate_rule_enforcement_authorization"
        ]
        is False
    )
...
def test_review_gate_does_not_create_star_law_rules():
    result = _build()

    assert result["star_law_rules_created"] is False


def test_review_gate_does_not_activate_star_law_rules():
    result = _build()

    assert result["star_law_rules_activated"] is False


def test_review_gate_does_not_enforce_star_law_rules():
    result = _build()
...
def test_review_gate_does_not_create_autonomous_governance():
    result = _build()

    assert result["autonomous_governance_created"] is False


def test_review_gate_does_not_authorize_autonomous_execution():
    result = _build()
...
def test_review_gate_does_not_authorize_durable_memory_write():
    result = _build()
...
def test_review_gate_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_enforcement_boundary_review_gate_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_37_0_star_law_candidate_rule_violation_observation_boundary_proposal():
