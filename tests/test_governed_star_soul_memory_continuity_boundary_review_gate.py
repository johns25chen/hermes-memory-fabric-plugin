...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_review_gate import (
    LAYER_MAPPING,
...
from tests.test_governed_star_soul_memory_continuity_boundary_proposal import (
    _build as _valid_v3_0_proposal,
)


_BASE_SOURCE_REPORT = _valid_v3_0_proposal()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_review_gate(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_0_star_soul_proposal_creates_v3_1_review_gate_ready():
    result = _build()
    ...
    assert result["source_star_soul_proposal_version"] == "3.0.0"
    for field in [
        "read_only",
        "read_only_memory",
        "review_gate_only",
    ...
    review = result["star_soul_memory_continuity_boundary_review_gate"]
    assert (
        review["review_status"]
        == "star_soul_memory_continuity_boundary_reviewed_for_human_review_only"
    )
    assert (
        review["boundary_type"]
        == "star_soul_memory_continuity_boundary_review_gate"
    )
    assert review["source_star_soul_proposal_version"] == "3.0.0"
    assert review["star_soul_memory_continuity_boundary_reviewed"] is True
    assert (
        review[
    ...
    assert (
        review[
            "source_star_soul_memory_continuity_boundary_proposal_valid"
        ]
        is True
    )
    assert (
        review[
            "thirteenth_memory_layer_star_soul_boundary_reviewed_for_human_review_only"
        ]
        is True
    )
...
def test_reviewed_chain_versions_exactly_match_required_ranges():
    ...
    assert (
        result["star_soul_memory_continuity_boundary_review_gate"][
            "reviewed_chain_versions"
        ]
        == expected
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("proposal_only", False),
        ("star_soul_memory_continuity_boundary_proposal_only", False),
        (
        ...
        ("source_star_law_review_gate_version", "2.59.0"),
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_chain_versions"] = ["v3.0.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_star_hub_chain_versions"] = ["v3.0.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_proposal_blocks_review_gate():
    report = _valid_source_report()
    proposal = report["star_soul_memory_continuity_boundary_proposal"]
...
def test_invalid_proposed_boundaries_block_review_gate():
    report = _valid_source_report()
    report["proposed_star_soul_memory_continuity_boundaries"] = {
...
def test_reviewed_boundaries_are_review_only_and_non_authorizing():
    result = _build()
    boundaries = result[
...
def test_review_object_preserves_all_false_flags(field: str):
    review = _build()["star_soul_memory_continuity_boundary_review_gate"]

    assert review[field] is False


def test_review_gate_does_not_cross_any_execution_or_authorization_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert boundary["boundary_review_gate_only"] is True
    assert boundary["reviewed_for_human_review_only"] is True
