...
from hermes_memory_fabric.governed_star_law_candidate_rule_enforcement_boundary_proposal import (
...
from tests.test_governed_star_law_candidate_rule_activation_boundary_review_gate import (
    _build as _valid_v2_34_review_gate,
)
...
def _valid_review_gate_report() -> dict[str, object]:
    report = deepcopy(_valid_v2_34_review_gate())
    review_gate = report["star_law_candidate_rule_activation_boundary_review_gate"]
    assert isinstance(review_gate, dict)
    review_gate.setdefault("star_law_candidate_rules_enforced", False)
    return report


def _build(report: dict[str, object] | None = None) -> dict[str, object]:
    return build_governed_star_law_candidate_rule_enforcement_boundary_proposal(
        report or _valid_review_gate_report()
    )


def test_valid_candidate_rule_activation_boundary_review_gate_creates_star_law_candidate_rule_enforcement_boundary_proposal_ready():
    result = _build()
    ...
    assert result["source_review_gate_version"] == "2.34.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["proposal_only"] is True
    assert result["candidate_rule_enforcement_boundary_proposal_only"] is True
    assert (
        result["star_law_candidate_rule_enforcement_boundary_proposal_only"] is True
    )
    assert (
        result["candidate_rule_enforcement_boundary_proposed_for_human_review_only"]
        is True
    )
    assert (
        result[
            "candidate_rule_enforcement_boundary_review_gate_ready_for_human_review_only"
        ]
        is True
    )
    assert all(check["passed"] is True for check in result["proposal_checks"])
    assert result["blocking_reasons"] == []
    proposal = result["star_law_candidate_rule_enforcement_boundary_proposal"]
    assert (
    ...
    assert proposal["source_review_gate_version"] == "2.34.0"
    assert proposal["candidate_rule_enforcement_boundary_proposed"] is True
    assert (
        proposal[
            "candidate_rule_enforcement_boundary_review_gate_ready_for_human_review_only"
        ]
    ...
    assert (
        proposal[
            "source_star_law_candidate_rule_activation_boundary_review_gate_valid"
        ]
        is True
    )


def test_proposed_chain_versions_exactly_match_v2_9_0_through_v2_34_0():
    result = _build()
    ...
    assert (
        result["star_law_candidate_rule_enforcement_boundary_proposal"][
            "proposed_chain_versions"
        ]
        == PROPOSED_CHAIN_VERSIONS
    )
...
def test_proposed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_34_0():
    result = _build()
...
def test_blocked_candidate_rule_activation_boundary_review_gate_blocks_proposal():
    ...
    proposal = result["star_law_candidate_rule_enforcement_boundary_proposal"]
    assert proposal["candidate_rule_enforcement_boundary_proposed"] is False
    assert (
        proposal[
            "candidate_rule_enforcement_boundary_review_gate_ready_for_human_review_only"
        ]
...
def test_star_law_candidate_rule_enforcement_claim_blocks_proposal():
    report = _valid_review_gate_report()
    report["star_law_candidate_rules_enforced"] = True

    assert _build(report)["status"] == "blocked"


def test_star_law_rule_creation_activation_and_enforcement_claims_block_proposal():
    for field in [
        "star_law_rules_created",
        "star_law_rules_activated",
        "star_law_rules_enforced",
...
def test_scheduling_authorization_and_performance_claims_block_proposal():
    for field in ["scheduling_authorized", "scheduling_performed"]:
        report = _valid_review_gate_report()
        report[field] = True
...
def test_dry_run_performed_executed_or_authorized_claims_block_proposal():
    for field in [
        "dry_run_performed",
        "dry_run_executed",
        "dry_run_execution_authorized",
...
def test_memory_write_graph_ledger_and_openclaw_authorization_claims_block_proposal():
    for field in [
        "memory_write_authorized",
        "memory_graph_mutation_authorized",
        "operation_ledger_creation_authorized",
...
def test_malformed_reviewed_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_chain_versions"] = ["v2.9.0", "v2.34.0"]
...
def test_malformed_reviewed_star_hub_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_star_hub_chain_versions"] = ["v2.19.0", "v2.34.0"]
...
def test_missing_required_review_gate_field_blocks_proposal():
    report = _valid_review_gate_report()
    del report["candidate_rule_enforcement_boundary_proposal_ready_for_human_review_only"]

    assert _build(report)["status"] == "blocked"


def test_proposed_candidate_rule_enforcement_boundaries_are_proposal_only_and_non_authorizing():
    result = _build()
    boundaries = result["proposed_candidate_rule_enforcement_boundaries"]

    assert isinstance(boundaries, dict)
    assert set(boundaries) == PROPOSED_CANDIDATE_RULE_ENFORCEMENT_BOUNDARY_KEYS
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_enforcement_boundary_proposal_to_json(
        result
    )
...
def test_star_law_candidate_rule_enforcement_boundary_proposal_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()["star_law_candidate_rule_enforcement_boundary_proposal"][field]
        is False
    )


def test_proposal_does_not_enforce_candidate_rules():
    result = _build()

    assert result["candidate_rule_enforcement_performed"] is False
    assert result["candidate_rule_enforcement_authorized"] is False
    proposal = result["star_law_candidate_rule_enforcement_boundary_proposal"]
    assert proposal["candidate_rule_enforcement_performed"] is False
    assert proposal["candidate_rule_enforcement_authorized"] is False


def test_proposal_does_not_authorize_candidate_rule_enforcement():
    result = _build()

    assert result["candidate_rule_enforcement_authorized"] is False
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_enforcement_boundary_proposal_ready_is_candidate_rule_enforcement"
        ]
        is False
    )
...
def test_proposal_does_not_create_activate_or_enforce_star_law_rules():
    ...
    assert (
        boundary[
            "star_law_candidate_rule_enforcement_boundary_proposal_ready_is_rule_creation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_enforcement_boundary_proposal_ready_is_rule_activation"
        ]
        is False
    )
    assert (
        boundary[
            "star_law_candidate_rule_enforcement_boundary_proposal_ready_is_rule_enforcement"
        ]
        is False
    )
...
def test_proposal_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_enforcement_boundary_proposal_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_proposal_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_enforcement_boundary_proposal_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_36_0_star_law_candidate_rule_enforcement_boundary_review_gate():
