...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposal import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_review_gate import (
    _build as _valid_v3_7_dry_run_review_gate,
)


_BASE_SOURCE_REPORT = _valid_v3_7_dry_run_review_gate()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposal(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_7_dry_run_review_gate_creates_v3_8_execution_proposal_ready():
    result = _build()
    ...
    assert all(
        ...
        for check in result[
            "approval_request_dry_run_execution_proposal_checks"
        ]
    )
    ...
    assert (
        proposal[
            "source_star_soul_memory_continuity_boundary_approval_request_dry_run_review_gate_valid"
        ]
        is True
    )
    assert (
        proposal[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposed"
        ]
        is True
    )
    assert (
        proposal[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_execution_structurally_proposed_for_human_review_only"
        ]
        is True
    )
    assert (
    ...
    assert set(
        proposal["approval_request_dry_run_execution_proposal_components"]
    ) >= APPROVAL_REQUEST_DRY_RUN_EXECUTION_PROPOSAL_COMPONENT_KEYS
...
def test_all_source_and_proposed_lineages_are_exact():
    ...
    assert (
        result["source_proposed_dry_run_chain_versions"]
        == dry_run_expected
    )
    assert (
        result["source_reviewed_dry_run_chain_versions"]
        == dry_run_expected
    )
    assert (
        SOURCE_PROPOSED_DRY_RUN_STAR_HUB_CHAIN_VERSIONS
    ...
    assert (
        PROPOSED_DRY_RUN_EXECUTION_CHAIN_VERSIONS == execution_expected
    )
    assert (
        result["proposed_dry_run_execution_chain_versions"]
        == execution_expected
    )
    assert (
        proposal["proposed_dry_run_execution_chain_versions"]
        == execution_expected
    )
    assert (
        PROPOSED_DRY_RUN_EXECUTION_STAR_HUB_CHAIN_VERSIONS
    ...
    assert (
        result["proposed_dry_run_execution_star_hub_chain_versions"]
        == execution_star_hub_expected
    )
    assert (
        proposal[
...
def test_blocked_source_dry_run_review_gate_blocks_execution_proposal():
...
def test_unsafe_claim_blocks_execution_proposal_and_output_stays_false(
    field: str,
):
    report = _valid_source_report()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("dry_run_review_gate_only", False),
        ("approval_request_dry_run_review_gate_only", False),
        (
...
def test_missing_or_invalid_required_source_field_blocks_execution_proposal(
...
def test_malformed_reviewed_dry_run_chain_versions_blocks_execution_proposal():
    report = _valid_source_report()
    report["reviewed_dry_run_chain_versions"] = ["v3.6.0"]

    assert _build(report)["status"] == "blocked"


def test_malformed_reviewed_dry_run_star_hub_chain_versions_blocks_execution_proposal():
    report = _valid_source_report()
    report["reviewed_dry_run_star_hub_chain_versions"] = ["v3.6.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_dry_run_review_gate_blocks_execution_proposal():
    report = _valid_source_report()
    review_gate = report[
    ...
    review_gate[
        "star_soul_memory_continuity_boundary_approval_request_dry_run_reviewed"
    ] = False

    assert _build(report)["status"] == "blocked"


def test_invalid_reviewed_dry_run_boundaries_block_execution_proposal():
    report = _valid_source_report()
    report[
...
def test_invalid_thirteenth_memory_layer_boundary_blocks_execution_proposal():
    report = _valid_source_report()
    boundary = report["thirteenth_memory_layer_boundary"]
    assert isinstance(boundary, dict)
    boundary["dry_run_executed"] = True

    assert _build(report)["status"] == "blocked"


def test_proposed_dry_run_execution_boundaries_are_proposal_only_and_non_executing():
    result = _build()
    boundaries = result[
    ...
    for boundary in boundaries.values():
        ...
        assert boundary["dry_run_execution_proposal_only"] is True
        assert (
            boundary["approval_request_dry_run_execution_proposal_only"]
            is True
        )
        ...
        for field in APPROVAL_REQUEST_DRY_RUN_EXECUTION_PROPOSAL_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposal_to_json(
        result
    )
...
@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REQUEST_DRY_RUN_EXECUTION_PROPOSAL_FALSE_FLAGS)
)
def test_execution_proposal_preserves_all_top_level_false_flags(
    field: str,
):
...
@pytest.mark.parametrize(
    "field",
    sorted(APPROVAL_REQUEST_DRY_RUN_EXECUTION_PROPOSAL_OBJECT_FALSE_FLAGS),
)
def test_execution_proposal_object_preserves_all_false_flags(field: str):
    proposal = _build()[
        "star_soul_memory_continuity_boundary_approval_request_dry_run_execution_proposal"
    ]
...
def test_dry_run_execution_proposal_crosses_no_unsafe_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert (
        ...
        == "v3.8.0 proposes approval request dry-run execution boundaries for human review only and does not execute dry-run, execute dry-run plan, create, submit, execute, grant, record approval, or record real human decisions"
    )
    ...
    assert (
        boundary[
            "boundary_approval_request_dry_run_execution_proposal_only"
        ]
        is True
    )
    assert (
        boundary[
            "approval_request_dry_run_execution_proposed_for_human_review_only"
        ]
        is True
    )
    ...
    assert (
        READY_NEXT_ALLOWED_STEP
        == "v3.9.0 Star-Soul Memory continuity boundary approval request dry-run execution review gate"
    )
...
def test_positive_unsafe_text_claim_blocks_execution_proposal(claim: str):
    report = _valid_source_report()
