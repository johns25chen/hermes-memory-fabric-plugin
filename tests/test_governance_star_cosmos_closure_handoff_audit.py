...
def test_constants_match_v5_13_contract():
    assert GOVERNANCE_STAR_COSMOS_CLOSURE_HANDOFF_AUDIT_VERSION == "6.5.0"
    assert (
        GOVERNANCE_STAR_COSMOS_CLOSURE_HANDOFF_AUDIT_SCHEMA_VERSION
        == "6.5.0"
    )
...
def test_audit_shape_is_deterministic_and_passes(audit):
    ...
    assert audit["version"] == "6.5.0"
    assert audit["schema_version"] == "6.5.0"
    ...
    assert audit["post_sandbox_review_boundary_version"] == "6.5.0"
