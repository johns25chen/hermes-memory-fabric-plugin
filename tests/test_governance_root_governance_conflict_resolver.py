...
def test_constants_match_v6_5_contract():
    assert GOVERNANCE_ROOT_GOVERNANCE_CONFLICT_RESOLVER_VERSION == "6.9.0"
    assert GOVERNANCE_ROOT_GOVERNANCE_CONFLICT_RESOLVER_SCHEMA_VERSION == "6.9.0"
    assert (
        GOVERNANCE_ROOT_GOVERNANCE_CONFLICT_RESOLVER_TYPE
...
def test_resolver_shape_is_deterministic_and_passes(resolver):
    ...
    assert resolver["version"] == "6.9.0"
    assert resolver["schema_version"] == "6.9.0"
...
def test_upstream_v6_4_handoff_verification(resolver):
    assert resolver["upstream_source_memory_invariant_matrix_version"] == "6.9.0"
