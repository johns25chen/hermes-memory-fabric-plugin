...
def test_constants_match_v6_2_contract():
    assert GOVERNANCE_ORIGIN_PROVENANCE_LEDGER_VERSION == "6.2.0"
    assert GOVERNANCE_ORIGIN_PROVENANCE_LEDGER_SCHEMA_VERSION == "6.2.0"
    assert (
        GOVERNANCE_ORIGIN_PROVENANCE_LEDGER_TYPE
...
def test_ledger_shape_is_deterministic_and_passes(ledger):
    ...
    assert ledger["version"] == "6.2.0"
    assert ledger["schema_version"] == "6.2.0"
...
def test_upstream_v6_1_handoff_verification(ledger):
    assert ledger["upstream_source_constitution_registry_version"] == "6.2.0"
...
def test_all_records_are_registered_metadata_only(ledger):
    for record in ledger["origin_provenance_records"]:
        ...
        assert record["recorded_in_version"] == "6.2.0"
