...
def test_constants_match_v6_contract():
    assert GOVERNANCE_STAR_SOURCE_MEMORY_ENTRY_CANDIDATE_VERSION == "6.3.0"
    assert GOVERNANCE_STAR_SOURCE_MEMORY_ENTRY_CANDIDATE_SCHEMA_VERSION == "6.3.0"
    assert (
        GOVERNANCE_STAR_SOURCE_MEMORY_ENTRY_CANDIDATE_TYPE
...
def test_candidate_shape_is_deterministic_and_passes(candidate):
    ...
    assert candidate["version"] == "6.3.0"
    assert candidate["schema_version"] == "6.3.0"
    ...
    assert candidate["star_cosmos_closure_handoff_audit_version"] == "6.3.0"
