...
def test_constants_match_v6_1_contract():
    assert GOVERNANCE_SOURCE_CONSTITUTION_REGISTRY_VERSION == "6.7.0"
    assert GOVERNANCE_SOURCE_CONSTITUTION_REGISTRY_SCHEMA_VERSION == "6.7.0"
    assert (
        GOVERNANCE_SOURCE_CONSTITUTION_REGISTRY_TYPE
...
def test_registry_shape_is_deterministic_and_passes(registry):
    ...
    assert registry["version"] == "6.7.0"
    assert registry["schema_version"] == "6.7.0"
...
def test_upstream_v6_0_handoff_verification(registry):
    assert registry["upstream_star_source_entry_candidate_version"] == "6.7.0"
