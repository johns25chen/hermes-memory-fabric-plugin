...
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_response_audit_boundary_proposal import (
...
from tests.test_governed_star_law_candidate_rule_violation_response_boundary_review_gate import (
    _build as _valid_v2_40_review_gate,
)
...
def _valid_review_gate_report() -> dict[str, object]:
    return deepcopy(_valid_v2_40_review_gate())
...
def test_valid_candidate_rule_violation_response_boundary_review_gate_creates_star_law_candidate_rule_violation_response_audit_boundary_proposal_ready():
    result = _build()
    ...
    assert result["source_review_gate_version"] == "2.40.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["proposal_only"] is True
    assert (
        result["candidate_rule_violation_response_audit_boundary_proposal_only"]
        is True
    )
    assert (
        result["star_law_candidate_rule_violation_response_audit_boundary_proposal_only"]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_boundary_proposed_for_human_review_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_boundary_review_gate_ready_for_human_review_only"
        ]
        is True
    )
    ...
    assert proposal["source_review_gate_version"] == "2.40.0"
    assert proposal["candidate_rule_violation_response_audit_boundary_proposed"] is True
    assert (
        proposal[
            "candidate_rule_violation_response_audit_boundary_review_gate_ready_for_human_review_only"
        ]
    ...
    assert (
        proposal[
            "source_star_law_candidate_rule_violation_response_boundary_review_gate_valid"
        ]
        is True
    )


def test_proposed_chain_versions_exactly_match_v2_9_0_through_v2_40_0():
    result = _build()
    ...
    assert (
        result["star_law_candidate_rule_violation_response_audit_boundary_proposal"][
            "proposed_chain_versions"
        ]
        == PROPOSED_CHAIN_VERSIONS
    )
...
def test_proposed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_40_0():
    result = _build()
...
def test_blocked_candidate_rule_violation_response_boundary_review_gate_blocks_proposal():
    ...
    assert proposal["candidate_rule_violation_response_audit_boundary_proposed"] is False
    assert result["proposed_chain_versions"] == PROPOSED_CHAIN_VERSIONS
    assert (
        result["proposed_star_hub_chain_versions"]
        == PROPOSED_STAR_HUB_CHAIN_VERSIONS
...
def test_unsafe_authorization_execution_write_audit_or_boundary_claim_blocks_proposal(field: str):
    report = _valid_review_gate_report()
    report[field] = True
...
def test_nested_audit_execution_claim_blocks_proposal():
    report = _valid_review_gate_report()
    review_gate = report[
        "star_law_candidate_rule_violation_response_boundary_review_gate"
    ]
    assert isinstance(review_gate, dict)
    review_gate["candidate_rule_violation_response_audit_executed"] = True

    result = _build(report)

    assert result["status"] == "blocked"
    assert result["candidate_rule_violation_response_audit_executed"] is False


def test_malformed_reviewed_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_chain_versions"] = ["v2.9.0", "v2.40.0"]
...
def test_malformed_reviewed_star_hub_chain_versions_blocks_proposal():
    report = _valid_review_gate_report()
    report["reviewed_star_hub_chain_versions"] = ["v2.19.0", "v2.40.0"]
...
def test_proposed_audit_boundaries_are_proposal_only_and_non_authorizing():
    result = _build()
    boundaries = result["proposed_candidate_rule_violation_response_audit_boundaries"]

    assert isinstance(boundaries, dict)
    assert set(boundaries) == PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_BOUNDARY_KEYS
...
def test_sensitive_field_names_and_values_are_not_leaked_in_serialized_output():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_violation_response_audit_boundary_proposal_to_json(
        result
    )
...
def test_star_law_candidate_rule_violation_response_audit_boundary_proposal_preserves_non_authorization_flags(
    field: str,
):
    assert (
        _build()[
...
def test_proposal_does_not_create_autonomous_governance_or_authorize_execution():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_response_audit_boundary_proposal_ready_is_autonomous_execution"
        ]
        is False
    )
...
def test_proposal_does_not_claim_civilization_core_completion():
    ...
    assert (
        result["non_authorization_boundary"][
            "star_law_candidate_rule_violation_response_audit_boundary_proposal_ready_is_civilization_core_completion"
        ]
        is False
    )
...
def test_next_allowed_step_is_v2_42_0_star_law_candidate_rule_violation_response_audit_boundary_review_gate():
