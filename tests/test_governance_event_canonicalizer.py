...
def test_public_versions_and_hash_algorithm():
    assert CANONICALIZER_VERSION == "4.4.0"
    assert CANONICAL_EVENT_SCHEMA_VERSION == "4.4.0"
...
def test_every_canonical_event_has_v430_canonicalization_version():
    result = canonicalize_governance_event_sequence(_full_events())

    assert all(
        event["canonicalization_version"] == "4.4.0"
