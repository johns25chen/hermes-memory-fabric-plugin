...
from hermes_memory_fabric.governance_source_handoff_boundary import (
    AUDIT_LOG_WRITE_STATUS,
    AUDIT_REPLAY_EXECUTION_STATUS,
    COMMON_DISABLED_FLAGS,
    CROSS_LAYER_REPAIR_STATUS,
    CROSS_LAYER_VALIDATION_EXECUTION_STATUS,
    GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_HASH_ALGORITHM,
    GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_SCHEMA_VERSION,
    GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_TYPE,
    GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_VERSION,
    ...
    V6_15_HANDOFF_STATUS,
    _handoff_hash,
    _handoff_record_hash,
    _source_handoff_boundary_hash,
    build_governance_source_handoff_boundary,
    get_governance_source_handoff_boundary_check,
    get_governance_source_handoff_boundary_contract,
    get_governance_source_handoff_boundary_record,
    get_governance_source_handoff_boundary_section,
    governance_source_handoff_boundary_to_json,
    list_governance_source_handoff_boundary_check_names,
    list_governance_source_handoff_boundary_contract_names,
    list_governance_source_handoff_boundary_record_ids,
    list_governance_source_handoff_boundary_section_names,
)
...
MODULE_PATH = (
    ...
    / "governance_source_handoff_boundary.py"
)
SMOKE_PATH = PROJECT_ROOT / "scripts" / "smoke_governance_source_handoff_boundary.py"
...
def result() -> dict[str, object]:
    return build_governance_source_handoff_boundary()


def test_constants_and_public_shape(result):
    assert GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_VERSION == "6.14.0"
    assert GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_SCHEMA_VERSION == "6.14.0"
    assert (
        GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_TYPE
        == "governance_source_handoff_boundary"
    )
    assert GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_HASH_ALGORITHM == "sha256"
    assert SOURCE_HANDOFF_BOUNDARY_STAGE == "v6.14_source_handoff_boundary"
    assert SOURCE_HANDOFF_BOUNDARY_MODE == "source_handoff_boundary_only"
    assert SOURCE_HANDOFF_BOUNDARY_STATUS == "source_handoff_boundary_candidate_only"
    assert SOURCE_HANDOFF_BOUNDARY_ACTIVE_STATUS == "not_active"
    assert SOURCE_HANDOFF_STATUS == "metadata_only"
    ...
    assert NEXT_STAGE == "v6.15_star_source_closure_audit"
    assert NEXT_STAGE_TITLE == "Star-Source Closure Audit"
    assert V6_15_HANDOFF_STATUS == "ready_for_star_source_closure_audit_design"
    assert result["source_handoff_boundary_status"] == "pass"
    assert result["blocking_reasons"] == []


def test_deterministic_repeated_build_and_hash(result):
    repeated = build_governance_source_handoff_boundary()
...
def test_upstream_v6_13_handoff_verification(result):
    assert result["upstream_civilization_core_stability_index_status"] == "pass"
    assert len(result["upstream_civilization_core_stability_index_hash"]) == 64
    assert result["upstream_handoff_status"] == (
        "ready_for_source_handoff_boundary_design"
    )
    assert result["upstream_next_stage"] == "v6.14_source_handoff_boundary"
    assert result["upstream_next_stage_title"] == "Source Handoff Boundary"
    assert result["upstream_civilization_core_stability_index_statuses_safe"] is True
    assert (
        result[
...
def test_all_twelve_records_complete_stable_and_metadata_only(result):
    records = result["source_handoff_boundary_records"]
    assert REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_RECORD_IDS == (
        EXPECTED_RECORD_IDS
    )
    assert [record["handoff_record_id"] for record in records] == list(
        EXPECTED_RECORD_IDS
    )
    assert len(records) == 12
    for record in records:
        assert record["handoff_record_status"] == "registered_metadata_only"
        assert record["introduced_in_version"] == "6.14.0"
        assert record["introduced_in_stage"] == "v6.14_source_handoff_boundary"
        ...
        assert record["source_handoff_boundary_metadata_required"] is True
        assert record["metadata_only_disposition"] == "metadata_only"
        assert record["blocking_reasons"] == []
        assert record["handoff_hash"] == _handoff_hash(record)
        assert record["handoff_record_hash"] == _handoff_record_hash(record)


def test_records_and_getters_are_detached_copy_safe(result):
    record_id = EXPECTED_RECORD_IDS[0]
    first = get_governance_source_handoff_boundary_record(record_id)
    second = get_governance_source_handoff_boundary_record(record_id)
    first["handoff_scope"] = "changed"
    first["network_call_performed"] = True
    assert second["handoff_scope"] != "changed"
    assert second["network_call_performed"] is False
    assert result["source_handoff_boundary_records"][0] != first

    for getter, name in (
        (
            get_governance_source_handoff_boundary_section,
            REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_SECTION_NAMES[0],
        ),
        (
            get_governance_source_handoff_boundary_contract,
            REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_CONTRACT_NAMES[0],
        ),
        (
            get_governance_source_handoff_boundary_check,
            REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_CHECK_NAMES[0],
        ),
    ):
...
def test_lists_are_stable_and_detached():
    ...
    expected = (
        ...
        list(REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_SECTION_NAMES),
        list(REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_CONTRACT_NAMES),
        list(REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_CHECK_NAMES),
    )
    assert observed == expected
    observed[0].append("changed")
    assert list_governance_source_handoff_boundary_record_ids() == expected[0]


def test_all_sections_contracts_and_checks_pass(result):
    for container, name_key, status_key, expected_names in (
        (
            "source_handoff_boundary_sections",
            "section_name",
            "section_status",
            REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_SECTION_NAMES,
        ),
        (
            "source_handoff_boundary_contracts",
            "contract_name",
            "contract_status",
            REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_CONTRACT_NAMES,
        ),
        (
            "source_handoff_boundary_checks",
            "check_name",
            "check_status",
            REQUIRED_GOVERNANCE_SOURCE_HANDOFF_BOUNDARY_CHECK_NAMES,
        ),
...
def test_all_safety_fields_false(result):
    ...
    for record in result["source_handoff_boundary_records"]:
...
def test_json_is_deterministic_and_rejects_non_finite(result):
    first = governance_source_handoff_boundary_to_json(result)
    second = governance_source_handoff_boundary_to_json(result)
    assert first == second
    ...
    with pytest.raises(ValueError):
        governance_source_handoff_boundary_to_json({"value": math.nan})
    with pytest.raises(TypeError):
        governance_source_handoff_boundary_to_json({1: "invalid"})


def test_no_sensitive_terms_leak(result):
    payload = governance_source_handoff_boundary_to_json(result).lower()
    sensitive_terms = (
        "api_" + "key",
        "pass" + "word",
        "creden" + "tial",
