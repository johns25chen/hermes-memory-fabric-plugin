...
from hermes_memory_fabric.p4_m3_governed_transition_intake_declared_transition_risk_envelope_contract import (
    ...
    list_governed_transition_intake_declared_transition_risk_envelope_contract_fields,
...
DATACLASS_FIELDS = {
    ...
    "field_name",
    "field_purpose",
    "p4_m3_declared_transition_risk_envelope_contract_category",
    "p4_m3_declared_transition_risk_envelope_semantics_disabled",
}
...
PREVIOUS_P4_M3_8_READ_ONLY_COMMANDS = EXPECTED_MEMORY_LOOP_COMMANDS - {
    "governed-transition-intake-declared-transition-risk-envelope-contract"
}
...
PROHIBITED_MEMORY_LOOP_COMMANDS = {
    ...
    "enforce-risk-policy",
    "select-risk-surface",
    "persist-transition-risk",
    "store-transition-risk",
    "mutate-transition-risk",
    "create-transition-risk-record",
    "update-transition-risk-record",
    "delete-transition-risk-record",
...
def test_declared_transition_risk_envelope_field_order_count_and_ids_are_stable():
    fields = (
        list_governed_transition_intake_declared_transition_risk_envelope_contract_fields()
    )
...
def test_every_declared_transition_risk_envelope_field_has_required_values():
    for field in (
        list_governed_transition_intake_declared_transition_risk_envelope_contract_fields()
    ):
        assert field.field_name.strip()
        assert field.field_purpose.strip()
        assert field.p4_m3_declared_transition_risk_envelope_contract_category.strip()
        assert field.p4_m3_declared_transition_risk_envelope_semantics_disabled.strip()
...
def test_json_output_is_stable_and_contains_required_boundaries(tmp_path):
    ...
    assert first_payload["status"]["phase"] == "P4-M3.9"
    assert (
...
def test_dict_conversion_and_status_report_are_deterministic():
    ...
    assert first_status["phase"] == "P4-M3.9"
    assert (
    ...
    assert (
        first_status[
            "governed_transition_intake_declared_transition_risk_envelope_contract_field_count"
        ]
        == 18
    )
    assert (
...
def test_operator_markdown_format_is_explicit_and_stable(tmp_path):
    ...
    assert second_stderr == ""
    assert first_stdout == second_stdout
    assert first_stdout.startswith("# P4-M3.9")
    assert not (tmp_path / ".local").exists()
...
def test_command_does_not_instantiate_writable_store(monkeypatch, tmp_path):
    ...
    assert markdown_stdout.startswith("# P4-M3.9")
    assert json_code == 0
    assert json_stderr == ""
...
def test_read_only_allowlist_includes_new_command_and_preserves_previous_commands():
    ...
    assert PREVIOUS_P4_M3_8_READ_ONLY_COMMANDS.issubset(commands)
...
def test_doc_contains_required_boundaries():
    doc = Path(
        "docs/CIVILIZATION_CORE_P4_M3_9_GOVERNED_TRANSITION_INTAKE_DECLARED_TRANSITION_RISK_ENVELOPE_CONTRACT.md"
    ).read_text()
...
def test_package_version_lock_and_no_entry_point():
    ...
    assert (
        "p4_m3_governed_transition_intake_declared_transition_risk_envelope_contract"
        not in entry_points
    )
    assert (
        "governed-transition-intake-declared-transition-risk-envelope-contract"
...
def test_custom_markdown_render_accepts_read_only_fields():
    field = GovernedTransitionIntakeDeclaredTransitionRiskEnvelopeContractField(
        field_order=1,
        field_id=(
            "custom-governed-transition-intake-declared-transition-risk-"
            "envelope-contract"
        ),
        field_name=(
            "Custom Governed Transition Intake Declared Transition Risk "
        ...
        p4_m3_declared_transition_risk_envelope_semantics_disabled=(
            "Custom governed transition declared transition risk envelope "
    ...
    assert (
        "custom-governed-transition-intake-declared-transition-risk-envelope-contract"
        in markdown
    )
