...
from hermes_memory_fabric.p4_m6_8_entry_ambiguity_non_inference_surface import (
    ...
    list_p4_m6_8_entry_ambiguity_non_inference_surface_fields,
    p4_m6_8_entry_ambiguity_non_inference_surface_field_ids,
    p4_m6_8_entry_ambiguity_non_inference_surface_report,
    p4_m6_8_entry_ambiguity_non_inference_surface_status_shape,
    render_p4_m6_8_entry_ambiguity_non_inference_surface_markdown,
    run_p4_m6_8_entry_ambiguity_non_inference_surface_command,
)
...
def test_command_exists_as_static_memory_loop_declaration() -> None:
    assert MEMORY_LOOP_COMMAND == "p4-m6-8-entry-ambiguity-non-inference-surface"
    assert (
        MEMORY_LOOP_COMMAND_INVOCATION
        == "memory-loop p4-m6-8-entry-ambiguity-non-inference-surface"
    )
    assert (
        P4_M6_8_ENTRY_AMBIGUITY_NON_INFERENCE_SURFACE_BOUNDARY["command"]
        == MEMORY_LOOP_COMMAND_INVOCATION
    )


def test_status_shape_includes_23_required_semantic_field_ids() -> None:
    fields = list_p4_m6_8_entry_ambiguity_non_inference_surface_fields()
    assert len(fields) == 23
    assert p4_m6_8_entry_ambiguity_non_inference_surface_field_ids() == FIELD_IDS
    assert tuple(p4_m6_8_entry_ambiguity_non_inference_surface_status_shape()) == FIELD_IDS
    assert all(isinstance(field, P4M68EntryAmbiguityNonInferenceSurfaceField) for field in fields)
    assert set(dataclasses.asdict(fields[0])) == {
        "field_order",
        "field_id",
        "field_name",
        "field_purpose",
        "p4_m6_8_entry_ambiguity_non_inference_surface_category",
        "p4_m6_8_entry_ambiguity_non_inference_surface_semantics_disabled",
    }


def test_direct_prior_reference_points_to_p4_m6_7() -> None:
    ...
    assert (
        report["status_shape"]["p4_m6_8_direct_prior_reference"]
        == "P4-M6.7 Entry Conflict Non-Resolution Surface"
    )
...
def test_required_boundary_phrases_are_present() -> None:
    ...
    rendered = render_p4_m6_8_entry_ambiguity_non_inference_surface_markdown()
...
def test_true_flags_count_is_148() -> None:
    report = p4_m6_8_entry_ambiguity_non_inference_surface_report()
    assert len(TRUE_STATUS_FLAGS) == 148
    assert report["true_flags"] == 148


def test_false_flags_count_is_273() -> None:
    report = p4_m6_8_entry_ambiguity_non_inference_surface_report()
    assert len(FALSE_STATUS_FLAGS) == 273
    assert report["false_flags"] == 273
...
def test_command_renderer_is_read_only() -> None:
    markdown = run_p4_m6_8_entry_ambiguity_non_inference_surface_command()
    parsed = json.loads(
        run_p4_m6_8_entry_ambiguity_non_inference_surface_command(output_format="json")
    )
    assert "Entry Ambiguity Non-Inference Surface" in markdown
    ...
    assert parsed["true_flags"] == 148
    assert parsed["false_flags"] == 273
...
def test_command_renderer_does_not_create_subspace_memory_in_fresh_temp_workspace(
    ...
    workspace.mkdir()
    output = run_p4_m6_8_entry_ambiguity_non_inference_surface_command(
...
def test_operator_markdown_command_is_read_only_and_pre_store(
    ...
    assert payload == {}
    assert stderr == ""
    assert stdout.startswith("# P4-M6.8 Entry Ambiguity Non-Inference Surface\n")
    for phrase in (
        "Entry Ambiguity Non-Inference Surface",
        "static entry ambiguity label surface",
        "ambiguity-non-inference-surface-only",
...
def test_operator_json_command_returns_required_report(tmp_path: Path) -> None:
    ...
    assert output["true_flags"] == 148
    assert output["false_flags"] == 273
    assert (
    ...
    assert status["stage"] == "P4-M6.8"
    assert status["surface"] == "Entry Ambiguity Non-Inference Surface"
    ...
    assert status["true_flags"] == 148
    assert status["false_flags"] == 273
    actual_true_keys = {
...
def test_parser_exposes_p4_m6_8_memory_loop_command() -> None:
...
def test_no_routing_execution_storage_or_mutation_behavior_exists() -> None:
    booleans = p4_m6_8_entry_ambiguity_non_inference_surface_report()[
        "status_booleans"
    ]
    for key in (
...
def test_docs_match_the_surface_boundary() -> None:
    docs = Path(
        "docs/CIVILIZATION_CORE_P4_M6_8_ENTRY_AMBIGUITY_NON_INFERENCE_SURFACE.md"
    ).read_text(encoding="utf-8")
    assert "# P4-M6.8 Entry Ambiguity Non-Inference Surface" in docs
    for phrase in REQUIRED_BOUNDARY_PHRASES:
        assert phrase in docs
    for field_id in FIELD_IDS:
        assert field_id in docs
    assert "true_flags=148" in docs
    assert "false_flags=273" in docs
