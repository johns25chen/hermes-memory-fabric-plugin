...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_review_gate import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_proposal import (
    _build as _valid_v3_2_approval_proposal,
)


_BASE_SOURCE_REPORT = _valid_v3_2_approval_proposal()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_review_gate(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_2_approval_proposal_creates_v3_3_review_gate_ready():
    result = _build()
    ...
    assert result["source_star_soul_approval_proposal_version"] == "3.2.0"
    for field in [
        "read_only",
        "read_only_memory",
        "review_gate_only",
    ...
    assert all(
        ...
        for check in result["approval_review_gate_checks"]
    )
    ...
    assert (
        review_gate[
            "source_star_soul_memory_continuity_boundary_approval_proposal_valid"
        ]
        is True
    )
    assert (
    ...
    assert set(review_gate["approval_review_gate_components"]) >= (
        APPROVAL_REVIEW_GATE_COMPONENT_KEYS
    )
...
def test_reviewed_chain_versions_exactly_match_required_ranges():
    ...
    assert (
        review_gate["reviewed_star_hub_chain_versions"]
        == expected_star_hub
    )


def test_blocked_source_approval_proposal_blocks_review_gate():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("source_star_soul_review_gate_version", "3.0.0"),
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_chain_versions"] = ["v3.2.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_star_hub_chain_versions"] = ["v3.2.0"]

    assert _build(report)["status"] == "blocked"


def test_invalid_nested_approval_proposal_blocks_review_gate():
    report = _valid_source_report()
    proposal = report[
...
def test_invalid_proposed_approval_boundaries_block_review_gate():
    report = _valid_source_report()
    report["proposed_star_soul_memory_continuity_approval_boundaries"] = {
        "unsafe": {"approval_proposal_only": False}
    }
...
def test_reviewed_approval_boundaries_are_review_only_non_authorizing():
    result = _build()
    boundaries = result[
    ...
    assert set(boundaries) >= APPROVAL_REVIEW_GATE_COMPONENT_KEYS
    for boundary in boundaries.values():
        ...
        for field in APPROVAL_REVIEW_GATE_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_review_gate_to_json(
        result
    )
...
@pytest.mark.parametrize("field", sorted(APPROVAL_REVIEW_GATE_FALSE_FLAGS))
def test_review_gate_preserves_all_top_level_false_flags(field: str):
    assert _build()[field] is False


@pytest.mark.parametrize(
    "field", sorted(APPROVAL_REVIEW_GATE_OBJECT_FALSE_FLAGS)
)
...
def test_review_gate_crosses_no_execution_or_authorization_boundary():
    result = _build()
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert boundary["boundary_approval_review_gate_only"] is True
    assert boundary["approval_reviewed_for_human_review_only"] is True
