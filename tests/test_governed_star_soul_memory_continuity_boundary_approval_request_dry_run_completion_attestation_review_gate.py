...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_review_gate import (
...
from tests.test_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_proposal import (
    _build as _valid_v3_10_completion_attestation_proposal,
)


_BASE_SOURCE_REPORT = _valid_v3_10_completion_attestation_proposal()
...
def _build(
    ...
    return build_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_review_gate(
        _valid_source_report() if report is None else report
    )


def test_valid_v3_10_completion_attestation_proposal_creates_v3_11_review_gate_ready():
    result = _build()
    ...
    assert all(
        ...
        for check in result[
            "approval_request_dry_run_completion_attestation_review_gate_checks"
        ]
    )
    ...
    assert (
        review_gate[
            "source_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_proposal_valid"
        ]
        is True
    )
    assert (
        review_gate[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_reviewed"
        ]
        is True
    )
    assert (
        review_gate[
            "star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_structurally_review_ready"
        ]
        is True
    )
    assert set(
        review_gate[
            "reviewed_dry_run_completion_attestation_boundary_components"
        ]
    ) >= APPROVAL_REQUEST_DRY_RUN_COMPLETION_ATTESTATION_REVIEW_GATE_COMPONENT_KEYS
...
def test_all_source_and_reviewed_lineages_are_exact():
    ...
    execution_expected = [*dry_run_expected, "v3.7.0", "v3.8.0"]
    ...
    reviewed_completion_attestation_expected = [
        *completion_attestation_expected,
        "v3.11.0",
    ]
    reviewed_completion_attestation_star_hub_expected = [
        *completion_attestation_star_hub_expected,
        "v3.11.0",
    ]
    result = _build()
    ...
    assert (
        result["reviewed_dry_run_completion_attestation_chain_versions"]
        == reviewed_completion_attestation_expected
    )
    assert (
        review_gate["reviewed_dry_run_completion_attestation_chain_versions"]
        == reviewed_completion_attestation_expected
    )
    assert (
        REVIEWED_DRY_RUN_COMPLETION_ATTESTATION_STAR_HUB_CHAIN_VERSIONS
...
def test_blocked_source_completion_attestation_proposal_blocks_review_gate():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("proposal_only", False),
        ("completion_attestation_proposal_only", False),
        ("dry_run_completion_attestation_proposal_only", False),
        (
...
def test_malformed_proposed_completion_attestation_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report["proposed_dry_run_completion_attestation_chain_versions"] = [
        "v3.10.0"
    ]
...
def test_malformed_proposed_completion_attestation_star_hub_chain_versions_blocks_review_gate():
    report = _valid_source_report()
    report[
...
def test_invalid_nested_completion_attestation_proposal_blocks_review_gate():
    report = _valid_source_report()
    proposal = report[
...
def test_invalid_proposed_completion_attestation_boundaries_block_review_gate():
    report = _valid_source_report()
    report[
...
def test_reviewed_completion_attestation_boundaries_are_review_only_and_non_executing():
    result = _build()
    boundaries = result[
    ...
    for boundary in boundaries.values():
        ...
        assert boundary["completion_attestation_review_gate_only"] is True
        assert boundary["dry_run_completion_attestation_review_gate_only"] is True
        assert (
            boundary[
                "approval_request_dry_run_completion_attestation_review_gate_only"
            ]
        ...
        for field in APPROVAL_REQUEST_DRY_RUN_COMPLETION_ATTESTATION_REVIEW_GATE_FALSE_FLAGS:
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_review_gate_to_json(
        result
    )
...
def test_layer_mapping_thirteenth_boundary_and_next_step_are_exact():
    ...
    assert (
        ...
        == "v3.11.0 reviews approval request dry-run completion attestation proposal boundaries for human review only and does not execute dry-run, execute dry-run plan, execute completion, execute attestation, finalize, close, create, submit, execute, grant, record approval, or record real human decisions"
    )
    ...
    assert (
        boundary[
            "boundary_approval_request_dry_run_completion_attestation_review_gate_only"
        ]
        is True
    )
    assert (
        boundary[
            "approval_request_dry_run_completion_attestation_reviewed_for_human_review_only"
        ]
        is True
    )
