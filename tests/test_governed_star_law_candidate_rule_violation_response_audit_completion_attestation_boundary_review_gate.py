...
from hermes_memory_fabric.governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate import (
...
from tests.test_governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal import (
    _build as _valid_v2_49_proposal,
)


def _valid_proposal_report() -> dict[str, object]:
    return deepcopy(_valid_v2_49_proposal())
...
def test_valid_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_creates_ready_review_gate():
    result = _build()
    ...
    assert result["source_proposal_version"] == "2.49.0"
    assert result["read_only"] is True
    assert result["read_only_memory"] is True
    assert result["review_gate_only"] is True
    assert (
        result[
            "candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_only"
        ]
        is True
    )
    assert (
        result[
            "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_completion_attestation_boundary_reviewed_for_human_review_only"
        ]
        is True
    )
    assert (
        result[
            "candidate_rule_violation_response_audit_completion_attestation_closure_boundary_proposal_ready_for_human_review_only"
        ]
        is True
    )
    ...
    assert (
        review_gate[
            "candidate_rule_violation_response_audit_completion_attestation_boundary_reviewed"
        ]
        is True
    )
    assert (
        review_gate[
            "candidate_rule_violation_response_audit_completion_attestation_boundary_structurally_review_ready"
        ]
        is True
    )
    assert (
    ...
    assert (
        review_gate[
            "source_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_valid"
        ]
        is True
    )
    assert (
        set(
            review_gate[
                "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_components"
            ]
        )
        == REVIEW_COMPONENTS
    )
...
def test_reviewed_chain_versions_exactly_match_v2_9_0_through_v2_49_0():
    expected = [f"v2.{minor}.0" for minor in range(9, 50)]

    assert REVIEWED_CHAIN_VERSIONS == expected
    assert _build()["reviewed_chain_versions"] == expected


def test_reviewed_star_hub_chain_versions_exactly_match_v2_19_0_through_v2_49_0():
    expected = [f"v2.{minor}.0" for minor in range(19, 50)]

    assert REVIEWED_STAR_HUB_CHAIN_VERSIONS == expected
    assert _build()["reviewed_star_hub_chain_versions"] == expected


def test_blocked_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_blocks_review_gate():
...
def test_unsafe_nested_proposal_authorization_flag_blocks_review_gate():
    ...
    proposal["candidate_rule_violation_response_audit_completion_authorized"] = (
        True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("proposal_only", False),
        (
            "candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_only",
            False,
        ),
        (
            "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_only",
            False,
        ),
...
def test_malformed_proposed_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_chain_versions"] = ["v2.49.0"]
...
def test_malformed_proposed_star_hub_chain_versions_blocks_review_gate():
    report = _valid_proposal_report()
    report["proposed_star_hub_chain_versions"] = ["v2.49.0"]
...
def test_reviewed_attestation_boundaries_are_review_only_and_non_authorizing():
    boundaries = _build()[
...
def test_sensitive_field_names_and_values_are_not_leaked():
    ...
    result = _build(report)
    serialized = governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_to_json(
        result
    )
