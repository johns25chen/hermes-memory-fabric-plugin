#!/usr/bin/env python3
"""Smoke test for governed Star-Law candidate rule violation observation boundary review gate."""
...
from smoke_governed_star_law_candidate_rule_violation_observation_boundary_proposal import (  # noqa: E402
    _valid_candidate_rule_enforcement_boundary_review_gate_report,
)


EXPECTED_FLAGS = {
    "status": "star_law_candidate_rule_violation_observation_boundary_review_gate_ready",
    "read_only": True,
...
def _valid_candidate_rule_violation_observation_boundary_proposal_report() -> dict[str, object]:
    ...
    try:
        ...
        for key, expected in EXPECTED_FLAGS.items():
            if result.get(key) != expected:
                print(
                    f"governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed {key}",
                    file=sys.stderr,
                )
                return 1
        ...
        if not isinstance(mapping, dict):
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed layer_mapping",
                file=sys.stderr,
            )
            return 1
        if mapping.get("primary_layer") != "星律记忆":
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed primary_layer",
                file=sys.stderr,
            )
            return 1
        if mapping.get("source_layer") != "星律记忆":
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed source_layer",
                file=sys.stderr,
            )
            return 1
        if mapping.get("primary_layer_status") != (
            ...
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed primary_layer_status",
                file=sys.stderr,
            )
            return 1
        if result.get("reviewed_chain_versions") != REVIEWED_CHAIN_VERSIONS:
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed reviewed_chain_versions",
                file=sys.stderr,
            )
            return 1
        if (
            ...
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed reviewed_star_hub_chain_versions",
                file=sys.stderr,
            )
            return 1
        if result.get("next_allowed_step") != READY_NEXT_ALLOWED_STEP:
            print(
                "governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed next_allowed_step",
                file=sys.stderr,
            )
            return 1
    except Exception as exc:
        print(
            f"governed_star_law_candidate_rule_violation_observation_boundary_review_gate=failed {type(exc).__name__}",
            file=sys.stderr,
        )
        return 1

    print("governed_star_law_candidate_rule_violation_observation_boundary_review_gate=passed")
    return 0
