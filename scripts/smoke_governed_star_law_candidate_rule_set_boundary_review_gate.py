#!/usr/bin/env python3
"""Smoke test for governed Star-Law candidate rule-set boundary review gate."""
...
from hermes_memory_fabric.governed_star_law_candidate_rule_set_boundary_proposal import (  # noqa: E402
...
from hermes_memory_fabric.governed_star_law_design_boundary_review_gate import (  # noqa: E402
    REVIEWED_CHAIN_VERSIONS as DESIGN_REVIEWED_CHAIN_VERSIONS,
    REVIEWED_STAR_HUB_CHAIN_VERSIONS as DESIGN_REVIEWED_STAR_HUB_CHAIN_VERSIONS,
)
...
def _valid_candidate_rule_set_boundary_proposal_report() -> dict[str, object]:
    return build_governed_star_law_candidate_rule_set_boundary_proposal(
        _valid_design_boundary_review_gate_report()
    )


def _valid_design_boundary_review_gate_report() -> dict[str, object]:
    return {
        "version": "2.30.0",
        ...
        "reviewed_chain_versions": list(DESIGN_REVIEWED_CHAIN_VERSIONS),
        "reviewed_star_hub_chain_versions": list(
            DESIGN_REVIEWED_STAR_HUB_CHAIN_VERSIONS
        ),
        "next_allowed_step": "v2.31.0 Star-Law candidate rule-set boundary proposal",
        "blocking_reasons": [],
        "star_law_design_boundary_review_gate": {
            "review_status": "star_law_design_boundary_reviewed_for_human_review_only",
        ...
        "reviewed_star_law_design_boundaries": {
...
def main() -> int:
    try:
        result = build_governed_star_law_candidate_rule_set_boundary_review_gate(
            _valid_candidate_rule_set_boundary_proposal_report()
        )
        for key, expected in EXPECTED_FLAGS.items():
            if result.get(key) != expected:
                print(
                    f"governed_star_law_candidate_rule_set_boundary_review_gate=failed {key}",
                    file=sys.stderr,
                )
                return 1
        ...
        if not isinstance(mapping, dict):
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed layer_mapping",
                file=sys.stderr,
            )
            return 1
        if mapping.get("primary_layer") != "星律记忆":
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed primary_layer",
                file=sys.stderr,
            )
            return 1
        if mapping.get("source_layer") != "星律记忆":
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed source_layer",
                file=sys.stderr,
            )
            return 1
        if mapping.get("primary_layer_status") != (
            ...
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed primary_layer_status",
                file=sys.stderr,
            )
            return 1
        if result.get("reviewed_chain_versions") != REVIEWED_CHAIN_VERSIONS:
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed reviewed_chain_versions",
                file=sys.stderr,
            )
            return 1
        if (
            ...
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed reviewed_star_hub_chain_versions",
                file=sys.stderr,
            )
            return 1
        if result.get("next_allowed_step") != READY_NEXT_ALLOWED_STEP:
            print(
                "governed_star_law_candidate_rule_set_boundary_review_gate=failed next_allowed_step",
                file=sys.stderr,
            )
            return 1
    except Exception as exc:
        print(
            f"governed_star_law_candidate_rule_set_boundary_review_gate=failed {type(exc).__name__}",
            file=sys.stderr,
        )
        return 1

    print("governed_star_law_candidate_rule_set_boundary_review_gate=passed")
    return 0
