#!/usr/bin/env python3
"""Smoke test for governed Star-Hub scheduling dry-run plan boundary proposal."""
...
from hermes_memory_fabric.governed_star_hub_scheduling_dry_run_plan_boundary_proposal import (  # noqa: E402
...
def main() -> int:
    try:
        result = build_governed_star_hub_scheduling_dry_run_plan_boundary_proposal(
            _valid_review_gate_report()
        )
        for key, expected in EXPECTED_FLAGS.items():
            if result.get(key) != expected:
                print(
                    f"governed_star_hub_scheduling_dry_run_plan_boundary_proposal=failed {key}",
                    file=sys.stderr,
                )
                return 1
        ...
        if mapping.get("primary_layer") != "星枢记忆":
            print(
                "governed_star_hub_scheduling_dry_run_plan_boundary_proposal=failed primary_layer",
                file=sys.stderr,
            )
            return 1
        if (
            ...
            print(
                "governed_star_hub_scheduling_dry_run_plan_boundary_proposal=failed primary_layer_status",
                file=sys.stderr,
            )
            return 1
        if result.get("proposed_chain_versions") != EXPECTED_CHAIN:
            print(
                "governed_star_hub_scheduling_dry_run_plan_boundary_proposal=failed proposed_chain_versions",
                file=sys.stderr,
            )
            return 1
        if result.get("next_allowed_step") != (
            ...
            print(
                "governed_star_hub_scheduling_dry_run_plan_boundary_proposal=failed next_allowed_step",
                file=sys.stderr,
            )
            return 1
    except Exception as exc:
        print(
            f"governed_star_hub_scheduling_dry_run_plan_boundary_proposal=failed {type(exc).__name__}",
            file=sys.stderr,
        )
        return 1

    print("governed_star_hub_scheduling_dry_run_plan_boundary_proposal=passed")
    return 0
