#!/usr/bin/env python3
"""Smoke test for the governed Star-Soul attestation approval review gate."""
...
from hermes_memory_fabric.governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_approval_review_gate import (  # noqa: E402
...
from smoke_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_approval_proposal import (  # noqa: E402
...
LABEL = (
    ...
    "dry_run_completion_attestation_approval_review_gate"
)


def _valid_v3_12_star_soul_approval_request_dry_run_completion_attestation_approval_proposal_report() -> (
    dict[str, object]
):
...
def main() -> int:
    try:
        result = build_governed_star_soul_memory_continuity_boundary_approval_request_dry_run_completion_attestation_approval_review_gate(
            _valid_v3_12_star_soul_approval_request_dry_run_completion_attestation_approval_proposal_report()
        )
        if (
            result.get("status")
        ...
        for key, expected in (
            APPROVAL_REQUEST_DRY_RUN_COMPLETION_ATTESTATION_APPROVAL_REVIEW_GATE_FALSE_FLAGS.items()
        ):
        ...
        if not isinstance(review_gate, dict):
            raise AssertionError("approval_review_gate")
        for key, expected in (
            APPROVAL_REQUEST_DRY_RUN_COMPLETION_ATTESTATION_APPROVAL_REVIEW_GATE_OBJECT_FALSE_FLAGS.items()
        ):
            if review_gate.get(key) is not expected:
                raise AssertionError(f"approval_review_gate_{key}")
        for key in [
            "approval_granted",
