...
from hermes_memory_fabric.governance_execution_adapter_manifest_policy_gate import (  # noqa: E402
    SAFETY_BOUNDARIES,
...
def main() -> int:
    try:
        first = build_governance_execution_adapter_manifest_policy_gate()
        second = build_governance_execution_adapter_manifest_policy_gate()

        if first != second:
            raise AssertionError("manifest policy gate changed between builds")
        ...
        if (
            ...
            raise AssertionError("manifest_policy_gate_stage")
        if first["manifest_policy_gate_mode"] != "policy_gate_only":
            raise AssertionError("manifest_policy_gate_mode")
        ...
        if (
            first["deterministic_manifest_policy_gate_hash"]
            != second["deterministic_manifest_policy_gate_hash"]
        ):
            raise AssertionError("deterministic_manifest_policy_gate_hash")
        for decision in first["manifest_policy_gate_decisions"]:
            if decision["decision_status"] != "pass":
                raise AssertionError(decision["decision_name"])
        for contract in first["manifest_policy_gate_contracts"]:
            if contract["contract_status"] != "pass":
                raise AssertionError(contract["contract_name"])
        for check in first["manifest_policy_gate_checks"]:
            if check["check_status"] != "pass":
                raise AssertionError(check["check_name"])
        _assert_safety(first)
        json_output = governance_execution_adapter_manifest_policy_gate_to_json(
            first
        )
        _assert_no_sensitive_leak(
            {
                "decisions": first["manifest_policy_gate_decisions"],
                "contracts": first["manifest_policy_gate_contracts"],
                "checks": first["manifest_policy_gate_checks"],
                "summary": first["manifest_policy_gate_summary"],
                "hashes": {
                    "manifest_policy_gate": first[
                        "deterministic_manifest_policy_gate_hash"
                    ],
                    "manifest_validation_matrix": first[
        ...
        print(
            f"governance_execution_adapter_manifest_policy_gate=failed {exc}",
            file=sys.stderr,
        )
        return 1

    print("governance_execution_adapter_manifest_policy_gate=passed")
    return 0
