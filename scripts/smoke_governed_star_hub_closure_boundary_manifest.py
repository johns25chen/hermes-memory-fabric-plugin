#!/usr/bin/env python3
"""Smoke test for governed Star-Hub closure boundary manifest."""
...
EXPECTED_FLAGS = {
    "status": "star_hub_closure_boundary_manifest_ready",
    "read_only": True,
...
def main() -> int:
    try:
        ...
        result = build_governed_star_hub_closure_boundary_manifest(audit_report)
        for key, expected in EXPECTED_FLAGS.items():
            if result.get(key) != expected:
                print(
                    f"governed_star_hub_closure_boundary_manifest=failed {key}",
                    file=sys.stderr,
                )
                return 1
        ...
        if mapping.get("primary_layer") != "星枢记忆":
            print(
                "governed_star_hub_closure_boundary_manifest=failed primary_layer",
                file=sys.stderr,
            )
            return 1
        if mapping.get("primary_layer_status") != (
            ...
            print(
                "governed_star_hub_closure_boundary_manifest=failed primary_layer_status",
                file=sys.stderr,
            )
            return 1
        if result.get("manifested_chain_versions") != EXPECTED_CHAIN:
            print(
                "governed_star_hub_closure_boundary_manifest=failed manifested_chain_versions",
                file=sys.stderr,
            )
            return 1
        if result.get("manifested_star_hub_chain_versions") != EXPECTED_STAR_HUB_CHAIN:
            print(
                "governed_star_hub_closure_boundary_manifest=failed manifested_star_hub_chain_versions",
                file=sys.stderr,
            )
            return 1
        if result.get("next_allowed_step") != "v2.28.0 Star-Law preflight boundary analysis":
            print(
                "governed_star_hub_closure_boundary_manifest=failed next_allowed_step",
                file=sys.stderr,
            )
            return 1
    except Exception as exc:
        print(
            f"governed_star_hub_closure_boundary_manifest=failed {type(exc).__name__}",
            file=sys.stderr,
        )
        return 1

    print("governed_star_hub_closure_boundary_manifest=passed")
    return 0
