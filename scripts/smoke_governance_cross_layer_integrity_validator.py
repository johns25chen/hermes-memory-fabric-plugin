...
from hermes_memory_fabric.governance_cross_layer_integrity_validator import (  # noqa: E402
...
def main() -> int:
    try:
        ...
        expected = {
            ...
            "upstream_source_audit_replay_engine_status": "pass",
            "upstream_handoff_status": (
                "ready_for_cross_layer_integrity_validator_design"
            ),
            "upstream_next_stage": "v6.12_cross_layer_integrity_validator",
        ...
        upstream_hash = first["upstream_source_audit_replay_engine_hash"]
        if not isinstance(upstream_hash, str) or len(upstream_hash) != 64:
            raise AssertionError("upstream hash")
        if upstream_hash != second[
            "upstream_source_audit_replay_engine_hash"
        ]:
            raise AssertionError("upstream hash stability")

        records = first["cross_layer_integrity_records"]
        if [record["integrity_record_id"] for record in records] != list(
            REQUIRED_GOVERNANCE_CROSS_LAYER_INTEGRITY_VALIDATOR_RECORD_IDS
        ):
            raise AssertionError("record ids")
        for record in records:
            if record["integrity_record_status"] != "registered_metadata_only":
                raise AssertionError(record["integrity_record_id"])
            if record["metadata_only_disposition"] != "metadata_only":
                raise AssertionError("metadata only disposition")
            if record["cross_layer_integrity_metadata_required"] is not True:
        ...
        for container, status_key in (
            ("cross_layer_integrity_sections", "section_status"),
            ("cross_layer_integrity_contracts", "contract_status"),
            ("cross_layer_integrity_checks", "check_status"),
        ):
            if not all(
        ...
        integrity_hash = first["deterministic_cross_layer_integrity_validator_hash"]
        if not isinstance(integrity_hash, str) or len(integrity_hash) != 64:
            raise AssertionError("integrity hash")
        if integrity_hash != second["deterministic_cross_layer_integrity_validator_hash"]:
        ...
        print(
            f"governance_cross_layer_integrity_validator=failed:{exc}",
            file=sys.stderr,
        )
        return 1

    print("governance_cross_layer_integrity_validator=passed")
    return 0
