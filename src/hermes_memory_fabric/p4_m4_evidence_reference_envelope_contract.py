...
P4_M4_2_PACKAGE_VERSION = "6.16.0"


@dataclass(frozen=True)
class EvidenceReferenceEnvelopeContractField:
    field_order: int
    field_id: str
    field_name: str
    field_purpose: str
    p4_m4_evidence_reference_envelope_contract_category: str
    p4_m4_evidence_reference_envelope_contract_semantics_disabled: str
...
EVIDENCE_REFERENCE_ENVELOPE_CONTRACT_BOUNDARY = (
    "P4-M4.2 Evidence Reference Envelope Contract read-only definition-only "
    "evidence-reference-envelope-design-only inspection-only. P4-M4.2 "
    "Evidence Reference Envelope Contract is definition only. "
...
def _field_purpose(field_id: str) -> str:
    return (
        ...
        "evidence-reference-envelope-design-only inspection-only P4-M4.2 "
        "Evidence Reference Envelope Contract context; P4-M4.1 Entry Gate "
        "Design Request Envelope Contract remains the direct prior request "
        "envelope reference; P4-M4.0 Entry Gate Design Boundary Contract "
        "remains the inherited prior design boundary reference; P4-M3.16 "
        "Governed Transition Intake Final Phase Handoff Summary remains the "
        "inherited prior closed-phase handoff reference; P4-M3 static "
        "definition chain remains closed; P4-M4 evidence reference envelope "
        "design starts only as a static envelope definition; no evidence "
        "intake; no live evidence parsing; no evidence validation; no "
        "reference resolution; no reference validation; no citation "
        "validation; no source fetching; no provenance writing; no record "
        "creation; no memory mutation; no P4-M5; no v7; no productization; "
        "no UI; no Operator Console; no version bump; no tag."
    )


_EVIDENCE_REFERENCE_ENVELOPE_CONTRACT_FIELDS = tuple(
    EvidenceReferenceEnvelopeContractField(
        index,
        field_id,
        _FIELD_NAMES[index - 1],
        _field_purpose(field_id),
        "p4-m4-evidence-reference-envelope-contract-category",
        (
            "no evidence intake semantics; no live evidence parsing semantics; "
            "no evidence validation semantics; no evidence acceptance "
            "semantics; no evidence rejection semantics; no evidence routing "
            "semantics; no evidence execution semantics; no evidence record "
            "creation semantics; no reference resolution semantics; no "
            "reference validation semantics; no citation validation semantics; "
...
def list_evidence_reference_envelope_contract_fields() -> tuple[EvidenceReferenceEnvelopeContractField, ...]:
    return _EVIDENCE_REFERENCE_ENVELOPE_CONTRACT_FIELDS


def evidence_reference_envelope_contract_field_ids() -> tuple[str, ...]:
    return tuple(
        field.field_id for field in list_evidence_reference_envelope_contract_fields()
    )


def render_evidence_reference_envelope_contract_markdown(
    fields: Sequence[EvidenceReferenceEnvelopeContractField] | None = None,
) -> str:
    field_values = (
        tuple(fields)
        if fields is not None
        else list_evidence_reference_envelope_contract_fields()
    )
    ...
    lines.extend(["", "## Evidence Reference Envelope Contract Fields", ""])
    for field in field_values:
        lines.extend([
            ...
            "- P4-M4 evidence reference envelope contract category: "
            f"{field.p4_m4_evidence_reference_envelope_contract_category}",
            "- P4-M4 evidence reference envelope contract semantics disabled: "
            f"{field.p4_m4_evidence_reference_envelope_contract_semantics_disabled}",
            "",
        ])
    return "\n".join(lines).rstrip() + "\n"


def evidence_reference_envelope_contract_as_dicts() -> tuple[dict[str, object], ...]:
    return tuple(
        asdict(field)
...
def evidence_reference_envelope_contract_report() -> dict[str, object]:
    status: dict[str, object] = {
        "phase": "P4-M4.2",
        "feature": "Evidence Reference Envelope Contract",
        "mode": "read-only",
        "boundary": EVIDENCE_REFERENCE_ENVELOPE_CONTRACT_BOUNDARY,
        "package_version": P4_M4_2_PACKAGE_VERSION,
        "evidence_reference_envelope_contract_field_count": len(_FIELD_IDS),
        "referenced_p4_m4_1_entry_gate_design_request_envelope_contract_field_count": len(
            entry_gate_design_request_envelope_contract_field_ids()
        ),
    }
