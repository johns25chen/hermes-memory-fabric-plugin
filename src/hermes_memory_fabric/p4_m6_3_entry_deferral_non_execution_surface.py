...
P4_M6_3_PACKAGE_VERSION = "6.16.0"


@dataclass(frozen=True)
class P4M63EntryDeferralNonExecutionSurfaceField:
    field_order: int
    field_id: str
    field_name: str
    field_purpose: str
    p4_m6_3_entry_deferral_non_execution_surface_category: str
    p4_m6_3_entry_deferral_non_execution_surface_semantics_disabled: str
...
P4_M6_3_ENTRY_DEFERRAL_NON_EXECUTION_SURFACE_BOUNDARY = (
    "P4-M6.3 Entry Deferral Non-Execution Surface read-only "
    "definition-only p4-m6-3-entry-deferral-non-execution-surface-only "
    "entry-deferral-non-execution-surface-only "
    "static-deferral-labels-only non-execution-deferral-surface-only "
    "non-rejection-deferral-surface-only non-denial-deferral-surface-only "
    "non-approval-deferral-surface-only "
    "non-authorization-deferral-surface-only "
    "non-confirmation-deferral-surface-only "
    "non-recommendation-deferral-surface-only "
    "non-routing-deferral-surface-only non-validation-deferral-surface-only "
    "non-inference-deferral-surface-only non-scoring-deferral-surface-only "
    "non-verdict-deferral-surface-only non-record-deferral-surface-only "
    "non-storage-deferral-surface-only non-persistence-deferral-surface-only "
    "non-mutation-deferral-surface-only "
    "implementation-corridor-not-started-boundary-only "
...
def _field_purpose(field_id: str) -> str:
    return (
        ...
        f"{P4_M6_3_ENTRY_DEFERRAL_NON_EXECUTION_SURFACE_BOUNDARY}"
    )


_P4_M6_3_ENTRY_DEFERRAL_NON_EXECUTION_SURFACE_FIELDS = tuple(
    P4M63EntryDeferralNonExecutionSurfaceField(
        index,
        field_id,
        f"P4-M6.3 Entry Deferral Non-Execution Surface Field {index}",
        _field_purpose(field_id),
        "p4-m6-3-entry-deferral-non-execution-surface-category",
        (
            "no deferral validation semantics; no deferral inference "
            "semantics; no deferral scoring semantics; no deferral verdict "
            "semantics; no rejection semantics; no denial semantics; no approval "
            "semantics; no authorization semantics; no confirmation semantics; "
            "no recommendation semantics; no routing semantics; no execution "
            "semantics; no readiness evidence semantics; no validation input "
            "semantics; no record creation semantics; no storage semantics; no "
            "persistence semantics; no mutation semantics; no API implementation "
            ...
            "auto-call semantics; no external system integration semantics; no "
            "network access semantics; no live API endpoint probe semantics; no "
            "live MCP server probe semantics; no live provider probe semantics; "
            ...
            "semantics; no connector data fetch semantics; no connector data "
            "write semantics; no connector mutation semantics; no authentication "
            "testing semantics; no authorization testing semantics; no schema "
            "validation semantics; no readiness validation semantics; no "
            "readiness inference semantics; no readiness scoring semantics; no "
            "readiness verdict semantics; no implementation start semantics; no "
            "v7 semantics; no productization semantics; no UI semantics; no "
...
def list_p4_m6_3_entry_deferral_non_execution_surface_fields() -> (
    tuple[P4M63EntryDeferralNonExecutionSurfaceField, ...]
):
    return _P4_M6_3_ENTRY_DEFERRAL_NON_EXECUTION_SURFACE_FIELDS


def p4_m6_3_entry_deferral_non_execution_surface_field_ids() -> tuple[str, ...]:
    return tuple(
        field.field_id
        for field in list_p4_m6_3_entry_deferral_non_execution_surface_fields()
    )


def render_p4_m6_3_entry_deferral_non_execution_surface_markdown(
    fields: Sequence[P4M63EntryDeferralNonExecutionSurfaceField] | None = None,
) -> str:
    field_values = (
        tuple(fields)
        if fields is not None
        else list_p4_m6_3_entry_deferral_non_execution_surface_fields()
    )
    ...
    for field in field_values:
        lines.extend(
            [
                ...
                "- P4-M6.3 entry deferral non-execution surface category: "
                f"{field.p4_m6_3_entry_deferral_non_execution_surface_category}",
                "- P4-M6.3 entry deferral non-execution surface semantics disabled: "
                f"{field.p4_m6_3_entry_deferral_non_execution_surface_semantics_disabled}",
                "",
            ]
        )
    return "\n".join(lines).rstrip() + "\n"
...
def p4_m6_3_entry_deferral_non_execution_surface_as_dicts() -> (
...
def p4_m6_3_entry_deferral_non_execution_surface_report() -> dict[str, object]:
    status: dict[str, object] = {
        "phase": "P4-M6.3",
        "feature": "Entry Deferral Non-Execution Surface",
        "mode": "read-only",
        "boundary": P4_M6_3_ENTRY_DEFERRAL_NON_EXECUTION_SURFACE_BOUNDARY,
        "package_version": P4_M6_3_PACKAGE_VERSION,
        "p4_m6_3_entry_deferral_non_execution_surface_field_count": len(_FIELD_IDS),
