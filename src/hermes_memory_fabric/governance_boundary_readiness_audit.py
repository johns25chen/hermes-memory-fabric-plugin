...
GOVERNANCE_BOUNDARY_READINESS_AUDIT_VERSION = "5.13.0"
GOVERNANCE_BOUNDARY_READINESS_AUDIT_SCHEMA_VERSION = "5.13.0"
...
def _version_alignment_check() -> dict[str, Any]:
    ...
    return _readiness_check(
        ...
        blocking_reasons=[
            "governance stack versions must align to 5.13.0"
        ]
        if mismatches
        else [],
    )
...
def _validation_matrix_status_check(
    ...
    blocking_reasons = _deduplicate(
        [
            *(
                ["validation matrix version must equal 5.13.0"]
...
def _fixture_pack_presence_check(
    ...
    blocking_reasons = _deduplicate(
        [
            *(
                ["fixture pack version must equal 5.13.0"]
