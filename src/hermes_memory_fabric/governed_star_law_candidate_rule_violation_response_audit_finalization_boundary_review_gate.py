"""Governed Star-Law candidate rule violation response audit finalization boundary review gate."""
...
from .governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal import (
    PROPOSAL_FALSE_FLAGS as SOURCE_PROPOSAL_FALSE_FLAGS,
    PROPOSAL_OBJECT_FALSE_FLAGS as SOURCE_PROPOSAL_OBJECT_FALSE_FLAGS,
    PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_FINALIZATION_BOUNDARY_KEYS as SOURCE_PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_FINALIZATION_BOUNDARY_KEYS,
...
GOVERNED_STAR_LAW_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_FINALIZATION_BOUNDARY_REVIEW_GATE_VERSION = (
    "2.46.0"
)
SOURCE_PROPOSAL_VERSION = "2.45.0"

REVIEWED_CHAIN_VERSIONS = [*SOURCE_PROPOSED_CHAIN_VERSIONS, "v2.45.0"]
...
LAYER_MAPPING = {
    ...
    "source_layer": "星律记忆",
    "source_layer_status": "Star-Law candidate rule violation response audit finalization boundary proposal complete for human review only",
    "supporting_layers": ["星穹记忆", "星域记忆", "星界记忆", "星辰记忆"],
    "direction": "Star-Law candidate rule violation response audit finalization boundary proposal -> Star-Law candidate rule violation response audit finalization boundary review gate",
}
...
SOURCE_REQUIRED_FLAGS = {
    ...
    "candidate_rule_violation_response_audit_finalization_boundary_proposed_for_human_review_only": True,
    "candidate_rule_violation_response_audit_finalization_boundary_review_gate_ready_for_human_review_only": True,
    **SOURCE_PROPOSAL_FALSE_FLAGS,
}

SOURCE_PROPOSAL_OBJECT_REQUIRED_FLAGS = {
    "candidate_rule_violation_response_audit_finalization_boundary_proposed": True,
    "candidate_rule_violation_response_audit_finalization_boundary_review_gate_ready_for_human_review_only": True,
    "source_star_law_candidate_rule_violation_response_audit_closure_boundary_review_gate_valid": True,
...
REVIEW_COMPONENTS = {
    "source_candidate_rule_violation_response_audit_finalization_boundary_proposal_review",
    "response_audit_finalization_eligibility_boundary_review",
    "response_audit_finalization_input_boundary_review",
    "response_audit_finalization_output_boundary_review",
    "response_audit_finalization_non_execution_boundary_review",
    "response_audit_finalization_non_recording_boundary_review",
    "response_audit_finalization_non_ledger_boundary_review",
    "response_audit_finalization_non_authorization_boundary_review",
...
def build_governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate(
    proposal_report: Mapping[str, Any],
) -> dict[str, Any]:
    """Return a local-only, non-authorizing audit finalization review gate."""
    ...
    return {
        "version": GOVERNED_STAR_LAW_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_FINALIZATION_BOUNDARY_REVIEW_GATE_VERSION,
        "status": status,
        **REVIEW_GATE_FLAGS,
        "candidate_rule_violation_response_audit_finalization_boundary_reviewed_for_human_review_only": ready,
        "candidate_rule_violation_response_audit_completion_boundary_proposal_ready_for_human_review_only": ready,
        ...
        "star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate_summary": _review_gate_summary(
            ready
        ),
        "review_gate_checks": checks,
        "source_proposal_summary": _source_proposal_summary(ready),
        "star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate": _review_gate(
            ready
        ),
        "reviewed_candidate_rule_violation_response_audit_finalization_boundaries": _reviewed_boundaries(
            ready
        ),
        ...
        "future_candidate_rule_violation_response_audit_completion_boundary_constraints": _future_completion_constraints(),
...
def governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate_to_json(
    report: Mapping[str, Any],
) -> str:
    """Serialize the audit finalization boundary review gate deterministically."""
...
def _review_gate_checks(
    ...
    _add_check(
        ...
        "source_proposal_version_must_be_2_45_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_proposal_status_must_be_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_ready",
    )
    ...
    _add_check(
        ...
        "source_must_not_claim_finalization_closure_audit_response_correction_violation_rule_autonomous_handoff_scheduling_approval_write_execution_dry_run_or_civilization_core_actions",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_must_not_textually_claim_finalization_closure_audit_response_correction_violation_rule_autonomous_write_approval_next_layer_or_civilization_core_actions",
    )
    ...
    if isinstance(mapping, Mapping):
        ...
        layer_checks = {
            ...
            "source_primary_layer_status": (
                ...
                "source_primary_layer_status_must_be_audit_finalization_boundary_proposal_only",
            ),
            "source_source_layer": (
            ...
            "source_source_layer_status": (
                ...
                == "Star-Law candidate rule violation response audit closure boundary review gate complete for human review only",
                "source_source_layer_status_must_be_audit_closure_boundary_review_gate_complete_for_human_review_only",
            ),
            ...
            "source_direction": (
                ...
                == "Star-Law candidate rule violation response audit closure boundary review gate -> Star-Law candidate rule violation response audit finalization boundary proposal",
                "source_direction_must_match_audit_closure_review_gate_to_audit_finalization_boundary_proposal",
            ),
        }
    ...
    _add_check(
        ...
        "source_proposed_chain_versions_must_exactly_match_v2_9_0_through_v2_44_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        == SOURCE_PROPOSED_STAR_HUB_CHAIN_VERSIONS,
        "source_proposed_star_hub_chain_versions_must_exactly_match_v2_19_0_through_v2_44_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_next_allowed_step_must_be_v2_46_0_audit_finalization_boundary_review_gate",
    )
    _add_check(
        checks,
        blocking_reasons,
    ...
    _add_check(
        checks,
        blocking_reasons,
        "source_audit_finalization_boundary_proposal_shape",
        isinstance(proposal, Mapping),
        "source_audit_finalization_boundary_proposal_must_be_mapping",
    )
    if isinstance(proposal, Mapping):
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_finalization_boundary_proposal_status",
            ...
            "source_audit_finalization_boundary_proposal_status_must_be_human_review_only",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_finalization_boundary_proposal_type",
            proposal.get("boundary_type")
            == "star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal",
            "source_audit_finalization_boundary_proposal_type_must_match",
        )
        for key, expected in SOURCE_PROPOSAL_OBJECT_REQUIRED_FLAGS.items():
            _add_check(
                checks,
                blocking_reasons,
                f"source_audit_finalization_boundary_proposal_{key}",
                proposal.get(key) is expected,
                f"source_audit_finalization_boundary_proposal_{key}_must_be_{str(expected).lower()}",
            )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_finalization_boundary_proposal_chain_versions",
            proposal.get("proposed_chain_versions")
            == SOURCE_PROPOSED_CHAIN_VERSIONS,
            "source_audit_finalization_boundary_proposal_chain_versions_must_match",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_finalization_boundary_proposal_star_hub_chain_versions",
            proposal.get("proposed_star_hub_chain_versions")
            == SOURCE_PROPOSED_STAR_HUB_CHAIN_VERSIONS,
            "source_audit_finalization_boundary_proposal_star_hub_chain_versions_must_match",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_finalization_boundary_proposal_next_allowed_step",
            proposal.get("next_allowed_step")
            == "v2.46.0 Star-Law candidate rule violation response audit finalization boundary review gate",
            "source_audit_finalization_boundary_proposal_next_allowed_step_must_match",
        )
    ...
    _add_check(
        checks,
        blocking_reasons,
        "source_proposed_audit_finalization_boundaries_shape",
        isinstance(boundaries, Mapping),
        "source_proposed_audit_finalization_boundaries_must_be_mapping",
    )
    if isinstance(boundaries, Mapping):
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_audit_finalization_boundaries_required_keys",
            set(boundaries)
            == SOURCE_PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_FINALIZATION_BOUNDARY_KEYS,
            "source_proposed_audit_finalization_boundaries_must_exactly_match_required_boundaries",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_audit_finalization_boundaries_proposal_only",
            _proposed_boundaries_remain_proposal_only(boundaries),
            "source_proposed_audit_finalization_boundaries_must_remain_proposal_only_and_non_authorizing",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_audit_finalization_boundaries_no_unsafe_claims",
            _unsafe_boundary_claims(
                boundaries, PROPOSED_BOUNDARY_ALLOWED_TRUE_KEYS
            )
            == [],
            "source_proposed_audit_finalization_boundaries_must_not_claim_execution_authorization_recording_enforcement_or_write_actions",
        )
...
def _source_proposal_summary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "source_audit_finalization_boundary_proposal_valid": ready,
...
def _review_gate(ready: bool) -> dict[str, Any]:
    return {
        "review_status": (
            "star_law_candidate_rule_violation_response_audit_finalization_boundary_reviewed_for_human_review_only"
            if ready
            else "blocked_no_star_law_candidate_rule_violation_response_audit_finalization_boundary_review_readiness_claim"
        ),
        "boundary_type": "star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate",
        "source_proposal_version": SOURCE_PROPOSAL_VERSION,
        "candidate_rule_violation_response_audit_finalization_boundary_reviewed": ready,
        "candidate_rule_violation_response_audit_finalization_boundary_structurally_review_ready": ready,
        "candidate_rule_violation_response_audit_completion_boundary_proposal_ready_for_human_review_only": ready,
        "source_star_law_candidate_rule_violation_response_audit_finalization_boundary_proposal_valid": ready,
        **REVIEW_OBJECT_FALSE_FLAGS,
        "reviewed_chain_versions": list(REVIEWED_CHAIN_VERSIONS),
        "reviewed_star_hub_chain_versions": list(REVIEWED_STAR_HUB_CHAIN_VERSIONS),
        "star_law_candidate_rule_violation_response_audit_finalization_boundary_review_components": _review_components(
            ready
        ),
        "next_allowed_step": READY_NEXT_ALLOWED_STEP,
        "non_authorization_notice": (
            "The Star-Law candidate rule violation response audit finalization boundary proposal has been reviewed for human review only. The candidate rule violation response audit completion boundary proposal is structurally ready for a separate later governed proposal step. This review performs no finalization, closure, audit, response, correction, enforcement, write, approval, handoff, scheduling, or autonomous action."
            if ready
            else "The Star-Law candidate rule violation response audit finalization boundary review gate is blocked and makes no readiness, execution, authorization, recording, ledger, enforcement, write, approval, handoff, scheduling, next-layer, or Civilization Core completion claim."
        ),
    }
...
def _reviewed_boundaries(ready: bool) -> dict[str, dict[str, Any]]:
    status = "reviewed_for_human_review_only" if ready else "blocked"
    summaries = {
        "response_audit_finalization_eligibility_boundary": "Eligibility was reviewed using sanitized v2.45 proposal readiness and exact chain traceability only.",
        "response_audit_finalization_input_scope_boundary": "Input scope was reviewed as proposal status, chain metadata, layer mapping, and proposal-only boundary shape.",
        "response_audit_finalization_output_scope_boundary": "Output scope was reviewed as sanitized boundary summaries for a later governed proposal.",
        "response_audit_finalization_non_execution_boundary": "Finalization behavior remains outside this review gate.",
        "response_audit_finalization_non_recording_boundary": "Finalization record creation remains outside this review gate.",
        "response_audit_finalization_non_ledger_boundary": "Finalization-ledger creation remains outside this review gate.",
        "response_audit_finalization_non_authorization_boundary": "Finalization authority remains outside this review gate.",
        "finalization_execution_non_authorization_boundary": "Finalization execution authority remains outside this review gate.",
        "finalization_record_non_creation_boundary": "Finalization record creation remains outside this review gate.",
        "finalization_ledger_non_creation_boundary": "Finalization-ledger creation remains outside this review gate.",
        "closure_execution_non_authorization_boundary": "Closure execution authority remains outside this review gate.",
        "closure_record_non_creation_boundary": "Closure record creation remains outside this review gate.",
        "closure_ledger_non_creation_boundary": "Closure-ledger creation remains outside this review gate.",
        "audit_execution_non_authorization_boundary": "Audit execution authority remains outside this review gate.",
        "audit_logging_non_execution_boundary": "Audit logging remains outside this review gate.",
        "audit_record_non_creation_boundary": "Audit record creation remains outside this review gate.",
        "audit_ledger_non_creation_boundary": "Audit-ledger creation remains outside this review gate.",
        "violation_response_non_execution_boundary": "Violation response remains outside this review gate.",
        "automated_correction_non_execution_boundary": "Automated correction remains outside this review gate.",
        "violation_enforcement_non_execution_boundary": "Violation enforcement remains outside this review gate.",
        "candidate_rule_enforcement_non_authorization_boundary": "Candidate-rule enforcement authority remains outside this review gate.",
        "human_operator_finalization_review_boundary": "Human Operator review remains required before any later governed stage.",
        "evidence_lineage_finalization_boundary": "Evidence lineage was reviewed as source version and exact chain metadata only.",
        "auditability_finalization_design_boundary": "Auditability was reviewed as deterministic local report structure only.",
        "false_positive_finalization_prevention_boundary": "False-positive prevention remains a required later human-review concern.",
        "rollback_finalization_boundary": "Rollback remains a review category without rollback action.",
        "suspension_finalization_boundary": "Suspension remains a review category without suspension action.",
        "non_authorization_boundary": "No execution, recording, ledger, enforcement, write, approval, handoff, or scheduling authority is granted.",
        "memory_write_boundary": "Durable memory write authority remains outside this review gate.",
        "memory_graph_boundary": "Memory Graph mutation authority remains outside this review gate.",
        "operation_ledger_boundary": "Operation-ledger creation remains outside this review gate.",
        "openclaw_boundary": "OpenClaw execution remains outside this review gate.",
        "approval_boundary": "Approval request and approval actions remain outside this review gate.",
        "future_review_gate_boundary": "The reviewed successor is limited to the v2.47.0 audit completion boundary proposal.",
    }
    return {
        key: {
            "review_only": True,
...
def _non_authorization_boundary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "only_permits_later_audit_completion_boundary_proposal": ready,
...
def _future_completion_constraints() -> list[str]:
    return [
        "A later v2.47.0 audit completion boundary proposal may consume this sanitized review report through a separate governed process.",
        "A later completion boundary proposal remains proposal-only and cannot execute finalization, closure, audit, response, correction, enforcement, writes, approvals, handoffs, scheduling, or autonomous actions.",
...
def _next_stage_constraints() -> list[str]:
    return [
        ...
        "v2.46.0 is a Star-Law candidate rule violation response audit finalization boundary review gate only.",
...
def _scope_boundaries() -> list[str]:
    return [
        ...
        "This v2.46.0 step is limited to 星律记忆 audit finalization boundary review only.",
    ]


def _risk_notes(sensitive_field_count: int) -> list[str]:
    notes = [
        "Review readiness must not be interpreted as finalization, closure, audit, response, correction, enforcement, write, approval, handoff, scheduling, or autonomous authority.",
...
def _required_human_actions(
    ...
    if ready:
        return [
            "human_operator_must_review_v2_46_0_audit_finalization_boundary_review_gate_before_any_v2_47_0_audit_completion_boundary_proposal",
    ...
    actions = [
        "repair_source_v2_45_0_audit_finalization_boundary_proposal_before_review_gate_can_continue",
        "confirm_all_unsafe_execution_authorization_recording_ledger_enforcement_write_approval_handoff_scheduling_and_completion_flags_remain_false",
        "rerun_local_v2_46_0_audit_finalization_boundary_review_gate_after_blockers_are_resolved",
    ]
    if blocking_reasons:
        actions.append(
            "resolve_blocking_reasons_before_audit_finalization_boundary_review_gate_can_be_ready"
        )
    return actions
...
__all__ = [
    ...
    "REVIEW_COMPONENTS",
    "SENSITIVE_KEYS",
    "build_governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate",
    "governed_star_law_candidate_rule_violation_response_audit_finalization_boundary_review_gate_to_json",
]
