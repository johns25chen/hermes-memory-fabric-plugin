...
P4_M4_0_PACKAGE_VERSION = "6.16.0"


@dataclass(frozen=True)
class EntryGateDesignBoundaryContractField:
    field_order: int
    field_id: str
    field_name: str
    field_purpose: str
    p4_m4_entry_gate_design_boundary_contract_category: str
    p4_m4_entry_gate_design_boundary_contract_semantics_disabled: str
...
BOUNDARY_PHRASE_LINES = (
    ...
    "P4-M4.0 is read-only",
    "P4-M4.0 is design-boundary-only",
    "P4-M4.0 is inspection-only",
    ...
    "P4-M4.0 is not reference integrity validation",
    ...
    "P4-M4.0 is not evidence mutation",
    "P4-M4.0 is not human context mutation",
...
ENTRY_GATE_DESIGN_BOUNDARY_CONTRACT_BOUNDARY = (
    "P4-M4.0 Entry Gate Design Boundary Contract read-only definition-only "
    "design-boundary-only inspection-only. P4-M4.0 Entry Gate Design "
    "Boundary Contract is definition only. "
...
def _field_purpose(field_id: str) -> str:
    return (
        f"Defines {field_id} as read-only definition-only design-boundary-only "
        "inspection-only P4-M4.0 Entry Gate Design Boundary Contract context; "
        "P4-M3.16 Governed Transition Intake Final Phase Handoff Summary "
        "remains the source prior phase handoff summary reference; P4-M3 "
        "static definition chain remains closed; P4-M4 design layer starts "
        "only as a boundary definition; P4-M4 entry gate remains not "
        "implemented; no live validation; no entry gate validation; no "
        "readiness validation; no working entry gate; no gate activation; no "
        "gate execution; no readiness verdict; no validation verdict; no "
        "entry verdict; no gate verdict; no approval; no authorization; no "
...
_ENTRY_GATE_DESIGN_BOUNDARY_CONTRACT_FIELDS = tuple(
    EntryGateDesignBoundaryContractField(
        index,
        field_id,
        _FIELD_NAMES[index - 1],
        _field_purpose(field_id),
        "p4-m4-entry-gate-design-boundary-contract-category",
        (
            "no live validation semantics; no entry gate validation semantics; "
            "no readiness validation semantics; no transition validation "
            "semantics; no package validation semantics; no verdict semantics; "
...
def list_entry_gate_design_boundary_contract_fields() -> tuple[EntryGateDesignBoundaryContractField, ...]:
    return _ENTRY_GATE_DESIGN_BOUNDARY_CONTRACT_FIELDS


def entry_gate_design_boundary_contract_field_ids() -> tuple[str, ...]:
    return tuple(
        field.field_id for field in list_entry_gate_design_boundary_contract_fields()
    )


def render_entry_gate_design_boundary_contract_markdown(
    fields: Sequence[EntryGateDesignBoundaryContractField] | None = None,
) -> str:
    field_values = tuple(fields) if fields is not None else list_entry_gate_design_boundary_contract_fields()
    ...
    lines.extend(["", "## Entry Gate Design Boundary Contract Fields", ""])
    for field in field_values:
        lines.extend([
            ...
            "- P4-M4 entry gate design boundary contract category: "
            f"{field.p4_m4_entry_gate_design_boundary_contract_category}",
            "- P4-M4 entry gate design boundary contract semantics disabled: "
            f"{field.p4_m4_entry_gate_design_boundary_contract_semantics_disabled}",
            "",
        ])
    return "\n".join(lines).rstrip() + "\n"


def entry_gate_design_boundary_contract_as_dicts() -> tuple[dict[str, object], ...]:
    return tuple(asdict(field) for field in list_entry_gate_design_boundary_contract_fields())


def entry_gate_design_boundary_contract_report() -> dict[str, object]:
    status: dict[str, object] = {
        "phase": "P4-M4.0",
        "feature": "Entry Gate Design Boundary Contract",
        "mode": "read-only",
        "boundary": ENTRY_GATE_DESIGN_BOUNDARY_CONTRACT_BOUNDARY,
        "package_version": P4_M4_0_PACKAGE_VERSION,
        "entry_gate_design_boundary_contract_field_count": len(_FIELD_IDS),
        "referenced_p4_m3_16_final_phase_handoff_summary_field_count": len(
            governed_transition_intake_final_phase_handoff_summary_field_ids()
        ),
    }
