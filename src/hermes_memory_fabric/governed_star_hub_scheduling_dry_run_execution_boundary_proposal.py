"""Governed Star-Hub scheduling dry-run execution boundary proposal for v2.23 gates."""
...
GOVERNED_STAR_HUB_SCHEDULING_DRY_RUN_EXECUTION_BOUNDARY_PROPOSAL_VERSION = "2.24.0"
SOURCE_REVIEW_GATE_VERSION = "2.23.0"
...
LAYER_MAPPING = {
    ...
    "direction": "Star-Hub scheduling dry-run plan boundary review gate -> Star-Hub scheduling dry-run execution boundary proposal",
}
...
def build_governed_star_hub_scheduling_dry_run_execution_boundary_proposal(
    review_gate_report: Mapping[str, Any],
) -> dict[str, Any]:
    """Return a local-only Star-Hub dry-run execution boundary proposal."""
    ...
    ready = status == "star_hub_scheduling_dry_run_execution_boundary_proposal_ready"

    return {
        "version": GOVERNED_STAR_HUB_SCHEDULING_DRY_RUN_EXECUTION_BOUNDARY_PROPOSAL_VERSION,
        "status": status,
        **PROPOSAL_FLAGS,
        ...
        "star_hub_scheduling_dry_run_execution_boundary_summary": _proposal_summary(
            ready
        ),
        ...
        "proposed_star_hub_dry_run_execution_boundary": (
            _proposed_star_hub_dry_run_execution_boundary(ready)
        ),
        ...
        "next_allowed_step": (
            READY_NEXT_ALLOWED_STEP
            if ready
            else "resolve_v2_24_0_star_hub_scheduling_dry_run_execution_boundary_proposal_blockers"
        ),
...
def governed_star_hub_scheduling_dry_run_execution_boundary_proposal_to_json(
    report: Mapping[str, Any],
) -> str:
    """Serialize a Star-Hub dry-run execution boundary proposal deterministically."""
...
def _proposal_checks(report: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    _add_check(
        ...
        "source_review_gate_version_must_be_2_23_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_review_gate_status_must_be_star_hub_scheduling_dry_run_plan_boundary_review_gate_ready",
    )
    ...
    if isinstance(mapping, Mapping):
        ...
        _add_check(
            ...
            "source_primary_layer_status_must_be_review_gate_only_not_scheduling_not_dry_run_execution",
        )
        _add_check(
            checks,
            blocking_reasons,
    ...
    _add_check(
        ...
        "source_reviewed_chain_versions_must_exactly_match_v2_9_0_through_v2_22_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_next_allowed_step_must_be_v2_24_0_star_hub_scheduling_dry_run_execution_boundary_proposal",
    )
    _add_check(
        checks,
        blocking_reasons,
    ...
    reviewed = report.get("reviewed_star_hub_dry_run_plan_boundary")
    _add_check(
        checks,
        blocking_reasons,
        "source_reviewed_star_hub_dry_run_plan_boundary_shape",
        isinstance(reviewed, Mapping),
        "source_reviewed_star_hub_dry_run_plan_boundary_must_be_mapping",
    )
    ...
    if isinstance(boundary, Mapping):
        for key in [
            "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_star_hub_scheduling",
            "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_dry_run_execution",
            "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_star_hub_handoff",
            "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_authorization",
            "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_civilization_core_completion",
        ]:
            _add_check(
                checks,
                blocking_reasons,
...
def _proposal_summary(ready: bool) -> dict[str, Any]:
    return {
        "proposal_status": (
            "ready_for_later_star_hub_scheduling_dry_run_execution_boundary_review_gate"
            if ready
            else "blocked"
        ),
        "source_review_gate_version": SOURCE_REVIEW_GATE_VERSION,
        "source_review_gate_ready": ready,
        "future_star_hub_scheduling_dry_run_execution_boundary_proposed_for_human_review_only": ready,
        ...
        "star_hub_handoff_authorized": False,
        "civilization_core_complete_claimed": False,
        "only_permits_later_dry_run_execution_boundary_review_gate": ready,
    }
...
def _source_review_gate_summary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "source_review_gate_ready": ready,
        "source_review_gate_authorized_dry_run_plan_review": False,
...
def _proposed_star_hub_dry_run_execution_boundary(ready: bool) -> dict[str, Any]:
    status = (
        "star_hub_scheduling_dry_run_execution_boundary_proposed_for_human_review_only"
        if ready
        else "blocked_no_star_hub_scheduling_dry_run_execution_boundary_proposal_readiness_claim"
    )
    return {
        "proposal_status": status,
        "boundary_type": "star_hub_scheduling_dry_run_execution_boundary_proposal",
        "source_review_gate_version": SOURCE_REVIEW_GATE_VERSION,
        "star_hub_dry_run_execution_boundary_proposed": ready,
        "dry_run_execution_boundary_authorized": False,
        "dry_run_execution_authorized": False,
        "dry_run_performed": False,
        "dry_run_executed": False,
        ...
        "star_dome_final_closure_claimed": False,
        "proposed_chain_versions": list(PROPOSED_CHAIN_VERSIONS),
        "dry_run_execution_boundary_components": {
            "dry_run_execution_scope_boundary": "future_dry_run_execution_scope_may_be_described_for_human_review_only",
            "scheduler_scope_boundary": "star_hub_scheduling_remains_unauthorized_and_unperformed",
            "no_real_execution_boundary": "no_dry_run_is_performed_or_executed_in_this_proposal",
...
def _non_authorization_boundary() -> dict[str, Any]:
    return {
        ...
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_approval": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_human_decision": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_real_approval_request": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_request_submission": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_request_execution": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_memory_write_permission": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_memory_graph_mutation_permission": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_operation_ledger_creation": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_openclaw_execution_permission": False,
        "star_hub_scheduling_dry_run_execution_boundary_proposal_ready_is_civilization_core_completion": False,
        "only_permits_later_star_hub_scheduling_dry_run_execution_boundary_review_gate": True,
...
def _preserved_governance_boundaries() -> list[str]:
    return [
        "Star-Hub scheduling dry-run execution boundary proposal ready is not authorization.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not scheduling.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not dry-run execution.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not handoff.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not approval.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not a real human decision.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not a real approval request.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not approval request submission.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not approval request execution.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not durable memory write permission.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not Memory Graph mutation permission.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not operation-ledger creation.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not OpenClaw execution permission.",
        "Star-Hub scheduling dry-run execution boundary proposal ready is not Civilization Core completion.",
        "Star-Hub scheduling dry-run execution boundary proposal ready only permits a later Star-Hub scheduling dry-run execution boundary review gate.",
...
def _future_execution_constraints() -> list[str]:
    return [
        "Future dry-run execution boundary review gate must remain governed and human-supervised.",
        "Future dry-run execution boundary review gate must not schedule anything in this proposal stage.",
        "Future dry-run execution boundary review gate must not perform or execute a dry-run in this proposal stage.",
        "Future dry-run execution boundary review gate must not authorize Star-Hub scheduling.",
        "Future dry-run execution boundary review gate must not authorize Star-Hub handoff.",
        "Future dry-run execution boundary review gate must not write durable memory.",
        "Future dry-run execution boundary review gate must not mutate Memory Graph.",
        "Future dry-run execution boundary review gate must not create operation-ledger entries.",
        "Future dry-run execution boundary review gate must not call OpenClaw.",
        "Future dry-run execution boundary review gate must not call GitHub APIs.",
        "Future dry-run execution boundary review gate must not create, submit, or execute approval requests.",
        "Future dry-run execution boundary review gate must not record human decisions.",
        "Future dry-run execution boundary review gate must preserve the Fifteen Memory Layers as the highest coordinate system.",
    ]


def _next_stage_constraints() -> list[str]:
    return [
        "The only next allowed step is v2.25.0 Star-Hub scheduling dry-run execution boundary review gate.",
        "v2.24.0 proposes a dry-run execution boundary only; it must not schedule anything.",
        "v2.24.0 must not perform or execute a dry-run.",
        "v2.24.0 must not authorize dry-run execution.",
...
def _scope_boundaries() -> list[str]:
    return [
        ...
        "This v2.24.0 step is limited to 星枢记忆 scheduling dry-run execution boundary proposal only, not scheduling and not dry-run execution.",
...
def _risk_notes(sensitive_field_count: int) -> list[str]:
    notes = [
        "Scheduling dry-run execution boundary proposal ready must not be treated as authorization.",
...
def _required_human_actions(status: str, blocking_reasons: list[str]) -> list[str]:
    if status == "star_hub_scheduling_dry_run_execution_boundary_proposal_ready":
        return [
            "human_operator_must_review_dry_run_execution_boundary_proposal_before_any_later_review_gate",
            "human_operator_must_confirm_execution_boundary_proposal_ready_is_not_authorization",
    ...
    actions = [
        "repair_source_star_hub_scheduling_dry_run_plan_boundary_review_gate_before_execution_boundary_proposal_can_continue",
        "confirm_all_write_approval_execution_handoff_scheduling_dry_run_completion_and_final_closure_flags_remain_false",
        "rerun_local_dry_run_execution_boundary_proposal_after_blocking_reasons_are_resolved",
    ]
    if blocking_reasons:
        actions.append(
            "resolve_blocking_reasons_before_dry_run_execution_boundary_proposal_can_be_ready"
        )
    return actions
...
__all__ = [
    "GOVERNED_STAR_HUB_SCHEDULING_DRY_RUN_EXECUTION_BOUNDARY_PROPOSAL_VERSION",
