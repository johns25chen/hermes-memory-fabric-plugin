"""Governed Star-Law candidate rule-set boundary review gate for v2.31 proposals."""
...
GOVERNED_STAR_LAW_CANDIDATE_RULE_SET_BOUNDARY_REVIEW_GATE_VERSION = "2.32.0"
SOURCE_PROPOSAL_VERSION = "2.31.0"
...
BLOCKED_NEXT_ALLOWED_STEP = (
    "resolve_v2_32_0_star_law_candidate_rule_set_boundary_review_gate_blockers"
)
...
SOURCE_REQUIRED_FLAGS = {
    ...
    "candidate_rule_set_boundary_proposed_for_human_review_only": True,
    "candidate_rule_set_review_gate_ready_for_human_review_only": True,
    "star_law_candidate_rules_created": False,
    "star_law_rules_created": False,
    "star_law_rules_activated": False,
    "star_law_rules_enforced": False,
...
SOURCE_PROPOSAL_BOUNDARY_REQUIRED_FLAGS = {
    "candidate_rule_set_boundary_proposed": True,
    "candidate_rule_set_boundary_review_gate_ready_for_human_review_only": True,
    "source_star_law_design_boundary_review_gate_valid": True,
    "star_law_candidate_rules_created": False,
    "star_law_rules_created": False,
    "star_law_rules_activated": False,
    "star_law_rules_enforced": False,
...
REVIEWED_BOUNDARY_ALLOWED_TRUE_KEYS = {
    "review_only",
}
...
SOURCE_PROPOSED_CANDIDATE_RULE_SET_BOUNDARY_KEYS = {
...
REVIEW_COMPONENTS = {
    "source_candidate_rule_set_boundary_proposal_review",
    "candidate_rule_taxonomy_boundary_review",
    "candidate_rule_eligibility_boundary_review",
    "candidate_rule_input_scope_boundary_review",
    "candidate_rule_output_scope_boundary_review",
    "candidate_rule_non_creation_boundary_review",
...
def build_governed_star_law_candidate_rule_set_boundary_review_gate(
    proposal_report: Mapping[str, Any],
) -> dict[str, Any]:
    """Return a local-only, non-authorizing candidate rule-set review gate."""
    ...
    ready = status == "star_law_candidate_rule_set_boundary_review_gate_ready"

    return {
        "version": GOVERNED_STAR_LAW_CANDIDATE_RULE_SET_BOUNDARY_REVIEW_GATE_VERSION,
        "status": status,
        **REVIEW_GATE_FLAGS,
        "candidate_rule_set_boundary_reviewed_for_human_review_only": ready,
        "candidate_rule_activation_boundary_proposal_ready_for_human_review_only": ready,
        ...
        "star_law_candidate_rule_set_boundary_review_gate_summary": (
        ...
        "star_law_candidate_rule_set_boundary_review_gate": (
            _star_law_candidate_rule_set_boundary_review_gate(ready)
        ),
        "reviewed_candidate_rule_set_boundaries": (
            _reviewed_candidate_rule_set_boundaries(ready)
        ),
        ...
        "future_candidate_rule_activation_boundary_constraints": (
            _future_candidate_rule_activation_boundary_constraints()
        ),
...
def governed_star_law_candidate_rule_set_boundary_review_gate_to_json(
    report: Mapping[str, Any],
) -> str:
    """Serialize a Star-Law candidate rule-set boundary review gate deterministically."""
...
def _review_gate_checks(
    ...
    _add_check(
        ...
        "source_proposal_version_must_be_2_31_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        "source_proposal_status_ready",
        report.get("status") == "star_law_candidate_rule_set_boundary_proposal_ready",
        "source_proposal_status_must_be_star_law_candidate_rule_set_boundary_proposal_ready",
    )
    ...
    _add_check(
        ...
        "source_must_not_claim_candidate_rule_creation_star_law_rule_creation_activation_enforcement_self_enforcing_law_autonomous_execution_handoff_scheduling_approval_write_execution_dry_run_or_civilization_core_completion",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_must_not_textually_claim_candidate_rule_creation_star_law_rule_creation_activation_enforcement_self_enforcing_law_autonomous_execution_write_approval_next_layer_or_civilization_core_completion",
    )
    ...
    if isinstance(mapping, Mapping):
        ...
        _add_check(
            ...
            "source_primary_layer_status_must_be_candidate_rule_set_boundary_proposal_only_not_rule_creation_activation_enforcement",
        )
        _add_check(
            checks,
            blocking_reasons,
        ...
        _add_check(
            ...
            "source_source_layer_status_must_be_design_boundary_review_gate_complete_for_human_review_only",
        )
        _add_check(
            checks,
            blocking_reasons,
        ...
        _add_check(
            ...
            "source_direction_must_be_design_boundary_review_gate_to_candidate_rule_set_boundary_proposal",
        )
    ...
    _add_check(
        ...
        "source_proposed_chain_versions_must_exactly_match_v2_9_0_through_v2_30_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        == SOURCE_PROPOSED_STAR_HUB_CHAIN_VERSIONS,
        "source_proposed_star_hub_chain_versions_must_exactly_match_v2_19_0_through_v2_30_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        == "v2.32.0 Star-Law candidate rule-set boundary review gate",
        "source_next_allowed_step_must_be_v2_32_0_star_law_candidate_rule_set_boundary_review_gate",
    )
    _add_check(
        checks,
        blocking_reasons,
    ...
    proposal = report.get("star_law_candidate_rule_set_boundary_proposal")
    _add_check(
        checks,
        blocking_reasons,
        "source_star_law_candidate_rule_set_boundary_proposal_shape",
        isinstance(proposal, Mapping),
        "source_star_law_candidate_rule_set_boundary_proposal_must_be_mapping",
    )
    if isinstance(proposal, Mapping):
        _add_check(
            checks,
            blocking_reasons,
            "source_candidate_rule_set_boundary_proposal_status",
            proposal.get("proposal_status")
            == "star_law_candidate_rule_set_boundary_proposed_for_human_review_only",
            "source_candidate_rule_set_boundary_proposal_status_must_be_human_review_only",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_candidate_rule_set_boundary_proposal_boundary_type",
            proposal.get("boundary_type")
            == "star_law_candidate_rule_set_boundary_proposal",
            "source_candidate_rule_set_boundary_proposal_boundary_type_must_match",
        )
        for key, expected in SOURCE_PROPOSAL_BOUNDARY_REQUIRED_FLAGS.items():
            _add_check(
                checks,
                blocking_reasons,
                f"source_candidate_rule_set_boundary_proposal_{key}",
                proposal.get(key) is expected,
                f"source_candidate_rule_set_boundary_proposal_{key}_must_be_{str(expected).lower()}",
            )
        _add_check(
            checks,
            blocking_reasons,
            "source_candidate_rule_set_boundary_proposal_chain_versions",
            proposal.get("proposed_chain_versions")
            == SOURCE_PROPOSED_CHAIN_VERSIONS,
            "source_candidate_rule_set_boundary_proposal_chain_versions_must_match",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_candidate_rule_set_boundary_proposal_star_hub_chain_versions",
            proposal.get("proposed_star_hub_chain_versions")
            == SOURCE_PROPOSED_STAR_HUB_CHAIN_VERSIONS,
            "source_candidate_rule_set_boundary_proposal_star_hub_chain_versions_must_match",
        )

    boundaries = report.get("proposed_candidate_rule_set_boundaries")
    _add_check(
        checks,
        blocking_reasons,
        "source_proposed_candidate_rule_set_boundaries_shape",
        isinstance(boundaries, Mapping),
        "source_proposed_candidate_rule_set_boundaries_must_be_mapping",
    )
    if isinstance(boundaries, Mapping):
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_candidate_rule_set_boundaries_required_keys",
            set(boundaries) == SOURCE_PROPOSED_CANDIDATE_RULE_SET_BOUNDARY_KEYS,
            "source_proposed_candidate_rule_set_boundaries_must_exactly_match_required_boundaries",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_candidate_rule_set_boundaries_proposal_only",
            _proposed_boundaries_remain_proposal_only(boundaries),
            "source_proposed_candidate_rule_set_boundaries_must_remain_proposal_only_and_non_authorizing",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_candidate_rule_set_boundaries_no_unsafe_claims",
            _unsafe_boundary_claims(boundaries, BOUNDARY_ALLOWED_TRUE_KEYS) == [],
            "source_proposed_candidate_rule_set_boundaries_must_not_claim_candidate_rule_creation_rule_creation_activation_enforcement_authorization_self_execution_autonomous_execution_or_memory_writing",
        )
...
def _review_gate_summary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "source_proposal_version": SOURCE_PROPOSAL_VERSION,
        "source_star_law_candidate_rule_set_boundary_proposal_valid": ready,
        "candidate_rule_set_boundary_reviewed_for_human_review_only": ready,
        "candidate_rule_activation_boundary_proposal_ready_for_human_review_only": ready,
        ...
        "civilization_core_complete_claimed": False,
        "only_permits_later_star_law_candidate_rule_activation_boundary_proposal": ready,
    }
...
def _source_proposal_summary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "source_candidate_rule_set_boundary_proposal_ready": ready,
...
def _star_law_candidate_rule_set_boundary_review_gate(
    ready: bool,
) -> dict[str, Any]:
    return {
        "review_status": (
            "star_law_candidate_rule_set_boundary_reviewed_for_human_review_only"
            if ready
            else "blocked_no_star_law_candidate_rule_set_boundary_review_readiness_claim"
        ),
        "boundary_type": "star_law_candidate_rule_set_boundary_review_gate",
        "source_proposal_version": SOURCE_PROPOSAL_VERSION,
        "candidate_rule_set_boundary_reviewed": ready,
        "candidate_rule_set_boundary_structurally_review_ready": ready,
        "candidate_rule_activation_boundary_proposal_ready_for_human_review_only": ready,
        "source_star_law_candidate_rule_set_boundary_proposal_valid": ready,
        "star_law_candidate_rules_created": False,
        "star_law_rules_created": False,
        ...
        "reviewed_chain_versions": list(REVIEWED_CHAIN_VERSIONS),
        "reviewed_star_hub_chain_versions": list(REVIEWED_STAR_HUB_CHAIN_VERSIONS),
        "star_law_candidate_rule_set_boundary_review_components": (
        ...
        "non_authorization_notice": (
            "The Star-Law candidate rule-set boundary proposal has been reviewed for human review only. "
            "The candidate rule activation boundary proposal is structurally ready for a later governed proposal step. "
            "This review gate creates no candidate Star-Law rules, creates no Star-Law rules, activates none, enforces none, creates no self-enforcing memory law, creates no autonomous governance, authorizes no autonomous execution, creates no self-executing policy, grants no approval, records no real human decision, performs no handoff, schedules nothing, performs no dry-run, executes no dry-run plan, writes no durable memory, mutates no Memory Graph, creates no operation-ledger entry, calls no OpenClaw or GitHub API, claims no Civilization Core completion, starts no 星魂 continuity, starts no 星宙 evolution, and starts no 星源 self-evolution."
            if ready
            else "The Star-Law candidate rule-set boundary review gate is blocked; no readiness, authorization, candidate rule creation, rule creation, rule activation, rule enforcement, autonomous execution, handoff, scheduling, approval, write, execution, dry-run, next-layer, or Civilization Core completion claim is made."
        ),
    }
...
def _review_components(ready: bool) -> dict[str, str]:
    ...
    return {
        component: status
        for component in sorted(REVIEW_COMPONENTS)
    }


def _reviewed_candidate_rule_set_boundaries(ready: bool) -> dict[str, Any]:
    status = "reviewed_for_human_review_only" if ready else "blocked"
    summaries = {
        "candidate_rule_taxonomy_boundary": "Candidate taxonomy boundary was structurally reviewed without creating candidate rules.",
        "candidate_rule_eligibility_boundary": "Candidate eligibility boundary was structurally reviewed without granting authorization.",
        "candidate_rule_input_scope_boundary": "Candidate input scope boundary was structurally reviewed using sanitized chain and governance status only.",
        "candidate_rule_output_scope_boundary": "Candidate output scope boundary was structurally reviewed without creating executable policy.",
        "candidate_rule_non_creation_boundary": "Candidate rule non-creation boundary was structurally reviewed.",
        ...
        "candidate_rule_evidence_lineage_boundary": "Evidence-lineage boundary was structurally reviewed without creating operation-ledger entries.",
        ...
        "future_review_gate_boundary": "Future boundary was structurally reviewed for a later governed candidate rule activation boundary proposal step only.",
    }
    return {
        key: {
            "review_only": True,
...
def _non_authorization_boundary(ready: bool) -> dict[str, Any]:
    return {
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_self_enforcing_memory_law": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_candidate_rule_creation": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_rule_creation": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_rule_activation": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_rule_enforcement": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_autonomous_governance": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_autonomous_execution": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_handoff": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_authorization": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_scheduling": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_dry_run_execution": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_approval": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_human_decision": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_memory_write_permission": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_memory_graph_mutation_permission": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_operation_ledger_creation": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_openclaw_execution_permission": False,
        "star_law_candidate_rule_set_boundary_review_gate_ready_is_civilization_core_completion": False,
        "only_permits_later_star_law_candidate_rule_activation_boundary_proposal": ready,
...
def _preserved_governance_boundaries() -> list[str]:
    return [
        "Star-Law candidate rule-set boundary review gate ready is not self-enforcing memory law.",
        "Star-Law candidate rule-set boundary review gate ready is not candidate rule creation.",
        "Star-Law candidate rule-set boundary review gate ready is not Star-Law rule creation.",
        "Star-Law candidate rule-set boundary review gate ready is not rule activation.",
        "Star-Law candidate rule-set boundary review gate ready is not rule enforcement.",
        "Star-Law candidate rule-set boundary review gate ready is not autonomous governance.",
        "Star-Law candidate rule-set boundary review gate ready is not autonomous execution.",
        "Star-Law candidate rule-set boundary review gate ready is not handoff.",
        "Star-Law candidate rule-set boundary review gate ready is not authorization.",
        "Star-Law candidate rule-set boundary review gate ready is not scheduling.",
        "Star-Law candidate rule-set boundary review gate ready is not dry-run execution.",
        "Star-Law candidate rule-set boundary review gate ready is not approval.",
        "Star-Law candidate rule-set boundary review gate ready is not a real human decision.",
        "Star-Law candidate rule-set boundary review gate ready is not durable memory write permission.",
        "Star-Law candidate rule-set boundary review gate ready is not Memory Graph mutation permission.",
        "Star-Law candidate rule-set boundary review gate ready is not operation-ledger creation.",
        "Star-Law candidate rule-set boundary review gate ready is not OpenClaw execution permission.",
        "Star-Law candidate rule-set boundary review gate ready is not Civilization Core completion.",
        "Star-Law candidate rule-set boundary review gate ready only permits a later Star-Law candidate rule activation boundary proposal.",
        "Human Operator remains final authority.",
    ]


def _future_candidate_rule_activation_boundary_constraints() -> list[str]:
    return [
        "A later Star-Law candidate rule activation boundary proposal may inspect this review gate through a separate governed process.",
        "A later candidate rule activation boundary proposal must not be treated as automatic candidate rule creation, Star-Law rule creation, activation, enforcement, approval, handoff, scheduling, memory writing, Memory Graph mutation, operation-ledger creation, OpenClaw execution, GitHub API action, dry-run execution, autonomous execution, or Civilization Core completion.",
        "Any future Star-Law candidate rule activation boundary must remain inactive until a separate Human Operator governed process explicitly authorizes an applicable later stage.",
...
def _next_stage_constraints() -> list[str]:
    return [
        "The only next allowed step is v2.33.0 Star-Law candidate rule activation boundary proposal.",
        "v2.32.0 is Star-Law candidate rule-set boundary review gate only, not rule creation, not rule activation, and not rule enforcement.",
        "v2.32.0 must not create candidate rules, create Star-Law rules, activate rules, or enforce rules.",
        "v2.32.0 must not authorize Star-Hub handoff, scheduling, approval, dry-run execution, memory writing, Memory Graph mutation, operation-ledger creation, OpenClaw execution, or GitHub API actions.",
...
def _scope_boundaries() -> list[str]:
    return [
        ...
        "This v2.32.0 step is limited to 星律记忆 Star-Law candidate rule-set boundary review gate only.",
...
def _risk_notes(sensitive_field_count: int) -> list[str]:
    notes = [
        "Star-Law candidate rule-set boundary review gate ready must not be treated as self-enforcing memory law, candidate rule creation, rule creation, rule activation, rule enforcement, autonomous governance, autonomous execution, handoff, authorization, scheduling, approval, dry-run execution, or Civilization Core completion.",
    ...
    if sensitive_field_count:
        notes.append(
            "Sensitive fields were omitted from candidate rule-set boundary review gate output."
        )
    return notes


def _required_human_actions(status: str, blocking_reasons: list[str]) -> list[str]:
    if status == "star_law_candidate_rule_set_boundary_review_gate_ready":
        return [
            "human_operator_must_review_star_law_candidate_rule_set_boundary_review_gate_before_any_later_candidate_rule_activation_boundary_proposal",
            "human_operator_must_confirm_candidate_rule_set_boundary_review_gate_ready_is_not_self_enforcing_law_candidate_rule_creation_rule_creation_rule_activation_rule_enforcement_autonomous_execution_handoff_authorization_scheduling_approval_write_execution_or_civilization_core_completion",
    ...
    actions = [
        "repair_source_v2_31_0_star_law_candidate_rule_set_boundary_proposal_before_review_gate_can_continue",
        "confirm_all_candidate_rule_creation_star_law_rule_creation_rule_activation_rule_enforcement_autonomous_execution_handoff_scheduling_dry_run_write_approval_execution_next_layer_and_civilization_core_completion_flags_remain_false",
        "rerun_local_star_law_candidate_rule_set_boundary_review_gate_after_blocking_reasons_are_resolved",
    ]
    if blocking_reasons:
        actions.append(
            "resolve_blocking_reasons_before_star_law_candidate_rule_set_boundary_review_gate_can_be_ready"
        )
    return actions
...
__all__ = [
    "GOVERNED_STAR_LAW_CANDIDATE_RULE_SET_BOUNDARY_REVIEW_GATE_VERSION",
    "REVIEWED_CHAIN_VERSIONS",
    "REVIEWED_STAR_HUB_CHAIN_VERSIONS",
    "READY_NEXT_ALLOWED_STEP",
    "build_governed_star_law_candidate_rule_set_boundary_review_gate",
    "governed_star_law_candidate_rule_set_boundary_review_gate_to_json",
]
