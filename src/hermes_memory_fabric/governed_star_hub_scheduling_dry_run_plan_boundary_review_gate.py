"""Governed Star-Hub scheduling dry-run plan boundary review gate for v2.22 proposals."""
...
GOVERNED_STAR_HUB_SCHEDULING_DRY_RUN_PLAN_BOUNDARY_REVIEW_GATE_VERSION = "2.23.0"
SOURCE_PROPOSAL_VERSION = "2.22.0"
...
LAYER_MAPPING = {
    ...
    "direction": "Star-Hub scheduling dry-run plan boundary proposal -> Star-Hub scheduling dry-run plan boundary review gate",
}

READY_NEXT_ALLOWED_STEP = "v2.24.0 Star-Hub scheduling dry-run execution boundary proposal"
...
UNSAFE_TRUE_FIELDS = {
    ...
    "dry_run_plan_review_authorized",
...
def build_governed_star_hub_scheduling_dry_run_plan_boundary_review_gate(
    proposal_report: Mapping[str, Any],
) -> dict[str, Any]:
    """Return a local-only, non-authorizing Star-Hub dry-run plan boundary review gate."""
    ...
    status = (
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready"
        if not blocking_reasons
        else "blocked"
    )
    ready = status == "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready"

    return {
        "version": GOVERNED_STAR_HUB_SCHEDULING_DRY_RUN_PLAN_BOUNDARY_REVIEW_GATE_VERSION,
        "status": status,
        **REVIEW_GATE_FLAGS,
        ...
        "star_hub_scheduling_dry_run_plan_boundary_review_summary": _review_summary(ready),
        "review_gate_checks": checks,
        "source_proposal_summary": _source_proposal_summary(ready),
        "reviewed_star_hub_dry_run_plan_boundary": _reviewed_star_hub_dry_run_plan_boundary(
            ready
        ),
        "non_authorization_boundary": _non_authorization_boundary(),
        "preserved_governance_boundaries": _preserved_governance_boundaries(),
        "future_dry_run_execution_constraints": _future_dry_run_execution_constraints(),
        ...
        "next_allowed_step": (
            ...
            if ready
            else "resolve_v2_23_0_star_hub_scheduling_dry_run_plan_boundary_review_gate_blockers"
        ),
...
def governed_star_hub_scheduling_dry_run_plan_boundary_review_gate_to_json(
    report: Mapping[str, Any],
) -> str:
    """Serialize a Star-Hub scheduling dry-run plan boundary review gate deterministically."""
...
def _review_gate_checks(report: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    _add_check(
        ...
        "source_proposal_version_must_be_2_22_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        "source_proposal_status_ready",
        report.get("status") == "star_hub_scheduling_dry_run_plan_boundary_proposal_ready",
        "source_proposal_status_must_be_star_hub_scheduling_dry_run_plan_boundary_proposal_ready",
    )
    ...
    if isinstance(mapping, Mapping):
        ...
        _add_check(
            ...
            == "scheduling dry-run plan boundary proposal only, not scheduling",
            "source_primary_layer_status_must_be_scheduling_dry_run_plan_boundary_proposal_only_not_scheduling",
        )
        _add_check(
            checks,
            blocking_reasons,
    ...
    _add_check(
        ...
        "source_proposed_chain_versions_must_exactly_match_v2_9_0_through_v2_21_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        == "v2.23.0 Star-Hub scheduling dry-run plan boundary review gate",
        "source_next_allowed_step_must_be_v2_23_0_star_hub_scheduling_dry_run_plan_boundary_review_gate",
    )
    _add_check(
        checks,
        blocking_reasons,
    ...
    proposed = report.get("proposed_star_hub_dry_run_plan_boundary")
    _add_check(
        checks,
        blocking_reasons,
        "source_proposed_star_hub_dry_run_plan_boundary_shape",
        isinstance(proposed, Mapping),
        "source_proposed_star_hub_dry_run_plan_boundary_must_be_mapping",
    )
    ...
    if isinstance(boundary, Mapping):
        for key in [
            "star_hub_scheduling_dry_run_plan_boundary_proposal_ready_is_star_hub_scheduling",
            "star_hub_scheduling_dry_run_plan_boundary_proposal_ready_is_dry_run_execution",
            "star_hub_scheduling_dry_run_plan_boundary_proposal_ready_is_star_hub_handoff",
            "star_hub_scheduling_dry_run_plan_boundary_proposal_ready_is_authorization",
            "star_hub_scheduling_dry_run_plan_boundary_proposal_ready_is_civilization_core_completion",
        ]:
            _add_check(
                checks,
                blocking_reasons,
...
def _review_summary(ready: bool) -> dict[str, Any]:
    return {
        "review_status": (
            "ready_for_later_star_hub_scheduling_dry_run_execution_boundary_proposal"
            if ready
            else "blocked"
        ),
        "source_proposal_version": SOURCE_PROPOSAL_VERSION,
        "source_proposal_ready": ready,
        "future_star_hub_scheduling_dry_run_plan_boundary_reviewed_for_human_review_only": ready,
        ...
        "star_hub_handoff_authorized": False,
        "civilization_core_complete_claimed": False,
        "only_permits_later_dry_run_execution_boundary_proposal": ready,
    }
...
def _source_proposal_summary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "source_dry_run_plan_boundary_proposal_ready": ready,
        "source_proposal_authorized_dry_run_plan": False,
...
def _reviewed_star_hub_dry_run_plan_boundary(ready: bool) -> dict[str, Any]:
    status = (
        "star_hub_scheduling_dry_run_plan_boundary_reviewed_for_human_review_only"
        if ready
        else "blocked_no_star_hub_scheduling_dry_run_plan_boundary_review_gate_readiness_claim"
    )
    return {
        "review_status": status,
        "boundary_type": "star_hub_scheduling_dry_run_plan_boundary_review_gate",
        "source_proposal_version": SOURCE_PROPOSAL_VERSION,
        "star_hub_dry_run_plan_boundary_reviewed": ready,
        "dry_run_plan_review_authorized": False,
        "dry_run_execution_authorized": False,
        "dry_run_performed": False,
        "dry_run_executed": False,
        ...
        "star_dome_final_closure_claimed": False,
        "reviewed_chain_versions": list(REVIEWED_CHAIN_VERSIONS),
        "dry_run_plan_review_gate_components": {
            "scheduler_scope_review": "review_future_scheduler_scope_boundary_without_scheduling",
            "dry_run_execution_boundary_review": "future_dry_run_execution_remains_unauthorized_and_unperformed",
...
def _non_authorization_boundary() -> dict[str, Any]:
    return {
        ...
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_approval": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_human_decision": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_real_approval_request": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_request_submission": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_request_execution": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_memory_write_permission": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_memory_graph_mutation_permission": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_operation_ledger_creation": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_openclaw_execution_permission": False,
        "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready_is_civilization_core_completion": False,
        "only_permits_later_star_hub_scheduling_dry_run_execution_boundary_proposal": True,
...
def _preserved_governance_boundaries() -> list[str]:
    return [
        "Star-Hub scheduling dry-run plan boundary review gate ready is not authorization.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not scheduling.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not dry-run execution.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not handoff.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not approval.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not a real human decision.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not a real approval request.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not approval request submission.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not approval request execution.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not durable memory write permission.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not Memory Graph mutation permission.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not operation-ledger creation.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not OpenClaw execution permission.",
        "Star-Hub scheduling dry-run plan boundary review gate ready is not Civilization Core completion.",
        "Star-Hub scheduling dry-run plan boundary review gate ready only permits a later Star-Hub scheduling dry-run execution boundary proposal.",
        "Human Operator remains final authority.",
    ]


def _future_dry_run_execution_constraints() -> list[str]:
    return [
        "Future dry-run execution boundary proposal must remain governed and human-supervised.",
        "Future dry-run execution boundary proposal must not schedule anything in this review gate stage.",
        "Future dry-run execution boundary proposal must not perform or execute a dry-run in this review gate stage.",
        "Future dry-run execution boundary proposal must not authorize Star-Hub scheduling.",
        "Future dry-run execution boundary proposal must not authorize Star-Hub handoff.",
        "Future dry-run execution boundary proposal must not write durable memory.",
        "Future dry-run execution boundary proposal must not mutate Memory Graph.",
        "Future dry-run execution boundary proposal must not create operation-ledger entries.",
        "Future dry-run execution boundary proposal must not call OpenClaw.",
        "Future dry-run execution boundary proposal must not call GitHub APIs.",
        "Future dry-run execution boundary proposal must not create, submit, or execute approval requests.",
        "Future dry-run execution boundary proposal must not record human decisions.",
        "Future dry-run execution boundary proposal must preserve the Fifteen Memory Layers as the highest coordinate system.",
    ]


def _next_stage_constraints() -> list[str]:
    return [
        "The only next allowed step is v2.24.0 Star-Hub scheduling dry-run execution boundary proposal.",
        "v2.23.0 reviews a dry-run plan boundary only; it must not schedule anything.",
        "v2.23.0 must not perform or execute a dry-run.",
        "v2.23.0 must not authorize dry-run execution.",
...
def _scope_boundaries() -> list[str]:
    return [
        ...
        "This v2.23.0 step is limited to 星枢记忆 scheduling dry-run plan boundary review gate only, not scheduling and not dry-run execution.",
...
def _risk_notes(sensitive_field_count: int) -> list[str]:
    notes = [
        "Scheduling dry-run plan boundary review gate ready must not be treated as authorization.",
...
def _required_human_actions(status: str, blocking_reasons: list[str]) -> list[str]:
    if status == "star_hub_scheduling_dry_run_plan_boundary_review_gate_ready":
        return [
            "human_operator_must_review_dry_run_plan_boundary_review_gate_before_any_later_execution_boundary_proposal",
            "human_operator_must_confirm_review_gate_ready_is_not_authorization",
    ...
    actions = [
        "repair_source_star_hub_scheduling_dry_run_plan_boundary_proposal_before_review_gate_can_continue",
        "confirm_all_write_approval_execution_handoff_scheduling_dry_run_completion_and_final_closure_flags_remain_false",
        "rerun_local_dry_run_plan_boundary_review_gate_after_blocking_reasons_are_resolved",
    ]
    if blocking_reasons:
        actions.append("resolve_blocking_reasons_before_dry_run_plan_boundary_review_gate_can_be_ready")
    return actions
...
__all__ = [
    "GOVERNED_STAR_HUB_SCHEDULING_DRY_RUN_PLAN_BOUNDARY_REVIEW_GATE_VERSION",
