"""Governed Star-Law candidate rule violation response audit completion attestation boundary review gate."""
...
from .governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal import (
    PROPOSAL_FALSE_FLAGS as SOURCE_PROPOSAL_FALSE_FLAGS,
    PROPOSAL_OBJECT_FALSE_FLAGS as SOURCE_PROPOSAL_OBJECT_FALSE_FLAGS,
    PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_COMPLETION_ATTESTATION_BOUNDARY_KEYS as SOURCE_PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_COMPLETION_ATTESTATION_BOUNDARY_KEYS,
...
GOVERNED_STAR_LAW_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_COMPLETION_ATTESTATION_BOUNDARY_REVIEW_GATE_VERSION = (
    "2.50.0"
)
SOURCE_PROPOSAL_VERSION = "2.49.0"

REVIEWED_CHAIN_VERSIONS = [*SOURCE_PROPOSED_CHAIN_VERSIONS, "v2.49.0"]
...
LAYER_MAPPING = {
    "primary_layer": "星律记忆",
    "primary_layer_status": "Star-Law candidate rule violation response audit completion attestation boundary review gate only, not attestation execution, not attestation record creation, not attestation-ledger creation, not completion execution, not finalization execution, not closure execution, not audit execution, not audit logging, not audit record creation, not audit-ledger creation, not violation response execution, not automated correction, not violation enforcement, and not autonomous execution",
    "source_layer": "星律记忆",
    "source_layer_status": "Star-Law candidate rule violation response audit completion attestation boundary proposal complete for human review only",
    "supporting_layers": ["星穹记忆", "星域记忆", "星界记忆", "星辰记忆"],
    "direction": "Star-Law candidate rule violation response audit completion attestation boundary proposal -> Star-Law candidate rule violation response audit completion attestation boundary review gate",
}
...
SOURCE_REQUIRED_FLAGS = {
    ...
    "candidate_rule_violation_response_audit_completion_attestation_boundary_proposed_for_human_review_only": True,
    "candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_ready_for_human_review_only": True,
    **SOURCE_PROPOSAL_FALSE_FLAGS,
}

SOURCE_PROPOSAL_OBJECT_REQUIRED_FLAGS = {
    "candidate_rule_violation_response_audit_completion_attestation_boundary_proposed": True,
    "candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_ready_for_human_review_only": True,
    "source_star_law_candidate_rule_violation_response_audit_completion_boundary_review_gate_valid": True,
...
REVIEW_COMPONENTS = {
    "source_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_review",
    "response_audit_completion_attestation_eligibility_boundary_review",
    "response_audit_completion_attestation_input_boundary_review",
    "response_audit_completion_attestation_output_boundary_review",
    "response_audit_completion_attestation_non_execution_boundary_review",
    "response_audit_completion_attestation_non_recording_boundary_review",
    "response_audit_completion_attestation_non_ledger_boundary_review",
    "response_audit_completion_attestation_non_authorization_boundary_review",
    ...
    "human_operator_attestation_review_boundary_review",
    "evidence_lineage_attestation_boundary_review",
    "auditability_attestation_design_boundary_review",
    "false_positive_attestation_prevention_boundary_review",
    "rollback_attestation_boundary_review",
    "suspension_attestation_boundary_review",
...
def build_governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate(
    proposal_report: Mapping[str, Any],
) -> dict[str, Any]:
    """Return a local-only, non-authorizing attestation boundary review gate."""
    ...
    return {
        "version": GOVERNED_STAR_LAW_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_COMPLETION_ATTESTATION_BOUNDARY_REVIEW_GATE_VERSION,
        "status": status,
        **REVIEW_GATE_FLAGS,
        "candidate_rule_violation_response_audit_completion_attestation_boundary_reviewed_for_human_review_only": ready,
        "candidate_rule_violation_response_audit_completion_attestation_closure_boundary_proposal_ready_for_human_review_only": ready,
        ...
        "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_summary": _review_gate_summary(
            ready
        ),
        "review_gate_checks": checks,
        "source_proposal_summary": _source_proposal_summary(ready),
        "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate": _review_gate(
            ready
        ),
        "reviewed_candidate_rule_violation_response_audit_completion_attestation_boundaries": _reviewed_boundaries(
            ready
        ),
        ...
        "future_candidate_rule_violation_response_audit_completion_attestation_closure_boundary_constraints": _future_attestation_closure_constraints(),
...
def governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_to_json(
    report: Mapping[str, Any],
) -> str:
    """Serialize the audit completion attestation boundary review gate deterministically."""
...
def _review_gate_checks(
    ...
    _add_check(
        ...
        "source_proposal_version_must_be_2_49_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_proposal_status_must_be_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_ready",
    )
    ...
    if isinstance(mapping, Mapping):
        ...
        layer_checks = {
            ...
            "source_primary_layer_status": (
                ...
                == "Star-Law candidate rule violation response audit completion attestation boundary proposal only, not attestation execution, not attestation record creation, not attestation-ledger creation, not completion execution, not finalization execution, not closure execution, not audit execution, not audit logging, not audit record creation, not audit-ledger creation, not violation response execution, not automated correction, not violation enforcement, and not autonomous execution",
                "source_primary_layer_status_must_be_audit_completion_attestation_boundary_proposal_only",
            ),
            "source_source_layer": (
            ...
            "source_source_layer_status": (
                ...
                == "Star-Law candidate rule violation response audit completion boundary review gate complete for human review only",
                "source_source_layer_status_must_be_audit_completion_boundary_review_gate_complete_for_human_review_only",
            ),
            ...
            "source_direction": (
                ...
                == "Star-Law candidate rule violation response audit completion boundary review gate -> Star-Law candidate rule violation response audit completion attestation boundary proposal",
                "source_direction_must_match_audit_completion_review_gate_to_audit_completion_attestation_boundary_proposal",
            ),
        }
    ...
    _add_check(
        ...
        "source_proposed_chain_versions_must_exactly_match_v2_9_0_through_v2_48_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        == SOURCE_PROPOSED_STAR_HUB_CHAIN_VERSIONS,
        "source_proposed_star_hub_chain_versions_must_exactly_match_v2_19_0_through_v2_48_0",
    )
    _add_check(
        checks,
        blocking_reasons,
        ...
        "source_next_allowed_step_must_be_v2_50_0_audit_completion_attestation_boundary_review_gate",
    )
    _add_check(
        checks,
        blocking_reasons,
    ...
    proposal = report.get(
        "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal"
    )
    _add_check(
        checks,
        blocking_reasons,
        "source_audit_completion_attestation_boundary_proposal_shape",
        isinstance(proposal, Mapping),
        "source_audit_completion_attestation_boundary_proposal_must_be_mapping",
    )
    if isinstance(proposal, Mapping):
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_completion_attestation_boundary_proposal_status",
            ...
            "source_audit_completion_attestation_boundary_proposal_status_must_be_human_review_only",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_completion_attestation_boundary_proposal_type",
            proposal.get("boundary_type")
            == "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal",
            "source_audit_completion_attestation_boundary_proposal_type_must_match",
        )
        for key, expected in SOURCE_PROPOSAL_OBJECT_REQUIRED_FLAGS.items():
            _add_check(
                checks,
                blocking_reasons,
                f"source_audit_completion_attestation_boundary_proposal_{key}",
                proposal.get(key) is expected,
                f"source_audit_completion_attestation_boundary_proposal_{key}_must_be_{str(expected).lower()}",
            )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_completion_attestation_boundary_proposal_chain_versions",
            proposal.get("proposed_chain_versions")
            == SOURCE_PROPOSED_CHAIN_VERSIONS,
            "source_audit_completion_attestation_boundary_proposal_chain_versions_must_match",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_completion_attestation_boundary_proposal_star_hub_chain_versions",
            proposal.get("proposed_star_hub_chain_versions")
            == SOURCE_PROPOSED_STAR_HUB_CHAIN_VERSIONS,
            "source_audit_completion_attestation_boundary_proposal_star_hub_chain_versions_must_match",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_audit_completion_attestation_boundary_proposal_next_allowed_step",
            proposal.get("next_allowed_step")
            == "v2.50.0 Star-Law candidate rule violation response audit completion attestation boundary review gate",
            "source_audit_completion_attestation_boundary_proposal_next_allowed_step_must_match",
        )

    boundaries = report.get(
        "proposed_candidate_rule_violation_response_audit_completion_attestation_boundaries"
    )
    _add_check(
        checks,
        blocking_reasons,
        "source_proposed_audit_completion_attestation_boundaries_shape",
        isinstance(boundaries, Mapping),
        "source_proposed_audit_completion_attestation_boundaries_must_be_mapping",
    )
    if isinstance(boundaries, Mapping):
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_audit_completion_attestation_boundaries_required_keys",
            set(boundaries)
            == SOURCE_PROPOSED_CANDIDATE_RULE_VIOLATION_RESPONSE_AUDIT_COMPLETION_ATTESTATION_BOUNDARY_KEYS,
            "source_proposed_audit_completion_attestation_boundaries_must_exactly_match_required_boundaries",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_audit_completion_attestation_boundaries_proposal_only",
            _proposed_boundaries_remain_proposal_only(boundaries),
            "source_proposed_audit_completion_attestation_boundaries_must_remain_proposal_only_and_non_authorizing",
        )
        _add_check(
            checks,
            blocking_reasons,
            "source_proposed_audit_completion_attestation_boundaries_no_unsafe_claims",
            _unsafe_boundary_claims(
                boundaries, PROPOSED_BOUNDARY_ALLOWED_TRUE_KEYS
            )
            == [],
            "source_proposed_audit_completion_attestation_boundaries_must_not_claim_execution_authorization_recording_enforcement_or_write_actions",
        )
...
def _source_proposal_summary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "source_audit_completion_attestation_boundary_proposal_valid": ready,
...
def _review_gate(ready: bool) -> dict[str, Any]:
    return {
        "review_status": (
            "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_reviewed_for_human_review_only"
            if ready
            else "blocked_no_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_readiness_claim"
        ),
        "boundary_type": "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate",
        "source_proposal_version": SOURCE_PROPOSAL_VERSION,
        "candidate_rule_violation_response_audit_completion_attestation_boundary_reviewed": ready,
        "candidate_rule_violation_response_audit_completion_attestation_boundary_structurally_review_ready": ready,
        "candidate_rule_violation_response_audit_completion_attestation_closure_boundary_proposal_ready_for_human_review_only": ready,
        "source_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_proposal_valid": ready,
        **REVIEW_OBJECT_FALSE_FLAGS,
        "reviewed_chain_versions": list(REVIEWED_CHAIN_VERSIONS),
        "reviewed_star_hub_chain_versions": list(REVIEWED_STAR_HUB_CHAIN_VERSIONS),
        "star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_components": _review_components(
            ready
        ),
        "next_allowed_step": READY_NEXT_ALLOWED_STEP,
        "non_authorization_notice": (
            "The Star-Law candidate rule violation response audit completion attestation boundary proposal has been reviewed for human review only. The candidate rule violation response audit completion attestation closure boundary proposal is structurally ready for a separate later governed proposal step. This review performs no attestation, completion, finalization, closure, audit, response, correction, enforcement, write, approval, handoff, scheduling, or autonomous action."
            if ready
            else "The Star-Law candidate rule violation response audit completion attestation boundary review gate is blocked and makes no readiness, execution, authorization, recording, ledger, enforcement, write, approval, handoff, scheduling, next-layer, or Civilization Core completion claim."
        ),
    }
...
def _reviewed_boundaries(ready: bool) -> dict[str, dict[str, Any]]:
    ...
    return {
        key: {
            ...
            "summary": "The source boundary was structurally reviewed from sanitized v2.49.0 proposal metadata only; no raw proposal data or authority was copied.",
...
def _non_authorization_boundary(ready: bool) -> dict[str, Any]:
    return {
        ...
        "only_permits_later_audit_completion_attestation_closure_boundary_proposal": ready,
...
def _future_attestation_closure_constraints() -> list[str]:
    return [
        "A later v2.51.0 audit completion attestation closure boundary proposal may consume this sanitized review report through a separate governed process.",
        "A later completion attestation closure boundary proposal remains proposal-only and cannot execute attestation, completion, finalization, closure, audit, response, correction, enforcement, writes, approvals, handoffs, scheduling, or autonomous actions.",
...
def _next_stage_constraints() -> list[str]:
    return [
        ...
        "v2.50.0 is a Star-Law candidate rule violation response audit completion attestation boundary review gate only.",
...
def _scope_boundaries() -> list[str]:
    return [
        ...
        "This v2.50.0 step is limited to 星律记忆 audit completion attestation boundary review only.",
    ]


def _risk_notes(sensitive_field_count: int) -> list[str]:
    notes = [
        "Review readiness must not be interpreted as attestation, completion, finalization, closure, audit, response, correction, enforcement, write, approval, handoff, scheduling, or autonomous authority.",
...
def _required_human_actions(
    ...
    if ready:
        return [
            "human_operator_must_review_v2_50_0_audit_completion_attestation_boundary_review_gate_before_any_v2_51_0_audit_completion_attestation_closure_boundary_proposal",
    ...
    actions = [
        "repair_source_v2_49_0_audit_completion_attestation_boundary_proposal_before_review_gate_can_continue",
        ...
        "rerun_local_v2_50_0_audit_completion_attestation_boundary_review_gate_after_blockers_are_resolved",
    ]
    if blocking_reasons:
        actions.append(
            "resolve_blocking_reasons_before_audit_completion_attestation_boundary_review_gate_can_be_ready"
        )
    return actions
...
__all__ = [
    ...
    "governed_star_law_candidate_rule_violation_response_audit_completion_attestation_boundary_review_gate_to_json",
]
